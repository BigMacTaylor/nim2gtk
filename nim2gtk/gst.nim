...
proc gst_allocation_params_new(): ptr AllocationParams00 {.
    importc, libprag.}

proc newAllocationParams*(): AllocationParams =
  fnew(result, gBoxedFreeGstAllocationParams)
  result.impl = gst_allocation_params_new()

proc newAllocationParams*(tdesc: typedesc): tdesc =
  assert(result is AllocationParams)
  fnew(result, gBoxedFreeGstAllocationParams)
  result.impl = gst_allocation_params_new()

proc initAllocationParams*[T](result: var T) {.deprecated.} =
  assert(result is AllocationParams)
  fnew(result, gBoxedFreeGstAllocationParams)
  result.impl = gst_allocation_params_new()
...
proc gst_object_default_deep_notify(obj: ptr gobject.Object00; orig: ptr Object00;
    pspec: ptr gobject.ParamSpec00; excludedProps: ptr cstring) {.
    importc, libprag.}

proc defaultDeepNotify*(obj: gobject.Object; orig: Object; pspec: gobject.ParamSpec;
    excludedProps: varargs[string, `$`]) =
  var fs469n23x: array[256, pointer]
  var fs469n23: cstringArray = cast[cstringArray](addr fs469n23x)
  gst_object_default_deep_notify(cast[ptr gobject.Object00](obj.impl), cast[ptr Object00](orig.impl), cast[ptr gobject.ParamSpec00](pspec.impl), seq2CstringArray(excludedProps, fs469n23))
...
proc gst_control_binding_sync_values(self: ptr ControlBinding00; obj: ptr Object00;
    timestamp: uint64; lastSync: uint64): gboolean {.
    importc, libprag.}

proc syncValues*(self: ControlBinding; obj: Object; timestamp: uint64;
    lastSync: uint64): bool =
  toBool(gst_control_binding_sync_values(cast[ptr ControlBinding00](self.impl), cast[ptr Object00](obj.impl), timestamp, lastSync))
...
proc copy*(self: Memory; offset: int64; size: int64): Memory =
  let impl0 = gst_memory_copy(cast[ptr Memory00](self.impl), offset, size)
  if impl0.isNil:
    return nil
...
proc typeSetSkipDocumentation*(`type`: GType) {.
    importc: "gst_element_type_set_skip_documentation", libprag.}
...
proc gst_element_decorate_stream_id(self: ptr Element00; streamId: cstring): cstring {.
    importc, libprag.}

proc decorateStreamId*(self: Element; streamId: cstring): string =
  let resul0 = gst_element_decorate_stream_id(cast[ptr Element00](self.impl), streamId)
  result = $resul0
  cogfree(resul0)
...
proc gst_element_request_pad_simple(self: ptr Element00; name: cstring): ptr Pad00 {.
    importc, libprag.}

proc requestPadSimple*(self: Element; name: cstring): Pad =
  let gobj = gst_element_request_pad_simple(cast[ptr Element00](self.impl), name)
  if gobj.isNil:
    return nil
  let qdata = g_object_get_qdata(gobj, Quark)
  if qdata != nil:
...
proc gst_event_type_to_sticky_ordering(`type`: EventType): uint32 {.
    importc, libprag.}

proc toStickyOrdering*(`type`: EventType): int =
  int(gst_event_type_to_sticky_ordering(`type`))
...
type
  StreamTypeFlag* {.size: sizeof(cint), pure.} = enum
    unknown = 0
    audio = 1
    video = 2
    container = 3
...
proc newDateTimeFromGDateTime*(dt: glib.DateTime = nil): DateTime =
  let impl0 = gst_date_time_new_from_g_date_time(if dt.isNil: nil else: cast[ptr glib.DateTime00](g_boxed_copy(g_date_time_get_type(), dt.impl)))
  if impl0.isNil:
    return nil
  fnew(result, gBoxedFreeGstDateTime)
  result.impl = impl0

proc newDateTimeFromGDateTime*(tdesc: typedesc; dt: glib.DateTime = nil): tdesc =
...
proc initDateTimeFromGDateTime*[T](result: var T; dt: glib.DateTime = nil) {.deprecated.} =
  assert(result is DateTime)
  let impl0 = gst_date_time_new_from_g_date_time(if dt.isNil: nil else: cast[ptr glib.DateTime00](g_boxed_copy(g_date_time_get_type(), dt.impl)))
  if impl0.isNil:
    return nil
...
proc newDateTimeNowLocalTime*(): DateTime =
  let impl0 = gst_date_time_new_now_local_time()
  if impl0.isNil:
    return nil
  fnew(result, gBoxedFreeGstDateTime)
  result.impl = impl0
...
proc newDateTimeNowUtc*(): DateTime =
  let impl0 = gst_date_time_new_now_utc()
  if impl0.isNil:
    return nil
  fnew(result, gBoxedFreeGstDateTime)
  result.impl = impl0
...
proc gst_structure_get_flags(self: ptr Structure00; fieldname: cstring; flagsType: GType;
    value: var uint32): gboolean {.
    importc, libprag.}

proc getFlags*(self: Structure; fieldname: cstring; flagsType: GType;
    value: var int): bool =
  var value_00: uint32
  result = toBool(gst_structure_get_flags(cast[ptr Structure00](self.impl), fieldname, flagsType, value_00))
  if value.addr != nil:
    value = int(value_00)
...
type
  SerializeFlag* {.size: sizeof(cint), pure.} = enum
    backwardCompat = 0
    strict = 1
...
proc none*(t: typedesc[SerializeFlags]): SerializeFlags = SerializeFlags({})

proc gst_structure_serialize(self: ptr Structure00; flags: SerializeFlags): cstring {.
    importc, libprag.}

proc serialize*(self: Structure; flags: SerializeFlags): string =
  let resul0 = gst_structure_serialize(cast[ptr Structure00](self.impl), flags)
  result = $resul0
  cogfree(resul0)

proc gst_structure_serialize_full(self: ptr Structure00; flags: SerializeFlags): cstring {.
    importc, libprag.}

proc serializeFull*(self: Structure; flags: SerializeFlags): string =
  let resul0 = gst_structure_serialize_full(cast[ptr Structure00](self.impl), flags)
  if resul0.isNil:
    return
  result = $resul0
  cogfree(resul0)
...
proc gst_buffer_new_memdup(data: ptr uint8; size: uint64): ptr Buffer00 {.
    importc, libprag.}

proc newBufferMemdup*(data: seq[uint8] | string): Buffer =
  let size = uint64(data.len)
  fnew(result, gBoxedFreeGstBuffer)
  result.impl = gst_buffer_new_memdup(cast[ptr uint8](unsafeaddr(data[0])), size)

proc newBufferMemdup*(tdesc: typedesc; data: seq[uint8] | string): tdesc =
  let size = uint64(data.len)
  assert(result is Buffer)
  fnew(result, gBoxedFreeGstBuffer)
  result.impl = gst_buffer_new_memdup(cast[ptr uint8](unsafeaddr(data[0])), size)

proc initBufferMemdup*[T](result: var T; data: seq[uint8] | string) {.deprecated.} =
  let size = uint64(data.len)
  assert(result is Buffer)
  fnew(result, gBoxedFreeGstBuffer)
  result.impl = gst_buffer_new_memdup(cast[ptr uint8](unsafeaddr(data[0])), size)
...
proc copyDeep*(self: Buffer): Buffer =
  let impl0 = gst_buffer_copy_deep(cast[ptr Buffer00](self.impl))
  if impl0.isNil:
    return nil
  fnew(result, gBoxedFreeGstBuffer)
  result.impl = impl0
...
proc gst_meta_info_is_custom(self: ptr MetaInfo00): gboolean {.
    importc, libprag.}

proc isCustom*(self: MetaInfo): bool =
  toBool(gst_meta_info_is_custom(cast[ptr MetaInfo00](self.impl)))

proc gst_meta_info_register(self: ptr MetaInfo00): ptr MetaInfo00 {.
    importc, libprag.}

proc registerMetaInfo*(self: MetaInfo): MetaInfo =
  new(result)
  result.impl = gst_meta_info_register(cast[ptr MetaInfo00](self.impl))
...
proc copyRegion*(self: Buffer; flags: BufferCopyFlags; offset: uint64;
    size: uint64): Buffer =
  let impl0 = gst_buffer_copy_region(cast[ptr Buffer00](self.impl), flags, offset, size)
  if impl0.isNil:
    return nil
  fnew(result, gBoxedFreeGstBuffer)
  result.impl = impl0
...
type
  CustomMeta00* {.pure.} = object
  CustomMeta* = ref object
    impl*: ptr CustomMeta00
    ignoreFinalizer*: bool

proc gst_custom_meta_get_structure(self: ptr CustomMeta00): ptr Structure00 {.
    importc, libprag.}

proc getStructure*(self: CustomMeta): Structure =
  fnew(result, gBoxedFreeGstStructure)
  result.impl = gst_custom_meta_get_structure(cast[ptr CustomMeta00](self.impl))
  result.impl = cast[typeof(result.impl)](g_boxed_copy(gst_structure_get_type(), result.impl))

proc structure*(self: CustomMeta): Structure =
  fnew(result, gBoxedFreeGstStructure)
  result.impl = gst_custom_meta_get_structure(cast[ptr CustomMeta00](self.impl))
  result.impl = cast[typeof(result.impl)](g_boxed_copy(gst_structure_get_type(), result.impl))

proc gst_custom_meta_has_name(self: ptr CustomMeta00; name: cstring): gboolean {.
    importc, libprag.}

proc hasName*(self: CustomMeta; name: cstring): bool =
  toBool(gst_custom_meta_has_name(cast[ptr CustomMeta00](self.impl), name))

proc gst_buffer_add_custom_meta(self: ptr Buffer00; name: cstring): ptr CustomMeta00 {.
    importc, libprag.}

proc addCustomMeta*(self: Buffer; name: cstring): CustomMeta =
  let impl0 = gst_buffer_add_custom_meta(cast[ptr Buffer00](self.impl), name)
  if impl0.isNil:
    return nil
  new(result)
  result.impl = impl0
...
proc gst_buffer_get_custom_meta(self: ptr Buffer00; name: cstring): ptr CustomMeta00 {.
    importc, libprag.}

proc getCustomMeta*(self: Buffer; name: cstring): CustomMeta =
  let impl0 = gst_buffer_get_custom_meta(cast[ptr Buffer00](self.impl), name)
  if impl0.isNil:
    return nil
  new(result)
  result.impl = impl0
...
proc gst_meta_serialize_simple(self: ptr Meta00; data: ptr ByteArray00): gboolean {.
    importc, libprag.}

proc serializeSimple*(self: Meta; data: ptr ByteArray00): bool =
  toBool(gst_meta_serialize_simple(cast[ptr Meta00](self.impl), data))
...
proc gst_meta_deserialize(buffer: ptr Buffer00; data: ptr uint8; size: uint64;
    consumed: var uint32): ptr Meta00 {.
    importc, libprag.}

proc deserialize*(buffer: Buffer; data: ptr uint8; size: uint64;
    consumed: var int): Meta =
  var consumed_00: uint32
  let impl0 = gst_meta_deserialize(cast[ptr Buffer00](buffer.impl), data, size, consumed_00)
  if impl0.isNil:
    return nil
  new(result)
  result.impl = impl0
  result.ignoreFinalizer = true
  if consumed.addr != nil:
    consumed = int(consumed_00)
...
proc gst_meta_register_custom_simple(name: cstring): ptr MetaInfo00 {.
    importc, libprag.}

proc registerCustomSimple*(name: cstring): MetaInfo =
  new(result)
  result.impl = gst_meta_register_custom_simple(name)
  result.ignoreFinalizer = true
...
type
  ByteArrayInterface00* {.pure.} = object
  ByteArrayInterface* = ref object
    impl*: ptr ByteArrayInterface00
    ignoreFinalizer*: bool

proc gst_meta_serialize(self: ptr Meta00; data: ptr ByteArrayInterface00): gboolean {.
    importc, libprag.}

proc serialize*(self: Meta; data: ByteArrayInterface): bool =
  toBool(gst_meta_serialize(cast[ptr Meta00](self.impl), cast[ptr ByteArrayInterface00](data.impl)))
...
proc gst_plugin_add_status_error(self: ptr Plugin00; message: cstring) {.
    importc, libprag.}

proc addStatusError*(self: Plugin; message: cstring) =
  gst_plugin_add_status_error(cast[ptr Plugin00](self.impl), message)

proc gst_plugin_add_status_info(self: ptr Plugin00; message: cstring) {.
    importc, libprag.}

proc addStatusInfo*(self: Plugin; message: cstring) =
  gst_plugin_add_status_info(cast[ptr Plugin00](self.impl), message)

proc gst_plugin_add_status_warning(self: ptr Plugin00; message: cstring) {.
    importc, libprag.}

proc addStatusWarning*(self: Plugin; message: cstring) =
  gst_plugin_add_status_warning(cast[ptr Plugin00](self.impl), message)
...
proc gst_plugin_get_status_errors(self: ptr Plugin00): ptr cstring {.
    importc, libprag.}

proc getStatusErrors*(self: Plugin): seq[string] =
  let resul0 = gst_plugin_get_status_errors(cast[ptr Plugin00](self.impl))
  if resul0.isNil:
    return
  result = cstringArrayToSeq(resul0)
  g_strfreev(resul0)

proc statusErrors*(self: Plugin): seq[string] =
  let resul0 = gst_plugin_get_status_errors(cast[ptr Plugin00](self.impl))
  if resul0.isNil:
    return
  result = cstringArrayToSeq(resul0)
...
proc gst_plugin_get_status_infos(self: ptr Plugin00): ptr cstring {.
    importc, libprag.}

proc getStatusInfos*(self: Plugin): seq[string] =
  let resul0 = gst_plugin_get_status_infos(cast[ptr Plugin00](self.impl))
  if resul0.isNil:
    return
  result = cstringArrayToSeq(resul0)
  g_strfreev(resul0)

proc statusInfos*(self: Plugin): seq[string] =
  let resul0 = gst_plugin_get_status_infos(cast[ptr Plugin00](self.impl))
  if resul0.isNil:
    return
  result = cstringArrayToSeq(resul0)
...
proc gst_plugin_get_status_warnings(self: ptr Plugin00): ptr cstring {.
    importc, libprag.}

proc getStatusWarnings*(self: Plugin): seq[string] =
  let resul0 = gst_plugin_get_status_warnings(cast[ptr Plugin00](self.impl))
  if resul0.isNil:
    return
  result = cstringArrayToSeq(resul0)
  g_strfreev(resul0)

proc statusWarnings*(self: Plugin): seq[string] =
  let resul0 = gst_plugin_get_status_warnings(cast[ptr Plugin00](self.impl))
  if resul0.isNil:
    return
  result = cstringArrayToSeq(resul0)
...
proc gst_element_factory_make_with_properties(factoryname: cstring; n: uint32;
    names: ptr cstring; values: ptr gobject.Value): ptr Element00 {.
    importc, libprag.}

proc makeWithProperties*(factoryname: cstring; n: int;
    names: openArray[string]; values: ptr gobject.Value): Element =
  var fs469n23x: array[256, pointer]
  var fs469n23: cstringArray = cast[cstringArray](addr fs469n23x)
  let gobj = gst_element_factory_make_with_properties(factoryname, uint32(n), seq2CstringArray(names, fs469n23), values)
  if gobj.isNil:
    return nil
  let qdata = g_object_get_qdata(gobj, Quark)
  if qdata != nil:
...
proc gst_element_factory_create_with_properties(self: ptr ElementFactory00;
    n: uint32; names: ptr cstring; values: ptr gobject.Value): ptr Element00 {.
    importc, libprag.}

proc createWithProperties*(self: ElementFactory; n: int;
    names: openArray[string]; values: ptr gobject.Value): Element =
  var fs469n23x: array[256, pointer]
  var fs469n23: cstringArray = cast[cstringArray](addr fs469n23x)
  let gobj = gst_element_factory_create_with_properties(cast[ptr ElementFactory00](self.impl), uint32(n), seq2CstringArray(names, fs469n23), values)
  if gobj.isNil:
    return nil
  let qdata = g_object_get_qdata(gobj, Quark)
  if qdata != nil:
...
proc gst_element_factory_get_skip_documentation(self: ptr ElementFactory00): gboolean {.
    importc, libprag.}

proc getSkipDocumentation*(self: ElementFactory): bool =
  toBool(gst_element_factory_get_skip_documentation(cast[ptr ElementFactory00](self.impl)))

proc skipDocumentation*(self: ElementFactory): bool =
  toBool(gst_element_factory_get_skip_documentation(cast[ptr ElementFactory00](self.impl)))
...
type
  RankFlag* {.size: sizeof(cint), pure.} = enum
    marginal = 6
    secondary = 7
    primary = 8
...
const
  RankNone* = Rank({})
proc none*(t: typedesc[Rank]): Rank = Rank({})
...
proc gst_caps_serialize(self: ptr Caps00; flags: SerializeFlags): cstring {.
    importc, libprag.}

proc serialize*(self: Caps; flags: SerializeFlags): string =
  let resul0 = gst_caps_serialize(cast[ptr Caps00](self.impl), flags)
  result = $resul0
  cogfree(resul0)
...
proc gst_caps_features_new_single(feature: cstring): ptr CapsFeatures00 {.
    importc, libprag.}

proc newCapsFeaturesSingle*(feature: cstring): CapsFeatures =
  fnew(result, gBoxedFreeGstCapsFeatures)
  result.impl = gst_caps_features_new_single(feature)

proc newCapsFeaturesSingle*(tdesc: typedesc; feature: cstring): tdesc =
  assert(result is CapsFeatures)
  fnew(result, gBoxedFreeGstCapsFeatures)
  result.impl = gst_caps_features_new_single(feature)

proc initCapsFeaturesSingle*[T](result: var T; feature: cstring) {.deprecated.} =
  assert(result is CapsFeatures)
  fnew(result, gBoxedFreeGstCapsFeatures)
  result.impl = gst_caps_features_new_single(feature)
...
proc newQueryAllocation*(caps: Caps = nil; needPool: bool): Query =
  fnew(result, gBoxedFreeGstQuery)
  result.impl = gst_query_new_allocation(if caps.isNil: nil else: cast[ptr Caps00](caps.impl), gboolean(needPool))

proc newQueryAllocation*(tdesc: typedesc; caps: Caps = nil; needPool: bool): tdesc =
  assert(result is Query)
  fnew(result, gBoxedFreeGstQuery)
  result.impl = gst_query_new_allocation(if caps.isNil: nil else: cast[ptr Caps00](caps.impl), gboolean(needPool))

proc initQueryAllocation*[T](result: var T; caps: Caps = nil; needPool: bool) {.deprecated.} =
  assert(result is Query)
  fnew(result, gBoxedFreeGstQuery)
  result.impl = gst_query_new_allocation(if caps.isNil: nil else: cast[ptr Caps00](caps.impl), gboolean(needPool))
...
proc gst_query_new_selectable(): ptr Query00 {.
    importc, libprag.}

proc newQuerySelectable*(): Query =
  fnew(result, gBoxedFreeGstQuery)
  result.impl = gst_query_new_selectable()

proc newQuerySelectable*(tdesc: typedesc): tdesc =
  assert(result is Query)
  fnew(result, gBoxedFreeGstQuery)
  result.impl = gst_query_new_selectable()

proc initQuerySelectable*[T](result: var T) {.deprecated.} =
  assert(result is Query)
  fnew(result, gBoxedFreeGstQuery)
  result.impl = gst_query_new_selectable()
...
proc parseAcceptCapsResult*(self: Query; resu: var bool) =
...
proc gst_query_parse_selectable(self: ptr Query00; selectable: var gboolean) {.
    importc, libprag.}

proc parseSelectable*(self: Query; selectable: var bool = cast[var bool](nil)) =
  var selectable_00: gboolean
  gst_query_parse_selectable(cast[ptr Query00](self.impl), selectable_00)
  if selectable.addr != nil:
    selectable = toBool(selectable_00)

proc parseSelectable*(self: Query): bool  =
  var result_00: gboolean
  gst_query_parse_selectable(cast[ptr Query00](self.impl), result_00)
  if result.addr != nil:
    result = toBool(result_00)
...
proc setCapsResult*(self: Query; caps: Caps = nil) =
  gst_query_set_caps_result(cast[ptr Query00](self.impl), if caps.isNil: nil else: cast[ptr Caps00](caps.impl))

proc `capsResult=`*(self: Query; caps: Caps = nil) =
  gst_query_set_caps_result(cast[ptr Query00](self.impl), if caps.isNil: nil else: cast[ptr Caps00](caps.impl))
...
proc gst_query_set_selectable(self: ptr Query00; selectable: gboolean) {.
    importc, libprag.}

proc setSelectable*(self: Query; selectable: bool = true) =
  gst_query_set_selectable(cast[ptr Query00](self.impl), gboolean(selectable))

proc `selectable=`*(self: Query; selectable: bool) =
  gst_query_set_selectable(cast[ptr Query00](self.impl), gboolean(selectable))

proc gst_query_set_uri(self: ptr Query00; uri: cstring) {.
    importc, libprag.}

proc setUri*(self: Query; uri: cstring = nil) =
  gst_query_set_uri(cast[ptr Query00](self.impl), uri)

proc `uri=`*(self: Query; uri: cstring = nil) =
  gst_query_set_uri(cast[ptr Query00](self.impl), uri)

proc gst_query_set_uri_redirection(self: ptr Query00; uri: cstring) {.
    importc, libprag.}

proc setUriRedirection*(self: Query; uri: cstring = nil) =
  gst_query_set_uri_redirection(cast[ptr Query00](self.impl), uri)

proc `uriRedirection=`*(self: Query; uri: cstring = nil) =
  gst_query_set_uri_redirection(cast[ptr Query00](self.impl), uri)
...
proc newQueryCustom*(`type`: QueryType; structure: Structure = nil): Query =
  fnew(result, gBoxedFreeGstQuery)
  result.impl = gst_query_new_custom(`type`, if structure.isNil: nil else: cast[ptr Structure00](g_boxed_copy(gst_structure_get_type(), structure.impl)))
...
proc setDocumentationCaps*(self: PadTemplate; caps: Caps) =
  gst_pad_template_set_documentation_caps(cast[ptr PadTemplate00](self.impl), cast[ptr Caps00](caps.impl))
...
proc newEventCaps*(caps: Caps): Event =
  fnew(result, gBoxedFreeGstEvent)
  result.impl = gst_event_new_caps(cast[ptr Caps00](caps.impl))
...
proc newEventCustom*(`type`: EventType; structure: Structure): Event =
  fnew(result, gBoxedFreeGstEvent)
  result.impl = gst_event_new_custom(`type`, cast[ptr Structure00](g_boxed_copy(gst_structure_get_type(), structure.impl)))
...
proc newEventSeek*(rate: cdouble; format: Format; flags: SeekFlags;
  ...
  fnew(result, gBoxedFreeGstEvent)
  result.impl = gst_event_new_seek(rate, format, flags, startType, start, stopType, stop)
...
proc newEventSegment*(segment: Segment): Event =
  fnew(result, gBoxedFreeGstEvent)
  result.impl = gst_event_new_segment(cast[ptr Segment00](segment.impl))
...
proc newEventStep*(format: Format; amount: uint64; rate: cdouble; flush: bool;
  ...
  fnew(result, gBoxedFreeGstEvent)
  result.impl = gst_event_new_step(format, amount, rate, gboolean(flush), gboolean(intermediate))
...
proc parseBufferSize*(self: Event; format: var Format = cast[var Format](nil);
    minsize: var int64 = cast[var int64](nil); maxsize: var int64 = cast[var int64](nil);
    async: var bool = cast[var bool](nil)) =
...
proc parseCaps*(self: Event; caps: var Caps = cast[var Caps](nil)) =
  if addr(caps) != nil:
    fnew(caps, gBoxedFreeGstCaps)
  gst_event_parse_caps(cast[ptr Event00](self.impl), cast[var ptr Caps00](if addr(caps) == nil: nil else: addr caps.impl))
  caps.impl = cast[ptr Caps00](g_boxed_copy(gst_caps_get_type(), caps.impl))

proc parseCaps*(self: Event): Caps  =
  if addr(result) != nil:
    fnew(result, gBoxedFreeGstCaps)
  gst_event_parse_caps(cast[ptr Event00](self.impl), cast[var ptr Caps00](if addr(result) == nil: nil else: addr result.impl))
  result.impl = cast[ptr Caps00](g_boxed_copy(gst_caps_get_type(), result.impl))

proc gst_event_parse_flush_stop(self: ptr Event00; resetTime: var gboolean) {.
    importc, libprag.}

proc parseFlushStop*(self: Event; resetTime: var bool = cast[var bool](nil)) =
...
proc parseGroupId*(self: Event; groupId: var int = cast[var int](nil)): bool =
...
proc parseLatency*(self: Event; latency: var uint64 = cast[var uint64](nil)) =
...
proc parseSeek*(self: Event; rate: var cdouble = cast[var cdouble](nil);
    format: var Format = cast[var Format](nil); flags: var SeekFlags = cast[var SeekFlags](nil);
    startType: var SeekType = cast[var SeekType](nil); start: var int64 = cast[var int64](nil);
    stopType: var SeekType = cast[var SeekType](nil); stop: var int64 = cast[var int64](nil)) =
...
proc parseSeekTrickmodeInterval*(self: Event; interval: var uint64 = cast[var uint64](nil)) =
...
proc parseSegment*(self: Event; segment: var Segment = cast[var Segment](nil)) =
  if addr(segment) != nil:
    fnew(segment, gBoxedFreeGstSegment)
  gst_event_parse_segment(cast[ptr Event00](self.impl), cast[var ptr Segment00](if addr(segment) == nil: nil else: addr segment.impl))
  segment.impl = cast[ptr Segment00](g_boxed_copy(gst_segment_get_type(), segment.impl))

proc parseSegment*(self: Event): Segment  =
  if addr(result) != nil:
    fnew(result, gBoxedFreeGstSegment)
  gst_event_parse_segment(cast[ptr Event00](self.impl), cast[var ptr Segment00](if addr(result) == nil: nil else: addr result.impl))
...
proc parseSelectStreams*(self: Event; streams: var seq[cstring] = cast[var seq[cstring]](nil)) =
  var tempResGL = seq2GList(streams)
...
proc parseStream*(self: Event; stream: var Stream = cast[var Stream](nil)) =
  var tmpoutgobjectarg: ptr Stream00
  gst_event_parse_stream(cast[ptr Event00](self.impl), cast[var ptr Stream00](if addr(stream) == nil: nil else: addr tmpoutgobjectarg))
#  dothemagic(stream
  if addr(stream) != nil:
    stream = nil
...
proc parseStream*(self: Event): Stream  =
  var tmpoutgobjectarg: ptr Stream00
  gst_event_parse_stream(cast[ptr Event00](self.impl), cast[var ptr Stream00](if addr(result) == nil: nil else: addr tmpoutgobjectarg))
#  dothemagic(result
  if addr(result) != nil:
    result = nil
...
proc parseStreamFlags*(self: Event; flags: var StreamFlags = cast[var StreamFlags](nil)) =
...
proc parseStreamGroupDone*(self: Event; groupId: var int = cast[var int](nil)) =
  var groupId_00: uint32
...
proc parseStreamStart*(self: Event; streamId: var string = cast[var string](nil)) =
...
proc parseTag*(self: Event; taglist: var TagList = cast[var TagList](nil)) =
  if addr(taglist) != nil:
    fnew(taglist, gBoxedFreeGstTagList)
  gst_event_parse_tag(cast[ptr Event00](self.impl), cast[var ptr TagList00](if addr(taglist) == nil: nil else: addr taglist.impl))
  taglist.impl = cast[ptr TagList00](g_boxed_copy(gst_tag_list_get_type(), taglist.impl))

proc parseTag*(self: Event): TagList  =
  if addr(result) != nil:
    fnew(result, gBoxedFreeGstTagList)
  gst_event_parse_tag(cast[ptr Event00](self.impl), cast[var ptr TagList00](if addr(result) == nil: nil else: addr result.impl))
...
proc newEventQos*(`type`: QOSType; proportion: cdouble; diff: int64;
  ...
  fnew(result, gBoxedFreeGstEvent)
  result.impl = gst_event_new_qos(`type`, proportion, diff, timestamp)
...
proc parseQos*(self: Event; `type`: var QOSType = cast[var QOSType](nil);
    proportion: var cdouble = cast[var cdouble](nil); diff: var int64 = cast[var int64](nil);
    timestamp: var uint64 = cast[var uint64](nil)) =
...
proc scStreamNotify*(self: StreamCollection;  p: proc (self: ptr StreamCollection00; propStream: ptr Stream00; prop: ptr gobject.ParamSpec00; xdata: pointer) {.cdecl.}, xdata: pointer, cf: gobject.ConnectFlags): culong =
...
proc parseStreamCollection*(self: Event; collection: var StreamCollection = cast[var StreamCollection](nil)) =
  var tmpoutgobjectarg: ptr StreamCollection00
  gst_event_parse_stream_collection(cast[ptr Event00](self.impl), cast[var ptr StreamCollection00](if addr(collection) == nil: nil else: addr tmpoutgobjectarg))
...
proc parseStreamCollection*(self: Event): StreamCollection  =
  var tmpoutgobjectarg: ptr StreamCollection00
  gst_event_parse_stream_collection(cast[ptr Event00](self.impl), cast[var ptr StreamCollection00](if addr(result) == nil: nil else: addr tmpoutgobjectarg))
#  dothemagic(result
  if addr(result) != nil:
    result = nil
...
proc getTags*(self: Toc): TagList =
  let impl0 = gst_toc_get_tags(cast[ptr Toc00](self.impl))
  if impl0.isNil:
    return nil
  fnew(result, gBoxedFreeGstTagList)
...
proc parseToc*(self: Event; toc: var Toc = cast[var Toc](nil);
    updated: var bool = cast[var bool](nil)) =
  if addr(toc) != nil:
    fnew(toc, gBoxedFreeGstToc)
  var updated_00: gboolean
  gst_event_parse_toc(cast[ptr Event00](self.impl), cast[var ptr Toc00](if addr(toc) == nil: nil else: addr toc.impl), updated_00)
  if updated.addr != nil:
...
proc getTags*(self: TocEntry): TagList =
  let impl0 = gst_toc_entry_get_tags(cast[ptr TocEntry00](self.impl))
  if impl0.isNil:
    return nil
  fnew(result, gBoxedFreeGstTagList)
  result.impl = cast[typeof(result.impl)](g_boxed_copy(gst_tag_list_get_type(), impl0))
...
proc getToc*(self: TocEntry): Toc =
  let impl0 = gst_toc_entry_get_toc(cast[ptr TocEntry00](self.impl))
  if impl0.isNil:
    return nil
  fnew(result, gBoxedFreeGstToc)
  result.impl = cast[typeof(result.impl)](g_boxed_copy(gst_toc_get_type(), impl0))
...
type
  GapFlags* {.size: sizeof(cint), pure.} = enum
    data = 1

proc gst_event_parse_gap_flags(self: ptr Event00; flags: var GapFlags) {.
    importc, libprag.}

proc parseGapFlags*(self: Event; flags: var GapFlags = cast[var GapFlags](nil)) =
  gst_event_parse_gap_flags(cast[ptr Event00](self.impl), flags)

proc parseGapFlags*(self: Event): GapFlags  =
  gst_event_parse_gap_flags(cast[ptr Event00](self.impl), result)

proc gst_event_set_gap_flags(self: ptr Event00; flags: GapFlags) {.
    importc, libprag.}

proc setGapFlags*(self: Event; flags: GapFlags) =
  gst_event_set_gap_flags(cast[ptr Event00](self.impl), flags)

proc `gapFlags=`*(self: Event; flags: GapFlags) =
  gst_event_set_gap_flags(cast[ptr Event00](self.impl), flags)
...
proc setContext*(self: Query; context: Context = nil) =
  gst_query_set_context(cast[ptr Query00](self.impl), if context.isNil: nil else: cast[ptr Context00](context.impl))

proc `context=`*(self: Query; context: Context = nil) =
  gst_query_set_context(cast[ptr Query00](self.impl), if context.isNil: nil else: cast[ptr Context00](context.impl))
...
proc gst_iterator_new_single(`type`: GType; obj: gobject.Value): ptr Iterator00 {.
    importc, libprag.}

proc newIteratorSingle*(`type`: GType; obj: gobject.Value): Iterator =
  fnew(result, gBoxedFreeGstIterator)
  result.impl = gst_iterator_new_single(`type`, obj)

proc newIteratorSingle*(tdesc: typedesc; `type`: GType; obj: gobject.Value): tdesc =
  assert(result is Iterator)
  fnew(result, gBoxedFreeGstIterator)
  result.impl = gst_iterator_new_single(`type`, obj)

proc initIteratorSingle*[T](result: var T; `type`: GType; obj: gobject.Value) {.deprecated.} =
  assert(result is Iterator)
  fnew(result, gBoxedFreeGstIterator)
  result.impl = gst_iterator_new_single(`type`, obj)
...
proc gst_pipeline_get_configured_latency(self: ptr Pipeline00): uint64 {.
    importc, libprag.}

proc getConfiguredLatency*(self: Pipeline): uint64 =
  gst_pipeline_get_configured_latency(cast[ptr Pipeline00](self.impl))

proc configuredLatency*(self: Pipeline): uint64 =
  gst_pipeline_get_configured_latency(cast[ptr Pipeline00](self.impl))
...
proc gst_pipeline_is_live(self: ptr Pipeline00): gboolean {.
    importc, libprag.}

proc isLive*(self: Pipeline): bool =
  toBool(gst_pipeline_is_live(cast[ptr Pipeline00](self.impl)))
...
proc scChildAdded*(self: ChildProxy | Pipeline | Bin;  p: proc (self: ptr ChildProxy00; obj: ptr gobject.Object00; name: cstring; xdata: pointer) {.cdecl.}, xdata: pointer, cf: gobject.ConnectFlags): culong =
  g_signal_connect_data(self.impl, "child-added", cast[GCallback](p), xdata, nil, cf)

proc scChildRemoved*(self: ChildProxy | Pipeline | Bin;  p: proc (self: ptr ChildProxy00; obj: ptr gobject.Object00; name: cstring; xdata: pointer) {.cdecl.}, xdata: pointer, cf: gobject.ConnectFlags): culong =
...
proc gst_child_proxy_get_child_by_name_recurse(self: ptr ChildProxy00; name: cstring): ptr gobject.Object00 {.
    importc, libprag.}

proc getChildByNameRecurse*(self: ChildProxy | Pipeline | Bin;
    name: cstring): gobject.Object =
  let gobj = gst_child_proxy_get_child_by_name_recurse(cast[ptr ChildProxy00](self.impl), name)
  if gobj.isNil:
    return nil
  let qdata = g_object_get_qdata(gobj, Quark)
  if qdata != nil:
...
type
  CustomMetaTransformFunction* = proc (transbuf: ptr Buffer00; meta: ptr CustomMeta00; buffer: ptr Buffer00;
    `type`: uint32; data: pointer; userData: pointer): gboolean {.cdecl.}

proc gst_meta_register_custom(name: cstring; tags: ptr cstring; transformFunc: CustomMetaTransformFunction;
    userData: pointer; destroyData: DestroyNotify): ptr MetaInfo00 {.
    importc, libprag.}

proc registerCustom*(name: cstring; tags: openArray[string]; transformFunc: CustomMetaTransformFunction;
    userData: pointer; destroyData: DestroyNotify): MetaInfo =
  var fs469n23x: array[256, pointer]
  var fs469n23: cstringArray = cast[cstringArray](addr fs469n23x)
  new(result)
  result.impl = gst_meta_register_custom(name, seq2CstringArray(tags, fs469n23), transformFunc, userData, destroyData)
  result.ignoreFinalizer = true
...
proc gst_debug_message_get_id(self: ptr DebugMessage00): cstring {.
    importc, libprag.}

proc getId*(self: DebugMessage): string =
  let resul0 = gst_debug_message_get_id(cast[ptr DebugMessage00](self.impl))
  if resul0.isNil:
    return
  result = $resul0

proc id*(self: DebugMessage): string =
  let resul0 = gst_debug_message_get_id(cast[ptr DebugMessage00](self.impl))
  if resul0.isNil:
    return
  result = $resul0
...
proc scProviderHidden*(self: DeviceProvider;  p: proc (self: ptr DeviceProvider00; obj: cstring; xdata: pointer) {.cdecl.}, xdata: pointer, cf: gobject.ConnectFlags): culong =
  g_signal_connect_data(self.impl, "provider-hidden", cast[GCallback](p), xdata, nil, cf)

proc scProviderUnhidden*(self: DeviceProvider;  p: proc (self: ptr DeviceProvider00; obj: cstring; xdata: pointer) {.cdecl.}, xdata: pointer, cf: gobject.ConnectFlags): culong =
...
proc gst_device_provider_is_started(self: ptr DeviceProvider00): gboolean {.
    importc, libprag.}

proc isStarted*(self: DeviceProvider): bool =
  toBool(gst_device_provider_is_started(cast[ptr DeviceProvider00](self.impl)))
...
const ELEMENT_FACTORY_TYPE_TIMESTAMPER* = 8192'u64
...
type
  LogFunction* = proc (category: ptr DebugCategory00; level: DebugLevel; file: cstring; function: cstring;
    line: int32; obj: ptr gobject.Object00; message: ptr DebugMessage00; userData: pointer) {.cdecl.}
...
type
  MetaClearFunction* = proc (buffer: ptr Buffer00; meta: ptr Meta00) {.cdecl.}

type
  MetaDeserializeFunction* = proc (info: ptr MetaInfo00; buffer: ptr Buffer00; data: ptr uint8; size: uint64;
    version: uint8): ptr Meta00 {.cdecl.}
...
type
  MetaSerializeFunction* = proc (meta: ptr Meta00; data: ptr ByteArrayInterface00; version: var uint8): gboolean {.cdecl.}
...
type
  ...
  MiniObjectCopyFunction* = proc (obj: ptr MiniObject00): ptr MiniObject00 {.cdecl.}
...
proc gst_mini_object_get_type*(): GType {.importc, libprag.}

proc gBoxedFreeGstMiniObject*(self: MiniObject) =
  if not self.ignoreFinalizer:
    boxedFree(gst_mini_object_get_type(), cast[ptr MiniObject00](self.impl))

when defined(gcDestructors):
  proc `=destroy`*(self: var typeof(MiniObject()[])) =
    when defined(gintroDebug):
      echo "destroy ", $typeof(self), ' ', cast[int](unsafeaddr self)
    if not self.ignoreFinalizer and self.impl != nil:
      boxedFree(gst_mini_object_get_type(), cast[ptr MiniObject00](self.impl))
...
proc newWithFinalizer*(x: var MiniObject) =
  when defined(gcDestructors):
    new(x)
  else:
    new(x, gBoxedFreeGstMiniObject)
...
proc newMessageApplication*(src: Object = nil; structure: Structure): Message =
  ...
  result.impl = gst_message_new_application(if src.isNil: nil else: cast[ptr Object00](src.impl), cast[ptr Structure00](g_boxed_copy(gst_structure_get_type(), structure.impl)))
...
proc newMessageBuffering*(src: Object = nil; percent: int): Message =
  fnew(result, gBoxedFreeGstMessage)
  result.impl = gst_message_new_buffering(if src.isNil: nil else: cast[ptr Object00](src.impl), int32(percent))
...
proc newMessageCustom*(`type`: MessageType; src: Object = nil; structure: Structure = nil): Message =
  fnew(result, gBoxedFreeGstMessage)
  result.impl = gst_message_new_custom(`type`, if src.isNil: nil else: cast[ptr Object00](src.impl), if structure.isNil: nil else: cast[ptr Structure00](g_boxed_copy(gst_structure_get_type(), structure.impl)))
...
proc newMessageDeviceAdded*(src: Object = nil; device: Device): Message =
  fnew(result, gBoxedFreeGstMessage)
  result.impl = gst_message_new_device_added(if src.isNil: nil else: cast[ptr Object00](src.impl), cast[ptr Device00](device.impl))

proc newMessageDeviceAdded*(tdesc: typedesc; src: Object = nil; device: Device): tdesc =
  assert(result is Message)
  fnew(result, gBoxedFreeGstMessage)
  result.impl = gst_message_new_device_added(if src.isNil: nil else: cast[ptr Object00](src.impl), cast[ptr Device00](device.impl))

proc initMessageDeviceAdded*[T](result: var T; src: Object = nil; device: Device) {.deprecated.} =
  assert(result is Message)
  fnew(result, gBoxedFreeGstMessage)
  result.impl = gst_message_new_device_added(if src.isNil: nil else: cast[ptr Object00](src.impl), cast[ptr Device00](device.impl))
...
proc newMessageDeviceChanged*(src: Object = nil; device: Device; changedDevice: Device): Message =
  fnew(result, gBoxedFreeGstMessage)
  result.impl = gst_message_new_device_changed(if src.isNil: nil else: cast[ptr Object00](src.impl), cast[ptr Device00](device.impl), cast[ptr Device00](changedDevice.impl))

proc newMessageDeviceChanged*(tdesc: typedesc; src: Object = nil; device: Device; changedDevice: Device): tdesc =
  assert(result is Message)
  fnew(result, gBoxedFreeGstMessage)
  result.impl = gst_message_new_device_changed(if src.isNil: nil else: cast[ptr Object00](src.impl), cast[ptr Device00](device.impl), cast[ptr Device00](changedDevice.impl))

proc initMessageDeviceChanged*[T](result: var T; src: Object = nil; device: Device; changedDevice: Device) {.deprecated.} =
  assert(result is Message)
  fnew(result, gBoxedFreeGstMessage)
  result.impl = gst_message_new_device_changed(if src.isNil: nil else: cast[ptr Object00](src.impl), cast[ptr Device00](device.impl), cast[ptr Device00](changedDevice.impl))
...
proc newMessageDeviceRemoved*(src: Object = nil; device: Device): Message =
  fnew(result, gBoxedFreeGstMessage)
  result.impl = gst_message_new_device_removed(if src.isNil: nil else: cast[ptr Object00](src.impl), cast[ptr Device00](device.impl))

proc newMessageDeviceRemoved*(tdesc: typedesc; src: Object = nil; device: Device): tdesc =
  assert(result is Message)
  fnew(result, gBoxedFreeGstMessage)
  result.impl = gst_message_new_device_removed(if src.isNil: nil else: cast[ptr Object00](src.impl), cast[ptr Device00](device.impl))

proc initMessageDeviceRemoved*[T](result: var T; src: Object = nil; device: Device) {.deprecated.} =
  assert(result is Message)
  fnew(result, gBoxedFreeGstMessage)
  result.impl = gst_message_new_device_removed(if src.isNil: nil else: cast[ptr Object00](src.impl), cast[ptr Device00](device.impl))
...
proc newMessageElement*(src: Object = nil; structure: Structure): Message =
  fnew(result, gBoxedFreeGstMessage)
  result.impl = gst_message_new_element(if src.isNil: nil else: cast[ptr Object00](src.impl), cast[ptr Structure00](g_boxed_copy(gst_structure_get_type(), structure.impl)))
...
proc newMessageErrorWithDetails*(src: Object = nil; error: ptr glib.Error;
  ...
  fnew(result, gBoxedFreeGstMessage)
  result.impl = gst_message_new_error_with_details(if src.isNil: nil else: cast[ptr Object00](src.impl), error, debug, if details.isNil: nil else: cast[ptr Structure00](g_boxed_copy(gst_structure_get_type(), details.impl)))
...
proc newMessageInfoWithDetails*(src: Object = nil; error: ptr glib.Error;
    debug: cstring; details: Structure = nil): Message =
  fnew(result, gBoxedFreeGstMessage)
  result.impl = gst_message_new_info_with_details(if src.isNil: nil else: cast[ptr Object00](src.impl), error, debug, if details.isNil: nil else: cast[ptr Structure00](g_boxed_copy(gst_structure_get_type(), details.impl)))
...
proc newMessageInstantRateRequest*(src: Object = nil; rateMultiplier: cdouble): Message =
  fnew(result, gBoxedFreeGstMessage)
  result.impl = gst_message_new_instant_rate_request(if src.isNil: nil else: cast[ptr Object00](src.impl), rateMultiplier)

proc newMessageInstantRateRequest*(tdesc: typedesc; src: Object = nil; rateMultiplier: cdouble): tdesc =
  assert(result is Message)
  fnew(result, gBoxedFreeGstMessage)
  result.impl = gst_message_new_instant_rate_request(if src.isNil: nil else: cast[ptr Object00](src.impl), rateMultiplier)

proc initMessageInstantRateRequest*[T](result: var T; src: Object = nil; rateMultiplier: cdouble) {.deprecated.} =
  assert(result is Message)
  fnew(result, gBoxedFreeGstMessage)
  result.impl = gst_message_new_instant_rate_request(if src.isNil: nil else: cast[ptr Object00](src.impl), rateMultiplier)
...
proc newMessageQos*(src: Object = nil; live: bool; runningTime: uint64;
    streamTime: uint64; timestamp: uint64; duration: uint64): Message =
  fnew(result, gBoxedFreeGstMessage)
  result.impl = gst_message_new_qos(if src.isNil: nil else: cast[ptr Object00](src.impl), gboolean(live), runningTime, streamTime, timestamp, duration)

proc newMessageQos*(tdesc: typedesc; src: Object = nil; live: bool; runningTime: uint64;
    streamTime: uint64; timestamp: uint64; duration: uint64): tdesc =
  assert(result is Message)
  fnew(result, gBoxedFreeGstMessage)
  result.impl = gst_message_new_qos(if src.isNil: nil else: cast[ptr Object00](src.impl), gboolean(live), runningTime, streamTime, timestamp, duration)

proc initMessageQos*[T](result: var T; src: Object = nil; live: bool; runningTime: uint64;
    streamTime: uint64; timestamp: uint64; duration: uint64) {.deprecated.} =
  assert(result is Message)
  fnew(result, gBoxedFreeGstMessage)
  result.impl = gst_message_new_qos(if src.isNil: nil else: cast[ptr Object00](src.impl), gboolean(live), runningTime, streamTime, timestamp, duration)
...
proc newMessageRedirect*(src: Object = nil; location: cstring; tagList: TagList = nil;
    entryStruct: Structure = nil): Message =
  fnew(result, gBoxedFreeGstMessage)
  result.impl = gst_message_new_redirect(if src.isNil: nil else: cast[ptr Object00](src.impl), location, if tagList.isNil: nil else: cast[ptr TagList00](g_boxed_copy(gst_tag_list_get_type(), tagList.impl)), if entryStruct.isNil: nil else: cast[ptr Structure00](g_boxed_copy(gst_structure_get_type(), entryStruct.impl)))

proc newMessageRedirect*(tdesc: typedesc; src: Object = nil; location: cstring; tagList: TagList = nil;
    entryStruct: Structure = nil): tdesc =
  assert(result is Message)
  fnew(result, gBoxedFreeGstMessage)
  result.impl = gst_message_new_redirect(if src.isNil: nil else: cast[ptr Object00](src.impl), location, if tagList.isNil: nil else: cast[ptr TagList00](g_boxed_copy(gst_tag_list_get_type(), tagList.impl)), if entryStruct.isNil: nil else: cast[ptr Structure00](g_boxed_copy(gst_structure_get_type(), entryStruct.impl)))

proc initMessageRedirect*[T](result: var T; src: Object = nil; location: cstring; tagList: TagList = nil;
...
proc newMessageStepDone*(src: Object = nil; format: Format; amount: uint64;
    rate: cdouble; flush: bool; intermediate: bool; duration: uint64; eos: bool): Message =
  fnew(result, gBoxedFreeGstMessage)
  result.impl = gst_message_new_step_done(if src.isNil: nil else: cast[ptr Object00](src.impl), format, amount, rate, gboolean(flush), gboolean(intermediate), duration, gboolean(eos))

proc newMessageStepDone*(tdesc: typedesc; src: Object = nil; format: Format; amount: uint64;
    rate: cdouble; flush: bool; intermediate: bool; duration: uint64; eos: bool): tdesc =
  assert(result is Message)
  fnew(result, gBoxedFreeGstMessage)
  result.impl = gst_message_new_step_done(if src.isNil: nil else: cast[ptr Object00](src.impl), format, amount, rate, gboolean(flush), gboolean(intermediate), duration, gboolean(eos))

proc initMessageStepDone*[T](result: var T; src: Object = nil; format: Format; amount: uint64;
...
proc newMessageStepStart*(src: Object = nil; active: bool; format: Format;
    amount: uint64; rate: cdouble; flush: bool; intermediate: bool): Message =
  fnew(result, gBoxedFreeGstMessage)
  result.impl = gst_message_new_step_start(if src.isNil: nil else: cast[ptr Object00](src.impl), gboolean(active), format, amount, rate, gboolean(flush), gboolean(intermediate))

proc newMessageStepStart*(tdesc: typedesc; src: Object = nil; active: bool; format: Format;
    amount: uint64; rate: cdouble; flush: bool; intermediate: bool): tdesc =
  assert(result is Message)
  fnew(result, gBoxedFreeGstMessage)
  result.impl = gst_message_new_step_start(if src.isNil: nil else: cast[ptr Object00](src.impl), gboolean(active), format, amount, rate, gboolean(flush), gboolean(intermediate))

proc initMessageStepStart*[T](result: var T; src: Object = nil; active: bool; format: Format;
...
proc newMessageStreamCollection*(src: Object = nil; collection: StreamCollection): Message =
  fnew(result, gBoxedFreeGstMessage)
  result.impl = gst_message_new_stream_collection(if src.isNil: nil else: cast[ptr Object00](src.impl), cast[ptr StreamCollection00](collection.impl))

proc newMessageStreamCollection*(tdesc: typedesc; src: Object = nil; collection: StreamCollection): tdesc =
  assert(result is Message)
  fnew(result, gBoxedFreeGstMessage)
  result.impl = gst_message_new_stream_collection(if src.isNil: nil else: cast[ptr Object00](src.impl), cast[ptr StreamCollection00](collection.impl))

proc initMessageStreamCollection*[T](result: var T; src: Object = nil; collection: StreamCollection) {.deprecated.} =
  assert(result is Message)
  fnew(result, gBoxedFreeGstMessage)
  result.impl = gst_message_new_stream_collection(if src.isNil: nil else: cast[ptr Object00](src.impl), cast[ptr StreamCollection00](collection.impl))
...
proc newMessageStreamsSelected*(src: Object = nil; collection: StreamCollection): Message =
  fnew(result, gBoxedFreeGstMessage)
  result.impl = gst_message_new_streams_selected(if src.isNil: nil else: cast[ptr Object00](src.impl), cast[ptr StreamCollection00](collection.impl))

proc newMessageStreamsSelected*(tdesc: typedesc; src: Object = nil; collection: StreamCollection): tdesc =
  assert(result is Message)
  fnew(result, gBoxedFreeGstMessage)
  result.impl = gst_message_new_streams_selected(if src.isNil: nil else: cast[ptr Object00](src.impl), cast[ptr StreamCollection00](collection.impl))

proc initMessageStreamsSelected*[T](result: var T; src: Object = nil; collection: StreamCollection) {.deprecated.} =
  assert(result is Message)
  fnew(result, gBoxedFreeGstMessage)
  result.impl = gst_message_new_streams_selected(if src.isNil: nil else: cast[ptr Object00](src.impl), cast[ptr StreamCollection00](collection.impl))
...
proc newMessageToc*(src: Object = nil; toc: Toc; updated: bool): Message =
  fnew(result, gBoxedFreeGstMessage)
  result.impl = gst_message_new_toc(if src.isNil: nil else: cast[ptr Object00](src.impl), cast[ptr Toc00](toc.impl), gboolean(updated))

proc newMessageToc*(tdesc: typedesc; src: Object = nil; toc: Toc; updated: bool): tdesc =
  assert(result is Message)
  fnew(result, gBoxedFreeGstMessage)
  result.impl = gst_message_new_toc(if src.isNil: nil else: cast[ptr Object00](src.impl), cast[ptr Toc00](toc.impl), gboolean(updated))

proc initMessageToc*[T](result: var T; src: Object = nil; toc: Toc; updated: bool) {.deprecated.} =
  assert(result is Message)
  fnew(result, gBoxedFreeGstMessage)
  result.impl = gst_message_new_toc(if src.isNil: nil else: cast[ptr Object00](src.impl), cast[ptr Toc00](toc.impl), gboolean(updated))
...
proc newMessageWarningWithDetails*(src: Object = nil; error: ptr glib.Error;
    debug: cstring; details: Structure = nil): Message =
  fnew(result, gBoxedFreeGstMessage)
  result.impl = gst_message_new_warning_with_details(if src.isNil: nil else: cast[ptr Object00](src.impl), error, debug, if details.isNil: nil else: cast[ptr Structure00](g_boxed_copy(gst_structure_get_type(), details.impl)))
...
proc parseErrorDetails*(self: Message; structure: var Structure = cast[var Structure](nil)) =
  if addr(structure) != nil:
    fnew(structure, gBoxedFreeGstStructure)
  gst_message_parse_error_details(cast[ptr Message00](self.impl), cast[var ptr Structure00](if addr(structure) == nil: nil else: addr structure.impl))
...
proc parseInfoDetails*(self: Message; structure: var Structure = cast[var Structure](nil)) =
  if addr(structure) != nil:
    fnew(structure, gBoxedFreeGstStructure)
  gst_message_parse_info_details(cast[ptr Message00](self.impl), cast[var ptr Structure00](if addr(structure) == nil: nil else: addr structure.impl))
  structure.impl = cast[ptr Structure00](g_boxed_copy(gst_structure_get_type(), structure.impl))
  if structure != nil and structure.impl == nil:
    structure.ignoreFinalizer = true
    structure = nil
...
proc gst_message_parse_property_notify(self: ptr Message00; obj: var ptr Object00;
...
#  dothemagic(obj
  ...
  if tmpoutgobjectarg != nil:
    ...
    if argqdata != nil:
      ...
      fnew(obj, gst.finalizeGObject)
      obj.impl = tmpoutgobjectarg
      GC_ref(obj)
      if g_object_is_floating(obj.impl).int != 0:
        discard g_object_ref_sink(obj.impl)
      g_object_add_toggle_ref(obj.impl, toggleNotify, addr(obj[]))
      g_object_unref(obj.impl)
      assert(g_object_get_qdata(obj.impl, Quark) == nil)
      g_object_set_qdata(obj.impl, Quark, addr(obj[]))
...
proc parseRedirectEntry*(self: Message; entryIndex: uint64;
  ...
  if tagList != nil and tagList.impl == nil:
    tagList.ignoreFinalizer = true
    tagList = nil
  if entryStruct != nil and entryStruct.impl == nil:
    entryStruct.ignoreFinalizer = true
    entryStruct = nil
...
proc parseWarningDetails*(self: Message; structure: var Structure = cast[var Structure](nil)) =
  if addr(structure) != nil:
    fnew(structure, gBoxedFreeGstStructure)
  gst_message_parse_warning_details(cast[ptr Message00](self.impl), cast[var ptr Structure00](if addr(structure) == nil: nil else: addr structure.impl))
  structure.impl = cast[ptr Structure00](g_boxed_copy(gst_structure_get_type(), structure.impl))
  if structure != nil and structure.impl == nil:
    structure.ignoreFinalizer = true
    structure = nil
...
proc gst_message_set_stream_status_object(self: ptr Message00; obj: gobject.Value) {.
    importc, libprag.}

proc setStreamStatusObject*(self: Message; obj: gobject.Value) =
  gst_message_set_stream_status_object(cast[ptr Message00](self.impl), obj)

proc `streamStatusObject=`*(self: Message; obj: gobject.Value) =
  gst_message_set_stream_status_object(cast[ptr Message00](self.impl), obj)
...
proc parseSinkMessage*(self: Event; msg: var Message = cast[var Message](nil)) =
  if addr(msg) != nil:
    fnew(msg, gBoxedFreeGstMessage)
  gst_event_parse_sink_message(cast[ptr Event00](self.impl), cast[var ptr Message00](if addr(msg) == nil: nil else: addr msg.impl))

proc parseSinkMessage*(self: Event): Message  =
  if addr(result) != nil:
    fnew(result, gBoxedFreeGstMessage)
  gst_event_parse_sink_message(cast[ptr Event00](self.impl), cast[var ptr Message00](if addr(result) == nil: nil else: addr result.impl))
...
proc newMessageProgress*(src: Object = nil; `type`: ProgressType; code: cstring;
    text: cstring): Message =
  fnew(result, gBoxedFreeGstMessage)
  result.impl = gst_message_new_progress(if src.isNil: nil else: cast[ptr Object00](src.impl), `type`, code, text)

proc newMessageProgress*(tdesc: typedesc; src: Object = nil; `type`: ProgressType; code: cstring;
    text: cstring): tdesc =
  assert(result is Message)
  fnew(result, gBoxedFreeGstMessage)
  result.impl = gst_message_new_progress(if src.isNil: nil else: cast[ptr Object00](src.impl), `type`, code, text)

proc initMessageProgress*[T](result: var T; src: Object = nil; `type`: ProgressType; code: cstring;
...
proc newMessageStreamStatus*(src: Object = nil; `type`: StreamStatusType;
    owner: Element): Message =
  fnew(result, gBoxedFreeGstMessage)
  result.impl = gst_message_new_stream_status(if src.isNil: nil else: cast[ptr Object00](src.impl), `type`, cast[ptr Element00](owner.impl))

proc newMessageStreamStatus*(tdesc: typedesc; src: Object = nil; `type`: StreamStatusType;
    owner: Element): tdesc =
  assert(result is Message)
  fnew(result, gBoxedFreeGstMessage)
  result.impl = gst_message_new_stream_status(if src.isNil: nil else: cast[ptr Object00](src.impl), `type`, cast[ptr Element00](owner.impl))

proc initMessageStreamStatus*[T](result: var T; src: Object = nil; `type`: StreamStatusType;
...
proc gst_task_pool_dispose_handle(self: ptr TaskPool00; id: pointer) {.
    importc, libprag.}

proc disposeHandle*(self: TaskPool; id: pointer) =
  gst_task_pool_dispose_handle(cast[ptr TaskPool00](self.impl), id)
...
type
  SharedTaskPool* = ref object of TaskPool
  SharedTaskPool00* = object of TaskPool00

proc gst_shared_task_pool_get_type*(): GType {.importc, libprag.}

when defined(gcDestructors):
  proc `=destroy`*(self: var typeof(SharedTaskPool()[])) =
    when defined(gintroDebug):
      echo "destroy ", $typeof(self), ' ', cast[int](unsafeaddr self)
    g_object_set_qdata(self.impl, Quark, nil)
    if not self.ignoreFinalizer and self.impl != nil:
...
proc gst_shared_task_pool_new(): ptr SharedTaskPool00 {.
    importc, libprag.}

proc newSharedTaskPool*(): SharedTaskPool =
  let gobj = gst_shared_task_pool_new()
  let qdata = g_object_get_qdata(gobj, Quark)
  if qdata != nil:
    result = cast[type(result)](qdata)
    assert(result.impl == gobj)
...
proc newSharedTaskPool*(tdesc: typedesc): tdesc =
  assert(result is SharedTaskPool)
  let gobj = gst_shared_task_pool_new()
  let qdata = g_object_get_qdata(gobj, Quark)
  if qdata != nil:
    result = cast[type(result)](qdata)
...
proc initSharedTaskPool*[T](result: var T) {.deprecated.} =
  assert(result is SharedTaskPool)
  let gobj = gst_shared_task_pool_new()
  let qdata = g_object_get_qdata(gobj, Quark)
  if qdata != nil:
...
proc gst_shared_task_pool_get_max_threads(self: ptr SharedTaskPool00): uint32 {.
    importc, libprag.}

proc getMaxThreads*(self: SharedTaskPool): int =
  int(gst_shared_task_pool_get_max_threads(cast[ptr SharedTaskPool00](self.impl)))

proc maxThreads*(self: SharedTaskPool): int =
  int(gst_shared_task_pool_get_max_threads(cast[ptr SharedTaskPool00](self.impl)))

proc gst_shared_task_pool_set_max_threads(self: ptr SharedTaskPool00; maxThreads: uint32) {.
    importc, libprag.}

proc setMaxThreads*(self: SharedTaskPool; maxThreads: int) =
  gst_shared_task_pool_set_max_threads(cast[ptr SharedTaskPool00](self.impl), uint32(maxThreads))

proc `maxThreads=`*(self: SharedTaskPool; maxThreads: int) =
  gst_shared_task_pool_set_max_threads(cast[ptr SharedTaskPool00](self.impl), uint32(maxThreads))
...
proc gst_type_find_peek(self: ptr TypeFind00; offset: int64; size: uint32): ptr uint8 {.
    importc, libprag.}

proc peek*(self: TypeFind; offset: int64; size: int): ptr uint8 =
  gst_type_find_peek(cast[ptr TypeFind00](self.impl), offset, uint32(size))
...
proc gst_type_find_suggest_empty_simple(self: ptr TypeFind00; probability: uint32;
    mediaType: cstring) {.
    importc, libprag.}

proc suggestEmptySimple*(self: TypeFind; probability: int;
    mediaType: cstring) =
  gst_type_find_suggest_empty_simple(cast[ptr TypeFind00](self.impl), uint32(probability), mediaType)
...
proc appendPath*(self: Uri; relativePath: cstring = nil): bool =
...
proc appendPathSegment*(self: Uri; pathSegment: cstring = nil): bool =
...
proc fromStringWithBase*(self: Uri; uri: cstring): Uri =
  let impl0 = gst_uri_from_string_with_base(cast[ptr Uri00](self.impl), uri)
  if impl0.isNil:
    return nil
  fnew(result, gBoxedFreeGstUri)
  result.impl = impl0
...
proc gst_uri_get_query_string_ordered(self: ptr Uri00; keys: ptr glib.List): cstring {.
    importc, libprag.}

proc getQueryStringOrdered*(self: Uri; keys: seq[cstring]): string =
  var tempResGL = seq2GList(keys)
  let resul0 = gst_uri_get_query_string_ordered(cast[ptr Uri00](self.impl), tempResGL)
  g_list_free(tempResGL)
  if resul0.isNil:
    return
  result = $resul0
...
proc setPath*(self: Uri; path: cstring = nil): bool =
...
proc setQueryString*(self: Uri; query: cstring = nil): bool =
...
proc gst_uri_to_string_with_keys(self: ptr Uri00; keys: ptr glib.List): cstring {.
    importc, libprag.}

proc toStringWithKeys*(self: Uri; keys: seq[cstring]): string =
  var tempResGL = seq2GList(keys)
  let resul0 = gst_uri_to_string_with_keys(cast[ptr Uri00](self.impl), tempResGL)
  g_list_free(tempResGL)
  result = $resul0
  cogfree(resul0)
...
const VERSION_MICRO* = 2'i32

const VERSION_MINOR* = 24'i32
...
type
  ValueDeserializeWithPSpecFunc* = proc (dest: gobject.Value; s: cstring; pspec: ptr gobject.ParamSpec00): gboolean {.cdecl.}
...
proc gst_debug_log_default(category: ptr DebugCategory00; level: DebugLevel;
    file: cstring; function: cstring; line: int32; obj: ptr gobject.Object00;
    message: ptr DebugMessage00; userData: pointer) {.
    importc, libprag.}

proc debugLogDefault*(category: DebugCategory; level: DebugLevel; file: cstring;
    function: cstring; line: int; obj: gobject.Object = nil; message: DebugMessage;
    userData: pointer) =
  gst_debug_log_default(cast[ptr DebugCategory00](category.impl), level, file, function, int32(line), if obj.isNil: nil else: cast[ptr gobject.Object00](obj.impl), cast[ptr DebugMessage00](message.impl), userData)
...
proc debugLogGetLine*(category: DebugCategory; level: DebugLevel; file: cstring;
    function: cstring; line: int; obj: gobject.Object = nil; message: DebugMessage): string =
  let resul0 = gst_debug_log_get_line(cast[ptr DebugCategory00](category.impl), level, file, function, int32(line), if obj.isNil: nil else: cast[ptr gobject.Object00](obj.impl), cast[ptr DebugMessage00](message.impl))
  result = $resul0
  cogfree(resul0)

proc gst_debug_log_id_literal(category: ptr DebugCategory00; level: DebugLevel;
    file: cstring; function: cstring; line: int32; id: cstring; messageString: cstring) {.
    importc, libprag.}

proc debugLogIdLiteral*(category: DebugCategory; level: DebugLevel;
    file: cstring; function: cstring; line: int; id: cstring = nil; messageString: cstring) =
  gst_debug_log_id_literal(cast[ptr DebugCategory00](category.impl), level, file, function, int32(line), id, messageString)

proc gst_debug_log_literal(category: ptr DebugCategory00; level: DebugLevel;
    file: cstring; function: cstring; line: int32; obj: ptr gobject.Object00;
    messageString: cstring) {.
    importc, libprag.}

proc debugLogLiteral*(category: DebugCategory; level: DebugLevel; file: cstring;
    function: cstring; line: int; obj: gobject.Object = nil; messageString: cstring) =
  gst_debug_log_literal(cast[ptr DebugCategory00](category.impl), level, file, function, int32(line), if obj.isNil: nil else: cast[ptr gobject.Object00](obj.impl), messageString)
...
proc typeIsPluginApi*(`type`: GType; flags: var PluginAPIFlags = cast[var PluginAPIFlags](nil)): bool =
...
proc gst_util_ceil_log2(v: uint32): uint32 {.
    importc, libprag.}

proc utilCeilLog2*(v: int): int =
  int(gst_util_ceil_log2(uint32(v)))
...
proc gst_util_filename_compare(a: cstring; b: cstring): int32 {.
    importc, libprag.}

proc utilFilenameCompare*(a: cstring; b: cstring): int =
  int(gst_util_filename_compare(a, b))
...
proc gst_util_get_object_array(obj: ptr gobject.Object00; name: cstring;
    array: var ptr gobject.ValueArray00): gboolean {.
    importc, libprag.}

proc utilGetObjectArray*(obj: gobject.Object; name: cstring; array: var gobject.ValueArray): bool =
  fnew(array, gBoxedFreeGValueArray)
  toBool(gst_util_get_object_array(cast[ptr gobject.Object00](obj.impl), name, cast[var ptr gobject.ValueArray00](addr array.impl)))
...
proc gst_util_set_object_arg(obj: ptr gobject.Object00; name: cstring; value: cstring) {.
    importc, libprag.}

proc utilSetObjectArg*(obj: gobject.Object; name: cstring; value: cstring) =
  gst_util_set_object_arg(cast[ptr gobject.Object00](obj.impl), name, value)

proc gst_util_set_object_array(obj: ptr gobject.Object00; name: cstring;
    array: ptr gobject.ValueArray00): gboolean {.
    importc, libprag.}

proc utilSetObjectArray*(obj: gobject.Object; name: cstring; array: gobject.ValueArray): bool =
  toBool(gst_util_set_object_array(cast[ptr gobject.Object00](obj.impl), name, cast[ptr gobject.ValueArray00](array.impl)))

proc utilSetValueFromString*(value: var gobject.Value; valueStr: cstring) {.
    importc: "gst_util_set_value_from_string", libprag.}

proc gst_util_simplify_fraction(numerator: ptr int32; denominator: ptr int32;
    nTerms: uint32; threshold: uint32) {.
    importc, libprag.}

proc utilSimplifyFraction*(numerator: ptr int32; denominator: ptr int32;
    nTerms: int; threshold: int) =
  gst_util_simplify_fraction(numerator, denominator, uint32(nTerms), uint32(threshold))
...
proc gst_value_deserialize_with_pspec(dest: var gobject.Value; src: cstring;
    pspec: ptr gobject.ParamSpec00): gboolean {.
    importc, libprag.}

proc valueDeserializeWithPspec*(dest: var gobject.Value; src: cstring;
    pspec: gobject.ParamSpec = nil): bool =
  toBool(gst_value_deserialize_with_pspec(dest, src, if pspec.isNil: nil else: cast[ptr gobject.ParamSpec00](pspec.impl)))
