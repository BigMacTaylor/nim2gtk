...
import gobject, gdkpixbuf, cairo, glib, gmodule, gio
...
proc rsvg_handle_free(self: ptr Handle00) {.
    importc, libprag.}

proc free*(self: Handle) =
  rsvg_handle_free(cast[ptr Handle00](self.impl))
...
proc rsvg_handle_get_desc(self: ptr Handle00): cstring {.
    importc, libprag.}

proc getDesc*(self: Handle): string =
  let resul0 = rsvg_handle_get_desc(cast[ptr Handle00](self.impl))
  if resul0.isNil:
    return
  result = $resul0

proc desc*(self: Handle): string =
  let resul0 = rsvg_handle_get_desc(cast[ptr Handle00](self.impl))
  if resul0.isNil:
    return
  result = $resul0
...
proc rsvg_handle_get_intrinsic_size_in_pixels(self: ptr Handle00; outWidth: var cdouble;
    outHeight: var cdouble): gboolean {.
    importc, libprag.}

proc getIntrinsicSizeInPixels*(self: Handle; outWidth: var cdouble = cast[var cdouble](nil);
    outHeight: var cdouble = cast[var cdouble](nil)): bool =
  toBool(rsvg_handle_get_intrinsic_size_in_pixels(cast[ptr Handle00](self.impl), outWidth, outHeight))

proc rsvg_handle_get_metadata(self: ptr Handle00): cstring {.
    importc, libprag.}

proc getMetadata*(self: Handle): string =
  let resul0 = rsvg_handle_get_metadata(cast[ptr Handle00](self.impl))
  if resul0.isNil:
    return
  result = $resul0

proc metadata*(self: Handle): string =
  let resul0 = rsvg_handle_get_metadata(cast[ptr Handle00](self.impl))
  if resul0.isNil:
    return
  result = $resul0
...
proc rsvg_handle_get_title(self: ptr Handle00): cstring {.
    importc, libprag.}

proc getTitle*(self: Handle): string =
  let resul0 = rsvg_handle_get_title(cast[ptr Handle00](self.impl))
  if resul0.isNil:
    return
  result = $resul0

proc title*(self: Handle): string =
  let resul0 = rsvg_handle_get_title(cast[ptr Handle00](self.impl))
  if resul0.isNil:
    return
  result = $resul0
...
const MINOR_VERSION* = 58'i32

type
  SizeFunc* = proc (width: var int32; height: var int32; userData: pointer) {.cdecl.}

proc rsvg_handle_set_size_callback(self: ptr Handle00; sizeFunc: SizeFunc;
    userData: pointer; userDataDestroy: DestroyNotify) {.
    importc, libprag.}

proc setSizeCallback*(self: Handle; sizeFunc: SizeFunc; userData: pointer;
    userDataDestroy: DestroyNotify) =
  rsvg_handle_set_size_callback(cast[ptr Handle00](self.impl), sizeFunc, userData, userDataDestroy)
...
proc init*() {.
    importc: "rsvg_init", libprag.}

proc rsvg_pixbuf_from_file(filename: cstring; error: ptr ptr glib.Error = nil): ptr gdkpixbuf.Pixbuf00 {.
    importc, libprag.}

proc pixbufFromFile*(filename: cstring): gdkpixbuf.Pixbuf =
  var gerror: ptr glib.Error
  let gobj = rsvg_pixbuf_from_file(filename, addr gerror)
  if gerror != nil:
    let msg = $gerror.message
    g_error_free(gerror[])
    raise newException(GException, msg)
...
proc rsvg_pixbuf_from_file_at_max_size(filename: cstring; maxWidth: int32;
    maxHeight: int32; error: ptr ptr glib.Error = nil): ptr gdkpixbuf.Pixbuf00 {.
    importc, libprag.}

proc pixbufFromFileAtMaxSize*(filename: cstring; maxWidth: int;
    maxHeight: int): gdkpixbuf.Pixbuf =
  var gerror: ptr glib.Error
  let gobj = rsvg_pixbuf_from_file_at_max_size(filename, int32(maxWidth), int32(maxHeight), addr gerror)
  if gerror != nil:
    let msg = $gerror.message
    g_error_free(gerror[])
    raise newException(GException, msg)
...
proc rsvg_pixbuf_from_file_at_size(filename: cstring; width: int32; height: int32;
    error: ptr ptr glib.Error = nil): ptr gdkpixbuf.Pixbuf00 {.
    importc, libprag.}

proc pixbufFromFileAtSize*(filename: cstring; width: int; height: int): gdkpixbuf.Pixbuf =
  var gerror: ptr glib.Error
  let gobj = rsvg_pixbuf_from_file_at_size(filename, int32(width), int32(height), addr gerror)
  if gerror != nil:
    let msg = $gerror.message
    g_error_free(gerror[])
    raise newException(GException, msg)
...
proc rsvg_pixbuf_from_file_at_zoom(filename: cstring; xZoom: cdouble; yZoom: cdouble;
    error: ptr ptr glib.Error = nil): ptr gdkpixbuf.Pixbuf00 {.
    importc, libprag.}

proc pixbufFromFileAtZoom*(filename: cstring; xZoom: cdouble; yZoom: cdouble): gdkpixbuf.Pixbuf =
  var gerror: ptr glib.Error
  let gobj = rsvg_pixbuf_from_file_at_zoom(filename, xZoom, yZoom, addr gerror)
  if gerror != nil:
    let msg = $gerror.message
    g_error_free(gerror[])
    raise newException(GException, msg)
...
proc rsvg_pixbuf_from_file_at_zoom_with_max(filename: cstring; xZoom: cdouble;
    yZoom: cdouble; maxWidth: int32; maxHeight: int32; error: ptr ptr glib.Error = nil): ptr gdkpixbuf.Pixbuf00 {.
    importc, libprag.}

proc pixbufFromFileAtZoomWithMax*(filename: cstring; xZoom: cdouble;
    yZoom: cdouble; maxWidth: int; maxHeight: int): gdkpixbuf.Pixbuf =
  var gerror: ptr glib.Error
  let gobj = rsvg_pixbuf_from_file_at_zoom_with_max(filename, xZoom, yZoom, int32(maxWidth), int32(maxHeight), addr gerror)
  if gerror != nil:
    let msg = $gerror.message
    g_error_free(gerror[])
    raise newException(GException, msg)
