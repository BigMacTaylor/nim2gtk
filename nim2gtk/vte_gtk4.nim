...
import pangocairo, gtk4, freetype2, glib, gdkpixbuf, harfbuzz, gmodule, cairo, graphene, gobject, pango, gio, gsk, gdk4
const Lib = "libvte-2.91-gtk4.so.0"
...
type
  EventContext00* {.pure.} = object
  EventContext* = ref object
    impl*: ptr EventContext00
    ignoreFinalizer*: bool

proc vte_event_context_get_coordinates(self: ptr EventContext00; x: ptr cdouble;
    y: ptr cdouble): gboolean {.
    importc, libprag.}

proc getCoordinates*(self: EventContext; x: ptr cdouble;
    y: ptr cdouble): bool =
  toBool(vte_event_context_get_coordinates(cast[ptr EventContext00](self.impl), x, y))
...
proc newPtyForeignSync*(fd: int; cancellable: gio.Cancellable = nil): Pty =
  ...
  if qdata != nil:
    ...
    fnew(result, vte_gtk4.finalizeGObject)
    result.impl = gobj
    GC_ref(result)
...
proc vte_regex_new_for_match_full(pattern: cstring; patternLength: int64;
    flags: uint32; extraFlags: uint32; errorOffset: var uint64; error: ptr ptr glib.Error = nil): ptr Regex00 {.
    importc, libprag.}

proc newRegexForMatchFull*(pattern: cstring; patternLength: int64;
    flags: int; extraFlags: int; errorOffset: var uint64 = cast[var uint64](nil)): Regex =
  var gerror: ptr glib.Error
  let impl0 = vte_regex_new_for_match_full(pattern, patternLength, uint32(flags), uint32(extraFlags), errorOffset, addr gerror)
  if gerror != nil:
    let msg = $gerror.message
    g_error_free(gerror[])
    raise newException(GException, msg)
...
proc newRegexForMatchFull*(tdesc: typedesc; pattern: cstring; patternLength: int64;
    flags: int; extraFlags: int; errorOffset: var uint64 = cast[var uint64](nil)): tdesc =
  var gerror: ptr glib.Error
...
proc initRegexForMatchFull*[T](result: var T; pattern: cstring; patternLength: int64;
    flags: int; extraFlags: int; errorOffset: var uint64 = cast[var uint64](nil)) {.deprecated.} =
  var gerror: ptr glib.Error
  assert(result is Regex)
  let impl0 = vte_regex_new_for_match_full(pattern, patternLength, uint32(flags), uint32(extraFlags), errorOffset, addr gerror)
  if gerror != nil:
...
proc vte_regex_new_for_search_full(pattern: cstring; patternLength: int64;
    flags: uint32; extraFlags: uint32; errorOffset: var uint64; error: ptr ptr glib.Error = nil): ptr Regex00 {.
    importc, libprag.}

proc newRegexForSearchFull*(pattern: cstring; patternLength: int64;
    flags: int; extraFlags: int; errorOffset: var uint64 = cast[var uint64](nil)): Regex =
  var gerror: ptr glib.Error
  let impl0 = vte_regex_new_for_search_full(pattern, patternLength, uint32(flags), uint32(extraFlags), errorOffset, addr gerror)
  if gerror != nil:
    let msg = $gerror.message
    g_error_free(gerror[])
    raise newException(GException, msg)
...
proc newRegexForSearchFull*(tdesc: typedesc; pattern: cstring; patternLength: int64;
    flags: int; extraFlags: int; errorOffset: var uint64 = cast[var uint64](nil)): tdesc =
  var gerror: ptr glib.Error
  assert(result is Regex)
  let impl0 = vte_regex_new_for_search_full(pattern, patternLength, uint32(flags), uint32(extraFlags), errorOffset, addr gerror)
...
proc initRegexForSearchFull*[T](result: var T; pattern: cstring; patternLength: int64;
    flags: int; extraFlags: int; errorOffset: var uint64 = cast[var uint64](nil)) {.deprecated.} =
  var gerror: ptr glib.Error
  assert(result is Regex)
  let impl0 = vte_regex_new_for_search_full(pattern, patternLength, uint32(flags), uint32(extraFlags), errorOffset, addr gerror)
...
type
  Terminal* = ref object of gtk4.Widget
  Terminal00* = object of gtk4.Widget00
...
proc scHyperlinkHoverUriChanged*(self: Terminal;  p: proc (self: ptr Terminal00; uri: cstring; bbox: gdk4.Rectangle; xdata: pointer) {.cdecl.}, xdata: pointer, cf: gobject.ConnectFlags): culong =
...
proc scSetupContextMenu*(self: Terminal;  p: proc (self: ptr Terminal00; context: ptr ptr EventContext00; xdata: pointer) {.cdecl.}, xdata: pointer, cf: gobject.ConnectFlags): culong =
  g_signal_connect_data(self.impl, "setup-context-menu", cast[GCallback](p), xdata, nil, cf)
...
proc vte_terminal_check_hyperlink_at(self: ptr Terminal00; x: cdouble; y: cdouble): cstring {.
    importc, libprag.}

proc checkHyperlinkAt*(self: Terminal; x: cdouble; y: cdouble): string =
  let resul0 = vte_terminal_check_hyperlink_at(cast[ptr Terminal00](self.impl), x, y)
  if resul0.isNil:
    return
  result = $resul0
  cogfree(resul0)

proc vte_terminal_check_match_at(self: ptr Terminal00; x: cdouble; y: cdouble;
    tag: var int32): cstring {.
    importc, libprag.}

proc checkMatchAt*(self: Terminal; x: cdouble; y: cdouble;
    tag: var int = cast[var int](nil)): string =
  var tag_00: int32
  let resul0 = vte_terminal_check_match_at(cast[ptr Terminal00](self.impl), x, y, tag_00)
  if resul0.isNil:
    return
  result = $resul0
  cogfree(resul0)
...
proc vte_terminal_check_regex_array_at(self: ptr Terminal00; x: cdouble;
    y: cdouble; regexes: ptr ptr Regex00; nRegexes: uint64; matchFlags: uint32;
    nMatches: var uint64): ptr cstring {.
    importc, libprag.}

proc checkRegexSimpleAt*(self: Terminal; x: cdouble; y: cdouble;
    regexes: ptr ptr Regex00; nRegexes: uint64; matchFlags: int; nMatches: var uint64 = cast[var uint64](nil)): seq[string] =
  let resul0 = vte_terminal_check_regex_array_at(cast[ptr Terminal00](self.impl), x, y, regexes, nRegexes, uint32(matchFlags), nMatches)
  if resul0.isNil:
    return
...
proc vte_terminal_get_color_background_for_draw(self: ptr Terminal00; color: var gdk4.RGBA) {.
    importc, libprag.}

proc getColorBackgroundForDraw*(self: Terminal; color: var gdk4.RGBA) =
  vte_terminal_get_color_background_for_draw(cast[ptr Terminal00](self.impl), color)

proc getColorBackgroundForDraw*(self: Terminal): gdk4.RGBA =
...
proc vte_terminal_get_context_menu(self: ptr Terminal00): ptr gtk4.Widget00 {.
    importc, libprag.}

proc getContextMenu*(self: Terminal): gtk4.Widget =
  let gobj = vte_terminal_get_context_menu(cast[ptr Terminal00](self.impl))
  if gobj.isNil:
    return nil
  let qdata = g_object_get_qdata(gobj, Quark)
  if qdata != nil:
...
proc contextMenu*(self: Terminal): gtk4.Widget =
  let gobj = vte_terminal_get_context_menu(cast[ptr Terminal00](self.impl))
  if gobj.isNil:
    return nil
  let qdata = g_object_get_qdata(gobj, Quark)
...
proc vte_terminal_get_context_menu_model(self: ptr Terminal00): ptr gio.MenuModel00 {.
    importc, libprag.}

proc getContextMenuModel*(self: Terminal): gio.MenuModel =
  let gobj = vte_terminal_get_context_menu_model(cast[ptr Terminal00](self.impl))
  if gobj.isNil:
    return nil
  let qdata = g_object_get_qdata(gobj, Quark)
  if qdata != nil:
...
proc contextMenuModel*(self: Terminal): gio.MenuModel =
  let gobj = vte_terminal_get_context_menu_model(cast[ptr Terminal00](self.impl))
  if gobj.isNil:
    return nil
  let qdata = g_object_get_qdata(gobj, Quark)
...
proc vte_terminal_get_enable_fallback_scrolling(self: ptr Terminal00): gboolean {.
    importc, libprag.}

proc getEnableFallbackScrolling*(self: Terminal): bool =
  toBool(vte_terminal_get_enable_fallback_scrolling(cast[ptr Terminal00](self.impl)))

proc enableFallbackScrolling*(self: Terminal): bool =
  toBool(vte_terminal_get_enable_fallback_scrolling(cast[ptr Terminal00](self.impl)))
...
proc vte_terminal_get_font_options(self: ptr Terminal00): ptr cairo.FontOptions00 {.
    importc, libprag.}

proc getFontOptions*(self: Terminal): cairo.FontOptions =
  let impl0 = vte_terminal_get_font_options(cast[ptr Terminal00](self.impl))
  if impl0.isNil:
    return nil
...
proc fontOptions*(self: Terminal): cairo.FontOptions =
  let impl0 = vte_terminal_get_font_options(cast[ptr Terminal00](self.impl))
  if impl0.isNil:
    return nil
  fnew(result, gBoxedFreeCairoFontOptions)
...
proc vte_terminal_get_scroll_on_insert(self: ptr Terminal00): gboolean {.
    importc, libprag.}

proc getScrollOnInsert*(self: Terminal): bool =
  toBool(vte_terminal_get_scroll_on_insert(cast[ptr Terminal00](self.impl)))

proc scrollOnInsert*(self: Terminal): bool =
  toBool(vte_terminal_get_scroll_on_insert(cast[ptr Terminal00](self.impl)))
...
proc vte_terminal_get_scroll_unit_is_pixels(self: ptr Terminal00): gboolean {.
    importc, libprag.}

proc getScrollUnitIsPixels*(self: Terminal): bool =
  toBool(vte_terminal_get_scroll_unit_is_pixels(cast[ptr Terminal00](self.impl)))

proc scrollUnitIsPixels*(self: Terminal): bool =
  toBool(vte_terminal_get_scroll_unit_is_pixels(cast[ptr Terminal00](self.impl)))
...
proc vte_terminal_get_text_format(self: ptr Terminal00; format: Format): cstring {.
    importc, libprag.}

proc getTextFormat*(self: Terminal; format: Format): string =
  let resul0 = vte_terminal_get_text_format(cast[ptr Terminal00](self.impl), format)
  if resul0.isNil:
    return
  result = $resul0
  cogfree(resul0)

proc vte_terminal_get_text_range_format(self: ptr Terminal00; format: Format;
    startRow: int64; startCol: int64; endRow: int64; endCol: int64; length: var uint64): cstring {.
    importc, libprag.}

proc getTextRangeFormat*(self: Terminal; format: Format; startRow: int64;
    startCol: int64; endRow: int64; endCol: int64; length: var uint64 = cast[var uint64](nil)): string =
  let resul0 = vte_terminal_get_text_range_format(cast[ptr Terminal00](self.impl), format, startRow, startCol, endRow, endCol, length)
  if resul0.isNil:
    return
  result = $resul0
  cogfree(resul0)

proc vte_terminal_get_text_selected(self: ptr Terminal00; format: Format): cstring {.
    importc, libprag.}

proc getTextSelected*(self: Terminal; format: Format): string =
  let resul0 = vte_terminal_get_text_selected(cast[ptr Terminal00](self.impl), format)
  if resul0.isNil:
    return
  result = $resul0
  cogfree(resul0)

proc vte_terminal_get_text_selected_full(self: ptr Terminal00; format: Format;
    length: var uint64): cstring {.
    importc, libprag.}

proc getTextSelectedFull*(self: Terminal; format: Format;
    length: var uint64 = cast[var uint64](nil)): string =
  let resul0 = vte_terminal_get_text_selected_full(cast[ptr Terminal00](self.impl), format, length)
  if resul0.isNil:
    return
  result = $resul0
  cogfree(resul0)
...
proc vte_terminal_get_xalign(self: ptr Terminal00): Align {.
    importc, libprag.}

proc getXalign*(self: Terminal): Align =
  vte_terminal_get_xalign(cast[ptr Terminal00](self.impl))

proc xalign*(self: Terminal): Align =
  vte_terminal_get_xalign(cast[ptr Terminal00](self.impl))

proc vte_terminal_get_xfill(self: ptr Terminal00): gboolean {.
    importc, libprag.}

proc getXfill*(self: Terminal): bool =
  toBool(vte_terminal_get_xfill(cast[ptr Terminal00](self.impl)))

proc xfill*(self: Terminal): bool =
  toBool(vte_terminal_get_xfill(cast[ptr Terminal00](self.impl)))

proc vte_terminal_get_yalign(self: ptr Terminal00): Align {.
    importc, libprag.}

proc getYalign*(self: Terminal): Align =
  vte_terminal_get_yalign(cast[ptr Terminal00](self.impl))

proc yalign*(self: Terminal): Align =
  vte_terminal_get_yalign(cast[ptr Terminal00](self.impl))

proc vte_terminal_get_yfill(self: ptr Terminal00): gboolean {.
    importc, libprag.}

proc getYfill*(self: Terminal): bool =
  toBool(vte_terminal_get_yfill(cast[ptr Terminal00](self.impl)))

proc yfill*(self: Terminal): bool =
  toBool(vte_terminal_get_yfill(cast[ptr Terminal00](self.impl)))
...
proc vte_terminal_match_set_cursor(self: ptr Terminal00; tag: int32; cursor: ptr gdk4.Cursor00) {.
    importc, libprag.}

proc matchSetCursor*(self: Terminal; tag: int; cursor: gdk4.Cursor = nil) =
  vte_terminal_match_set_cursor(cast[ptr Terminal00](self.impl), int32(tag), if cursor.isNil: nil else: cast[ptr gdk4.Cursor00](cursor.impl))
...
proc vte_terminal_paste_text(self: ptr Terminal00; text: cstring) {.
    importc, libprag.}

proc pasteText*(self: Terminal; text: cstring) =
  vte_terminal_paste_text(cast[ptr Terminal00](self.impl), text)
...
proc vte_terminal_set_color_background(self: ptr Terminal00; background: gdk4.RGBA) {.
    importc, libprag.}

proc setColorBackground*(self: Terminal; background: gdk4.RGBA) =
  vte_terminal_set_color_background(cast[ptr Terminal00](self.impl), background)

proc `colorBackground=`*(self: Terminal; background: gdk4.RGBA) =
  vte_terminal_set_color_background(cast[ptr Terminal00](self.impl), background)

proc vte_terminal_set_color_bold(self: ptr Terminal00; bold: gdk4.RGBA) {.
    importc, libprag.}

proc setColorBold*(self: Terminal; bold: gdk4.RGBA = cast[var gdk4.RGBA](nil)) =
  vte_terminal_set_color_bold(cast[ptr Terminal00](self.impl), bold)

proc `colorBold=`*(self: Terminal; bold: gdk4.RGBA = cast[var gdk4.RGBA](nil)) =
  vte_terminal_set_color_bold(cast[ptr Terminal00](self.impl), bold)

proc vte_terminal_set_color_cursor(self: ptr Terminal00; cursorBackground: gdk4.RGBA) {.
    importc, libprag.}

proc setColorCursor*(self: Terminal; cursorBackground: gdk4.RGBA = cast[var gdk4.RGBA](nil)) =
  vte_terminal_set_color_cursor(cast[ptr Terminal00](self.impl), cursorBackground)

proc `colorCursor=`*(self: Terminal; cursorBackground: gdk4.RGBA = cast[var gdk4.RGBA](nil)) =
  vte_terminal_set_color_cursor(cast[ptr Terminal00](self.impl), cursorBackground)

proc vte_terminal_set_color_cursor_foreground(self: ptr Terminal00; cursorForeground: gdk4.RGBA) {.
    importc, libprag.}

proc setColorCursorForeground*(self: Terminal; cursorForeground: gdk4.RGBA = cast[var gdk4.RGBA](nil)) =
  vte_terminal_set_color_cursor_foreground(cast[ptr Terminal00](self.impl), cursorForeground)

proc `colorCursorForeground=`*(self: Terminal; cursorForeground: gdk4.RGBA = cast[var gdk4.RGBA](nil)) =
  vte_terminal_set_color_cursor_foreground(cast[ptr Terminal00](self.impl), cursorForeground)

proc vte_terminal_set_color_foreground(self: ptr Terminal00; foreground: gdk4.RGBA) {.
    importc, libprag.}

proc setColorForeground*(self: Terminal; foreground: gdk4.RGBA) =
  vte_terminal_set_color_foreground(cast[ptr Terminal00](self.impl), foreground)

proc `colorForeground=`*(self: Terminal; foreground: gdk4.RGBA) =
  vte_terminal_set_color_foreground(cast[ptr Terminal00](self.impl), foreground)

proc vte_terminal_set_color_highlight(self: ptr Terminal00; highlightBackground: gdk4.RGBA) {.
    importc, libprag.}

proc setColorHighlight*(self: Terminal; highlightBackground: gdk4.RGBA = cast[var gdk4.RGBA](nil)) =
  vte_terminal_set_color_highlight(cast[ptr Terminal00](self.impl), highlightBackground)

proc `colorHighlight=`*(self: Terminal; highlightBackground: gdk4.RGBA = cast[var gdk4.RGBA](nil)) =
  vte_terminal_set_color_highlight(cast[ptr Terminal00](self.impl), highlightBackground)

proc vte_terminal_set_color_highlight_foreground(self: ptr Terminal00; highlightForeground: gdk4.RGBA) {.
    importc, libprag.}

proc setColorHighlightForeground*(self: Terminal; highlightForeground: gdk4.RGBA = cast[var gdk4.RGBA](nil)) =
  vte_terminal_set_color_highlight_foreground(cast[ptr Terminal00](self.impl), highlightForeground)

proc `colorHighlightForeground=`*(self: Terminal; highlightForeground: gdk4.RGBA = cast[var gdk4.RGBA](nil)) =
  vte_terminal_set_color_highlight_foreground(cast[ptr Terminal00](self.impl), highlightForeground)

proc vte_terminal_set_colors(self: ptr Terminal00; foreground: gdk4.RGBA;
    background: gdk4.RGBA; palette: ptr gdk4.RGBA; paletteSize: uint64) {.
    importc, libprag.}

proc setColors*(self: Terminal; foreground: gdk4.RGBA = cast[var gdk4.RGBA](nil);
    background: gdk4.RGBA = cast[var gdk4.RGBA](nil); palette: ptr gdk4.RGBA;
    paletteSize: uint64) =
  vte_terminal_set_colors(cast[ptr Terminal00](self.impl), foreground, background, palette, paletteSize)

proc vte_terminal_set_context_menu(self: ptr Terminal00; menu: ptr gtk4.Widget00) {.
    importc, libprag.}

proc setContextMenu*(self: Terminal; menu: gtk4.Widget = nil) =
  vte_terminal_set_context_menu(cast[ptr Terminal00](self.impl), if menu.isNil: nil else: cast[ptr gtk4.Widget00](menu.impl))

proc `contextMenu=`*(self: Terminal; menu: gtk4.Widget = nil) =
  vte_terminal_set_context_menu(cast[ptr Terminal00](self.impl), if menu.isNil: nil else: cast[ptr gtk4.Widget00](menu.impl))

proc vte_terminal_set_context_menu_model(self: ptr Terminal00; model: ptr gio.MenuModel00) {.
    importc, libprag.}

proc setContextMenuModel*(self: Terminal; model: gio.MenuModel = nil) =
  vte_terminal_set_context_menu_model(cast[ptr Terminal00](self.impl), if model.isNil: nil else: cast[ptr gio.MenuModel00](model.impl))

proc `contextMenuModel=`*(self: Terminal; model: gio.MenuModel = nil) =
  vte_terminal_set_context_menu_model(cast[ptr Terminal00](self.impl), if model.isNil: nil else: cast[ptr gio.MenuModel00](model.impl))
...
proc vte_terminal_set_enable_fallback_scrolling(self: ptr Terminal00; enable: gboolean) {.
    importc, libprag.}

proc setEnableFallbackScrolling*(self: Terminal; enable: bool = true) =
  vte_terminal_set_enable_fallback_scrolling(cast[ptr Terminal00](self.impl), gboolean(enable))

proc `enableFallbackScrolling=`*(self: Terminal; enable: bool) =
  vte_terminal_set_enable_fallback_scrolling(cast[ptr Terminal00](self.impl), gboolean(enable))
...
proc vte_terminal_set_font_options(self: ptr Terminal00; fontOptions: ptr cairo.FontOptions00) {.
    importc, libprag.}

proc setFontOptions*(self: Terminal; fontOptions: cairo.FontOptions = nil) =
  vte_terminal_set_font_options(cast[ptr Terminal00](self.impl), if fontOptions.isNil: nil else: cast[ptr cairo.FontOptions00](fontOptions.impl))

proc `fontOptions=`*(self: Terminal; fontOptions: cairo.FontOptions = nil) =
  vte_terminal_set_font_options(cast[ptr Terminal00](self.impl), if fontOptions.isNil: nil else: cast[ptr cairo.FontOptions00](fontOptions.impl))
...
proc vte_terminal_set_scroll_on_insert(self: ptr Terminal00; scroll: gboolean) {.
    importc, libprag.}

proc setScrollOnInsert*(self: Terminal; scroll: bool = true) =
  vte_terminal_set_scroll_on_insert(cast[ptr Terminal00](self.impl), gboolean(scroll))

proc `scrollOnInsert=`*(self: Terminal; scroll: bool) =
  vte_terminal_set_scroll_on_insert(cast[ptr Terminal00](self.impl), gboolean(scroll))
...
proc vte_terminal_set_scroll_unit_is_pixels(self: ptr Terminal00; enable: gboolean) {.
    importc, libprag.}

proc setScrollUnitIsPixels*(self: Terminal; enable: bool = true) =
  vte_terminal_set_scroll_unit_is_pixels(cast[ptr Terminal00](self.impl), gboolean(enable))

proc `scrollUnitIsPixels=`*(self: Terminal; enable: bool) =
  vte_terminal_set_scroll_unit_is_pixels(cast[ptr Terminal00](self.impl), gboolean(enable))
...
proc vte_terminal_set_xalign(self: ptr Terminal00; align: Align) {.
    importc, libprag.}

proc setXalign*(self: Terminal; align: Align) =
  vte_terminal_set_xalign(cast[ptr Terminal00](self.impl), align)

proc `xalign=`*(self: Terminal; align: Align) =
  vte_terminal_set_xalign(cast[ptr Terminal00](self.impl), align)

proc vte_terminal_set_xfill(self: ptr Terminal00; fill: gboolean) {.
    importc, libprag.}

proc setXfill*(self: Terminal; fill: bool = true) =
  vte_terminal_set_xfill(cast[ptr Terminal00](self.impl), gboolean(fill))

proc `xfill=`*(self: Terminal; fill: bool) =
  vte_terminal_set_xfill(cast[ptr Terminal00](self.impl), gboolean(fill))

proc vte_terminal_set_yalign(self: ptr Terminal00; align: Align) {.
    importc, libprag.}

proc setYalign*(self: Terminal; align: Align) =
  vte_terminal_set_yalign(cast[ptr Terminal00](self.impl), align)

proc `yalign=`*(self: Terminal; align: Align) =
  vte_terminal_set_yalign(cast[ptr Terminal00](self.impl), align)

proc vte_terminal_set_yfill(self: ptr Terminal00; fill: gboolean) {.
    importc, libprag.}

proc setYfill*(self: Terminal; fill: bool = true) =
  vte_terminal_set_yfill(cast[ptr Terminal00](self.impl), gboolean(fill))

proc `yfill=`*(self: Terminal; fill: bool) =
  vte_terminal_set_yfill(cast[ptr Terminal00](self.impl), gboolean(fill))
...
proc getText*(self: Terminal; isSelected: SelectionFunc; userData: pointer;
    attributes: var ptr GArray00 = cast[var ptr GArray00](nil)): string =
...
proc getTextIncludeTrailingSpaces*(self: Terminal; isSelected: SelectionFunc;
    userData: pointer; attributes: var ptr GArray00 = cast[var ptr GArray00](nil)): string =
...
proc eventContextGetType*(): GType {.
    importc: "vte_event_context_get_type", libprag.}
...
proc initable*(x: vte_gtk4.Pty): gio.Initable = cast[gio.Initable](x)

proc accessible*(x: vte_gtk4.Terminal): gtk4.Accessible = cast[gtk4.Accessible](x)

proc buildable*(x: vte_gtk4.Terminal): gtk4.Buildable = cast[gtk4.Buildable](x)

proc constraintTarget*(x: vte_gtk4.Terminal): gtk4.ConstraintTarget = cast[gtk4.ConstraintTarget](x)

proc scrollable*(x: vte_gtk4.Terminal): gtk4.Scrollable = cast[gtk4.Scrollable](x)
