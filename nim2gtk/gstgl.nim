...
proc gst_gl_context_get_config(self: ptr GLContext00): ptr gst.Structure00 {.
    importc, libprag.}

proc getConfig*(self: GLContext): gst.Structure =
  let impl0 = gst_gl_context_get_config(cast[ptr GLContext00](self.impl))
  if impl0.isNil:
    return nil
  fnew(result, gBoxedFreeGstStructure)
  result.impl = impl0

proc config*(self: GLContext): gst.Structure =
  let impl0 = gst_gl_context_get_config(cast[ptr GLContext00](self.impl))
  if impl0.isNil:
    return nil
  fnew(result, gBoxedFreeGstStructure)
...
proc getThread*(self: GLContext): glib.Thread =
  let impl0 = gst_gl_context_get_thread(cast[ptr GLContext00](self.impl))
  if impl0.isNil:
    return nil
...
proc gst_gl_context_request_config(self: ptr GLContext00; glConfig: ptr gst.Structure00): gboolean {.
    importc, libprag.}

proc requestConfig*(self: GLContext; glConfig: gst.Structure = nil): bool =
  toBool(gst_gl_context_request_config(cast[ptr GLContext00](self.impl), if glConfig.isNil: nil else: cast[ptr gst.Structure00](g_boxed_copy(gst_structure_get_type(), glConfig.impl))))
...
proc scCreateContext*(self: GLDisplay;  p: proc (self: ptr GLDisplay00; context: ptr GLContext00; xdata: pointer): ptr GLContext00 {.cdecl.}, xdata: pointer, cf: gobject.ConnectFlags): culong =
...
proc createContext*(self: GLDisplay; otherContext: GLContext = nil;
    pContext: var GLContext): bool =
  var tmpoutgobjectarg: ptr GLContext00
  var gerror: ptr glib.Error
  let resul0 = gst_gl_display_create_context(cast[ptr GLDisplay00](self.impl), if otherContext.isNil: nil else: cast[ptr GLContext00](otherContext.impl), tmpoutgobjectarg, addr gerror)
  if gerror != nil:
    let msg = $gerror.message
    g_error_free(gerror[])
    raise newException(GException, msg)
...
proc gst_gl_display_ensure_context(self: ptr GLDisplay00; otherContext: ptr GLContext00;
    context: var ptr GLContext00; error: ptr ptr glib.Error = nil): gboolean {.
    importc, libprag.}

proc ensureContext*(self: GLDisplay; otherContext: GLContext = nil;
    context: var GLContext): bool =
  var gerror: ptr glib.Error
  let resul0 = gst_gl_display_ensure_context(cast[ptr GLDisplay00](self.impl), if otherContext.isNil: nil else: cast[ptr GLContext00](otherContext.impl), cast[var ptr GLContext00](if addr(context) == nil: nil else: addr context.impl), addr gerror)
  if gerror != nil:
    let msg = $gerror.message
    g_error_free(gerror[])
    raise newException(GException, msg)
...
proc gst_gl_display_new_with_type(`type`: GLDisplayType): ptr GLDisplay00 {.
    importc, libprag.}

proc newGLDisplayWithType*(`type`: GLDisplayType): GLDisplay =
  let gobj = gst_gl_display_new_with_type(`type`)
  if gobj.isNil:
    return nil
  let qdata = g_object_get_qdata(gobj, Quark)
  if qdata != nil:
...
proc newGLDisplayWithType*(tdesc: typedesc; `type`: GLDisplayType): tdesc =
  assert(result is GLDisplay)
  let gobj = gst_gl_display_new_with_type(`type`)
  if gobj.isNil:
    return nil
...
proc initGLDisplayWithType*[T](result: var T; `type`: GLDisplayType) {.deprecated.} =
  assert(result is GLDisplay)
  let gobj = gst_gl_display_new_with_type(`type`)
  if gobj.isNil:
    return nil
...
proc scWindowHandleChanged*(self: GLWindow;  p: proc (self: ptr gobject.Object00; xdata: pointer) {.cdecl.}, xdata: pointer, cf: gobject.ConnectFlags): culong =
  g_signal_connect_data(self.impl, "window-handle-changed", cast[GCallback](p), xdata, nil, cf)
...
proc alloc*(allocator: GLBaseMemoryAllocator; params: GLAllocationParams): GLBaseMemory =
  let impl0 = gst_gl_base_memory_alloc(cast[ptr GLBaseMemoryAllocator00](allocator.impl), cast[ptr GLAllocationParams00](params.impl))
  if impl0.isNil:
    return nil
...
type
  GLBaseMemoryTransferFlag* {.size: sizeof(cint), pure.} = enum
...
type
  ...
  GLBaseMixer00* = object of gstvideo.VideoAggregator00

proc gst_gl_base_mixer_get_type*(): GType {.importc, libprag.}

when defined(gcDestructors):
  proc `=destroy`*(self: var typeof(GLBaseMixer()[])) =
    when defined(gintroDebug):
      echo "destroy ", $typeof(self), ' ', cast[int](unsafeaddr self)
    g_object_set_qdata(self.impl, Quark, nil)
    if not self.ignoreFinalizer and self.impl != nil:
...
proc gst_gl_base_mixer_get_gl_context(self: ptr GLBaseMixer00): ptr GLContext00 {.
    importc, libprag.}

proc getGlContext*(self: GLBaseMixer): GLContext =
  let gobj = gst_gl_base_mixer_get_gl_context(cast[ptr GLBaseMixer00](self.impl))
  if gobj.isNil:
    return nil
  let qdata = g_object_get_qdata(gobj, Quark)
  if qdata != nil:
...
proc glContext*(self: GLBaseMixer): GLContext =
  let gobj = gst_gl_base_mixer_get_gl_context(cast[ptr GLBaseMixer00](self.impl))
  if gobj.isNil:
    return nil
  let qdata = g_object_get_qdata(gobj, Quark)
...
type
  GLBaseMixerPad* = ref object of gstvideo.VideoAggregatorPad
  GLBaseMixerPad00* = object of gstvideo.VideoAggregatorPad00

proc gst_gl_base_mixer_pad_get_type*(): GType {.importc, libprag.}

when defined(gcDestructors):
  proc `=destroy`*(self: var typeof(GLBaseMixerPad()[])) =
    when defined(gintroDebug):
      echo "destroy ", $typeof(self), ' ', cast[int](unsafeaddr self)
    g_object_set_qdata(self.impl, Quark, nil)
    if not self.ignoreFinalizer and self.impl != nil:
...
proc gst_gl_buffer_get_type*(): GType {.importc, libprag.}

proc gBoxedFreeGstGLBuffer*(self: GLBuffer) =
  if not self.ignoreFinalizer:
    boxedFree(gst_gl_buffer_get_type(), cast[ptr GLBuffer00](self.impl))

when defined(gcDestructors):
  proc `=destroy`*(self: var typeof(GLBuffer()[])) =
    when defined(gintroDebug):
      echo "destroy ", $typeof(self), ' ', cast[int](unsafeaddr self)
    if not self.ignoreFinalizer and self.impl != nil:
      boxedFree(gst_gl_buffer_get_type(), cast[ptr GLBuffer00](self.impl))
...
proc newWithFinalizer*(x: var GLBuffer) =
  when defined(gcDestructors):
    new(x)
  else:
    new(x, gBoxedFreeGstGLBuffer)
...
proc gst_gl_buffer_pool_get_gl_allocation_params(self: ptr GLBufferPool00): ptr GLAllocationParams00 {.
    importc, libprag.}

proc getGlAllocationParams*(self: GLBufferPool): GLAllocationParams =
  let impl0 = gst_gl_buffer_pool_get_gl_allocation_params(cast[ptr GLBufferPool00](self.impl))
  if impl0.isNil:
    return nil
  fnew(result, gBoxedFreeGstGLAllocationParams)
  result.impl = impl0

proc glAllocationParams*(self: GLBufferPool): GLAllocationParams =
  let impl0 = gst_gl_buffer_pool_get_gl_allocation_params(cast[ptr GLBufferPool00](self.impl))
  if impl0.isNil:
    return nil
  fnew(result, gBoxedFreeGstGLAllocationParams)
...
proc gst_gl_color_convert_swizzle_shader_string(context: ptr GLContext00): cstring {.
    importc, libprag.}

proc swizzleShaderString*(context: GLContext): string =
  let resul0 = gst_gl_color_convert_swizzle_shader_string(cast[ptr GLContext00](context.impl))
  result = $resul0
  cogfree(resul0)
...
proc gst_gl_color_convert_yuv_to_rgb_shader_string(context: ptr GLContext00): cstring {.
    importc, libprag.}

proc yuvToRgbShaderString*(context: GLContext): string =
  let resul0 = gst_gl_color_convert_yuv_to_rgb_shader_string(cast[ptr GLContext00](context.impl))
  result = $resul0
  cogfree(resul0)
...
proc perform*(self: GLColorConvert; inbuf: gst.Buffer): gst.Buffer =
  let impl0 = gst_gl_color_convert_perform(cast[ptr GLColorConvert00](self.impl), cast[ptr gst.Buffer00](inbuf.impl))
  if impl0.isNil:
    return nil
...
type
  GLConfigCaveat* {.size: sizeof(cint), pure.} = enum
    none = 0
    slow = 1
    nonConformant = 2

proc gst_gl_config_caveat_to_string(caveat: GLConfigCaveat): cstring {.
    importc, libprag.}

proc toString*(caveat: GLConfigCaveat): string =
  let resul0 = gst_gl_config_caveat_to_string(caveat)
  if resul0.isNil:
    return
  result = $resul0

type
  GLConfigSurfaceTypeFlag* {.size: sizeof(cint), pure.} = enum
    window = 0
    pbuffer = 1
    pixmap = 2
...
proc none*(t: typedesc[GLConfigSurfaceType]): GLConfigSurfaceType = GLConfigSurfaceType({})

proc gst_gl_config_surface_type_to_string(surfaceType: GLConfigSurfaceType): cstring {.
    importc, libprag.}

proc toString*(surfaceType: GLConfigSurfaceType): string =
  let resul0 = gst_gl_config_surface_type_to_string(surfaceType)
  if resul0.isNil:
    return
  result = $resul0
...
proc gst_gl_memory_get_type*(): GType {.importc, libprag.}

proc gBoxedFreeGstGLMemory*(self: GLMemory) =
  if not self.ignoreFinalizer:
    boxedFree(gst_gl_memory_get_type(), cast[ptr GLMemory00](self.impl))

when defined(gcDestructors):
  proc `=destroy`*(self: var typeof(GLMemory()[])) =
    when defined(gintroDebug):
      echo "destroy ", $typeof(self), ' ', cast[int](unsafeaddr self)
    if not self.ignoreFinalizer and self.impl != nil:
      boxedFree(gst_gl_memory_get_type(), cast[ptr GLMemory00](self.impl))
...
proc newWithFinalizer*(x: var GLMemory) =
  when defined(gcDestructors):
    new(x)
  else:
    new(x, gBoxedFreeGstGLMemory)
...
proc gst_gl_format_n_components(glFormat: GLFormat): uint32 {.
    importc, libprag.}

proc nComponents*(glFormat: GLFormat): int =
  int(gst_gl_format_n_components(glFormat))
...
proc toBufferPoolOption*(target: GLTextureTarget): string =
  let resul0 = gst_gl_texture_target_to_buffer_pool_option(target)
  if resul0.isNil:
    return
  result = $resul0
...
proc toString*(target: GLTextureTarget): string =
  let resul0 = gst_gl_texture_target_to_string(target)
  if resul0.isNil:
    return
  result = $resul0
...
proc gst_gl_memory_pbo_get_type*(): GType {.importc, libprag.}

proc gBoxedFreeGstGLMemoryPBO*(self: GLMemoryPBO) =
  if not self.ignoreFinalizer:
    boxedFree(gst_gl_memory_pbo_get_type(), cast[ptr GLMemoryPBO00](self.impl))

when defined(gcDestructors):
  proc `=destroy`*(self: var typeof(GLMemoryPBO()[])) =
    when defined(gintroDebug):
      echo "destroy ", $typeof(self), ' ', cast[int](unsafeaddr self)
    if not self.ignoreFinalizer and self.impl != nil:
      boxedFree(gst_gl_memory_pbo_get_type(), cast[ptr GLMemoryPBO00](self.impl))
...
proc newWithFinalizer*(x: var GLMemoryPBO) =
  when defined(gcDestructors):
    new(x)
  else:
    new(x, gBoxedFreeGstGLMemoryPBO)
...
type
  GLMixer* = ref object of GLBaseMixer
  GLMixer00* = object of GLBaseMixer00

proc gst_gl_mixer_get_type*(): GType {.importc, libprag.}

when defined(gcDestructors):
  proc `=destroy`*(self: var typeof(GLMixer()[])) =
    when defined(gintroDebug):
      echo "destroy ", $typeof(self), ' ', cast[int](unsafeaddr self)
    g_object_set_qdata(self.impl, Quark, nil)
    if not self.ignoreFinalizer and self.impl != nil:
...
proc gst_gl_mixer_get_framebuffer(self: ptr GLMixer00): ptr GLFramebuffer00 {.
    importc, libprag.}

proc getFramebuffer*(self: GLMixer): GLFramebuffer =
  let gobj = gst_gl_mixer_get_framebuffer(cast[ptr GLMixer00](self.impl))
  let qdata = g_object_get_qdata(gobj, Quark)
  if qdata != nil:
    result = cast[type(result)](qdata)
    assert(result.impl == gobj)
...
proc framebuffer*(self: GLMixer): GLFramebuffer =
  let gobj = gst_gl_mixer_get_framebuffer(cast[ptr GLMixer00](self.impl))
  let qdata = g_object_get_qdata(gobj, Quark)
  if qdata != nil:
    result = cast[type(result)](qdata)
...
proc gst_gl_mixer_process_textures(self: ptr GLMixer00; outbuf: ptr gst.Buffer00): gboolean {.
    importc, libprag.}

proc processTextures*(self: GLMixer; outbuf: gst.Buffer): bool =
  toBool(gst_gl_mixer_process_textures(cast[ptr GLMixer00](self.impl), cast[ptr gst.Buffer00](outbuf.impl)))

type
  GLMixerPad* = ref object of GLBaseMixerPad
  GLMixerPad00* = object of GLBaseMixerPad00

proc gst_gl_mixer_pad_get_type*(): GType {.importc, libprag.}

when defined(gcDestructors):
  proc `=destroy`*(self: var typeof(GLMixerPad()[])) =
    when defined(gintroDebug):
      echo "destroy ", $typeof(self), ' ', cast[int](unsafeaddr self)
    g_object_set_qdata(self.impl, Quark, nil)
    if not self.ignoreFinalizer and self.impl != nil:
...
proc gst_gl_renderbuffer_get_type*(): GType {.importc, libprag.}

proc gBoxedFreeGstGLRenderbuffer*(self: GLRenderbuffer) =
  if not self.ignoreFinalizer:
    boxedFree(gst_gl_renderbuffer_get_type(), cast[ptr GLRenderbuffer00](self.impl))

when defined(gcDestructors):
  proc `=destroy`*(self: var typeof(GLRenderbuffer()[])) =
    when defined(gintroDebug):
      echo "destroy ", $typeof(self), ' ', cast[int](unsafeaddr self)
    if not self.ignoreFinalizer and self.impl != nil:
      boxedFree(gst_gl_renderbuffer_get_type(), cast[ptr GLRenderbuffer00](self.impl))
...
proc newWithFinalizer*(x: var GLRenderbuffer) =
  when defined(gcDestructors):
    new(x)
  else:
    new(x, gBoxedFreeGstGLRenderbuffer)
...
proc gst_gl_upload_fixate_caps(self: ptr GLUpload00; direction: gst.PadDirection;
    caps: ptr gst.Caps00; othercaps: ptr gst.Caps00): ptr gst.Caps00 {.
    importc, libprag.}

proc fixateCaps*(self: GLUpload; direction: gst.PadDirection;
    caps: gst.Caps; othercaps: gst.Caps): gst.Caps =
  fnew(result, gBoxedFreeGstCaps)
  result.impl = gst_gl_upload_fixate_caps(cast[ptr GLUpload00](self.impl), direction, cast[ptr gst.Caps00](caps.impl), cast[ptr gst.Caps00](g_boxed_copy(gst_caps_get_type(), othercaps.impl)))
...
proc perform*(self: GLViewConvert; inbuf: gst.Buffer): gst.Buffer =
  let impl0 = gst_gl_view_convert_perform(cast[ptr GLViewConvert00](self.impl), cast[ptr gst.Buffer00](inbuf.impl))
  if impl0.isNil:
    return nil
  fnew(result, gBoxedFreeGstBuffer)
  result.impl = impl0
...
proc bufferPoolConfigGetGlAllocationParams*(config: gst.Structure): GLAllocationParams =
  let impl0 = gst_buffer_pool_config_get_gl_allocation_params(cast[ptr gst.Structure00](config.impl))
  if impl0.isNil:
    return nil
  fnew(result, gBoxedFreeGstGLAllocationParams)
  result.impl = impl0

proc gst_buffer_pool_config_get_gl_min_free_queue_size(config: ptr gst.Structure00): uint32 {.
    importc, libprag.}

proc bufferPoolConfigGetGlMinFreeQueueSize*(config: gst.Structure): int =
  int(gst_buffer_pool_config_get_gl_min_free_queue_size(cast[ptr gst.Structure00](config.impl)))
...
proc bufferPoolConfigSetGlAllocationParams*(config: gst.Structure;
    params: GLAllocationParams = nil) =
  gst_buffer_pool_config_set_gl_allocation_params(cast[ptr gst.Structure00](config.impl), if params.isNil: nil else: cast[ptr GLAllocationParams00](params.impl))

proc gst_buffer_pool_config_set_gl_min_free_queue_size(config: ptr gst.Structure00;
    queueSize: uint32) {.
    importc, libprag.}

proc bufferPoolConfigSetGlMinFreeQueueSize*(config: gst.Structure;
    queueSize: int) =
  gst_buffer_pool_config_set_gl_min_free_queue_size(cast[ptr gst.Structure00](config.impl), uint32(queueSize))

proc gst_context_get_gl_display(context: ptr gst.Context00; display: var ptr GLDisplay00): gboolean {.
    importc, libprag.}

proc contextGetGlDisplay*(context: gst.Context; display: var GLDisplay = cast[var GLDisplay](nil)): bool =
  var tmpoutgobjectarg: ptr GLDisplay00
  result = toBool(gst_context_get_gl_display(cast[ptr gst.Context00](context.impl), cast[var ptr GLDisplay00](if addr(display) == nil: nil else: addr tmpoutgobjectarg)))
#  dothemagic(display
  if addr(display) != nil:
    display = nil
...
proc contextSetGlDisplay*(context: gst.Context; display: GLDisplay = nil) =
  gst_context_set_gl_display(cast[ptr gst.Context00](context.impl), if display.isNil: nil else: cast[ptr GLDisplay00](display.impl))
...
proc gst_gl_ensure_element_data(element: ptr gst.Element00; displayPtr: var ptr GLDisplay00;
    otherContextPtr: var ptr GLContext00): gboolean {.
    importc, libprag.}

proc glEnsureElementData*(element: gst.Element; displayPtr: var GLDisplay;
    otherContextPtr: var GLContext): bool =
  toBool(gst_gl_ensure_element_data(cast[ptr gst.Element00](element.impl), cast[var ptr GLDisplay00](addr displayPtr.impl), cast[var ptr GLContext00](addr otherContextPtr.impl)))

proc gst_gl_get_affine_transformation_meta_as_ndc(meta: ptr gstvideo.VideoAffineTransformationMeta00;
    matrix: var ptr array[16, cfloat]) {.
    importc, libprag.}

proc glGetAffineTransformationMetaAsNdc*(meta: gstvideo.VideoAffineTransformationMeta = nil;
    matrix: var ptr array[16, cfloat]) =
  gst_gl_get_affine_transformation_meta_as_ndc(if meta.isNil: nil else: cast[ptr gstvideo.VideoAffineTransformationMeta00](meta.impl), matrix)

proc glGetAffineTransformationMetaAsNdc*(meta: gstvideo.VideoAffineTransformationMeta = nil): ptr array[16, cfloat] =
  gst_gl_get_affine_transformation_meta_as_ndc(if meta.isNil: nil else: cast[ptr gstvideo.VideoAffineTransformationMeta00](meta.impl), result)
...
#  dothemagic(otherContext
  let argqdata = g_object_get_qdata(tmpoutgobjectarg, Quark)
  if argqdata != nil:
    otherContext = cast[type(otherContext)](argqdata)
    assert(otherContext.impl == tmpoutgobjectarg)
  else:
    fnew(otherContext, gstgl.finalizeGObject)
    otherContext.impl = tmpoutgobjectarg
    GC_ref(otherContext)
    if g_object_is_floating(otherContext.impl).int != 0:
      discard g_object_ref_sink(otherContext.impl)
    g_object_add_toggle_ref(otherContext.impl, toggleNotify, addr(otherContext[]))
    g_object_unref(otherContext.impl)
    assert(g_object_get_qdata(otherContext.impl, Quark) == nil)
    g_object_set_qdata(otherContext.impl, Quark, addr(otherContext[]))


proc glMultiplyMatrix4*(a: ptr array[16, cfloat]; b: ptr array[16, cfloat];
    resu: var ptr array[16, cfloat]) {.
    importc: "gst_gl_multiply_matrix4", libprag.}

proc gst_gl_set_affine_transformation_meta_from_ndc(meta: ptr gstvideo.VideoAffineTransformationMeta00;
    matrix: ptr array[16, cfloat]) {.
    importc, libprag.}

proc glSetAffineTransformationMetaFromNdc*(meta: gstvideo.VideoAffineTransformationMeta;
    matrix: ptr array[16, cfloat]) =
  gst_gl_set_affine_transformation_meta_from_ndc(cast[ptr gstvideo.VideoAffineTransformationMeta00](meta.impl), matrix)
...
proc glSwizzleInvert*(swizzle: ptr array[4, int32]; inversion: var ptr array[4, int32]) {.
    importc: "gst_gl_swizzle_invert", libprag.}

proc glSwizzleInvert*(swizzle: ptr array[4, int32]): ptr array[4, int32] {.
    importc: "gst_gl_swizzle_invert", libprag.}
...
proc gst_gl_video_format_swizzle(videoFormat: gstvideo.VideoFormat; swizzle: var ptr array[4, int32]): gboolean {.
    importc, libprag.}

proc glVideoFormatSwizzle*(videoFormat: gstvideo.VideoFormat; swizzle: var ptr array[4, int32]): bool =
  toBool(gst_gl_video_format_swizzle(videoFormat, swizzle))
