...
proc doTunnel*(self: RTSPConnection; conn2: RTSPConnection = nil): RTSPResult =
  gst_rtsp_connection_do_tunnel(cast[ptr RTSPConnection00](self.impl), if conn2.isNil: nil else: cast[ptr RTSPConnection00](conn2.impl))
...
proc gst_rtsp_connection_add_extra_http_request_header(self: ptr RTSPConnection00;
    key: cstring; value: cstring) {.
    importc, libprag.}

proc addExtraHttpRequestHeader*(self: RTSPConnection;
    key: cstring; value: cstring) =
  gst_rtsp_connection_add_extra_http_request_header(cast[ptr RTSPConnection00](self.impl), key, value)

proc gst_rtsp_connection_get_ignore_x_server_reply(self: ptr RTSPConnection00): gboolean {.
    importc, libprag.}

proc getIgnoreXServerReply*(self: RTSPConnection): bool =
  toBool(gst_rtsp_connection_get_ignore_x_server_reply(cast[ptr RTSPConnection00](self.impl)))

proc ignoreXServerReply*(self: RTSPConnection): bool =
  toBool(gst_rtsp_connection_get_ignore_x_server_reply(cast[ptr RTSPConnection00](self.impl)))
...
proc getTunnelid*(self: RTSPConnection): string =
  let resul0 = gst_rtsp_connection_get_tunnelid(cast[ptr RTSPConnection00](self.impl))
  if resul0.isNil:
    return
  result = $resul0
...
proc read*(self: RTSPConnection; data: seq[uint8] | string;
    timeout: glib.TimeVal): RTSPResult =
  let size = int(data.len)
  gst_rtsp_connection_read(cast[ptr RTSPConnection00](self.impl), cast[ptr uint8](unsafeaddr(data[0])), uint32(size), timeout)

proc gst_rtsp_connection_read_usec(self: ptr RTSPConnection00; data: ptr uint8;
    size: uint32; timeout: int64): RTSPResult {.
    importc, libprag.}

proc readUsec*(self: RTSPConnection; data: seq[uint8] | string;
    timeout: int64): RTSPResult =
  let size = int(data.len)
  gst_rtsp_connection_read_usec(cast[ptr RTSPConnection00](self.impl), cast[ptr uint8](unsafeaddr(data[0])), uint32(size), timeout)
...
proc gst_rtsp_connection_set_ignore_x_server_reply(self: ptr RTSPConnection00;
    ignore: gboolean) {.
    importc, libprag.}

proc setIgnoreXServerReply*(self: RTSPConnection;
    ignore: bool = true) =
  gst_rtsp_connection_set_ignore_x_server_reply(cast[ptr RTSPConnection00](self.impl), gboolean(ignore))

proc `ignoreXServerReply=`*(self: RTSPConnection;
    ignore: bool) =
  gst_rtsp_connection_set_ignore_x_server_reply(cast[ptr RTSPConnection00](self.impl), gboolean(ignore))
...
proc setTlsDatabase*(self: RTSPConnection; database: gio.TlsDatabase = nil) =
  gst_rtsp_connection_set_tls_database(cast[ptr RTSPConnection00](self.impl), if database.isNil: nil else: cast[ptr gio.TlsDatabase00](database.impl))

proc `tlsDatabase=`*(self: RTSPConnection; database: gio.TlsDatabase = nil) =
  gst_rtsp_connection_set_tls_database(cast[ptr RTSPConnection00](self.impl), if database.isNil: nil else: cast[ptr gio.TlsDatabase00](database.impl))
...
proc setTlsInteraction*(self: RTSPConnection; interaction: gio.TlsInteraction = nil) =
  gst_rtsp_connection_set_tls_interaction(cast[ptr RTSPConnection00](self.impl), if interaction.isNil: nil else: cast[ptr gio.TlsInteraction00](interaction.impl))

proc `tlsInteraction=`*(self: RTSPConnection; interaction: gio.TlsInteraction = nil) =
  gst_rtsp_connection_set_tls_interaction(cast[ptr RTSPConnection00](self.impl), if interaction.isNil: nil else: cast[ptr gio.TlsInteraction00](interaction.impl))
...
proc write*(self: RTSPConnection; data: seq[uint8] | string;
    timeout: glib.TimeVal): RTSPResult =
  let size = int(data.len)
  gst_rtsp_connection_write(cast[ptr RTSPConnection00](self.impl), cast[ptr uint8](unsafeaddr(data[0])), uint32(size), timeout)

proc gst_rtsp_connection_write_usec(self: ptr RTSPConnection00; data: ptr uint8;
    size: uint32; timeout: int64): RTSPResult {.
    importc, libprag.}

proc writeUsec*(self: RTSPConnection; data: seq[uint8] | string;
    timeout: int64): RTSPResult =
  let size = int(data.len)
  gst_rtsp_connection_write_usec(cast[ptr RTSPConnection00](self.impl), cast[ptr uint8](unsafeaddr(data[0])), uint32(size), timeout)
...
proc gst_rtsp_connection_poll(self: ptr RTSPConnection00; events: RTSPEvent;
    revents: var RTSPEvent; timeout: glib.TimeVal): RTSPResult {.
    importc, libprag.}

proc poll*(self: RTSPConnection; events: RTSPEvent; revents: var RTSPEvent;
    timeout: glib.TimeVal): RTSPResult =
...
proc gst_rtsp_connection_poll_usec(self: ptr RTSPConnection00; events: RTSPEvent;
    revents: var RTSPEvent; timeout: int64): RTSPResult {.
    importc, libprag.}
...
proc getHeaderByName*(self: RTSPMessage; header: cstring;
    value: var string = cast[var string](nil); index: int): RTSPResult =
...
proc parseData*(self: RTSPMessage; channel: var uint8 = cast[var uint8](nil)): RTSPResult =
...
type
  RTSPMethodFlag* {.size: sizeof(cint), pure.} = enum
    describe = 0
    announce = 1
    getParameter = 2
    options = 3
...
proc invalid*(t: typedesc[RTSPMethod]): RTSPMethod = RTSPMethod({})

proc gst_rtsp_method_as_text(`method`: RTSPMethod): cstring {.
    importc, libprag.}

proc asText*(`method`: RTSPMethod): string =
  let resul0 = gst_rtsp_method_as_text(`method`)
  if resul0.isNil:
    return
  result = $resul0
...
proc getHeader*(self: RTSPMessage; field: RTSPHeaderField;
    value: var string = cast[var string](nil); indx: int): RTSPResult =
  var value_00: cstring
...
proc scSend*(self: RTSPExtension;  p: proc (self: ptr RTSPExtension00; obj: pointer; p0: pointer; xdata: pointer): RTSPResult {.cdecl.}, xdata: pointer, cf: gobject.ConnectFlags): culong =
...
type
  RTSPLowerTransFlag* {.size: sizeof(cint), pure.} = enum
    udp = 0
    udpMcast = 1
    tcp = 2
    http = 4
...
proc unknown*(t: typedesc[RTSPLowerTrans]): RTSPLowerTrans = RTSPLowerTrans({})
...
type
  RTSPProfileFlag* {.size: sizeof(cint), pure.} = enum
    avp = 0
    savp = 1
    avpf = 2
    savpf = 3
...
proc unknown*(t: typedesc[RTSPProfile]): RTSPProfile = RTSPProfile({})
...
proc gst_rtsp_transport_get_mime(trans: RTSPTransMode; mime: var cstring): RTSPResult {.
    importc, libprag.}

proc getMime*(trans: RTSPTransMode; mime: var string): RTSPResult =
  var mime_00: cstring
  result = gst_rtsp_transport_get_mime(trans, mime_00)
  if mime.addr != nil:
    mime = $(mime_00)

proc gst_rtsp_transport_init(transport: var RTSPTransport00): RTSPResult {.
    importc, libprag.}

proc init*(transport: var RTSPTransport00): RTSPResult =
  gst_rtsp_transport_init(transport)

proc gst_rtsp_transport_new(transport: var ptr RTSPTransport00): RTSPResult {.
    importc, libprag.}

proc new*(transport: var RTSPTransport): RTSPResult =
  fnew(transport, finalizerfree)
  gst_rtsp_transport_new(cast[var ptr RTSPTransport00](addr transport.impl))

proc gst_rtsp_transport_parse(str: cstring; transport: var RTSPTransport00): RTSPResult {.
    importc, libprag.}

proc parse*(str: cstring; transport: var RTSPTransport00): RTSPResult =
  gst_rtsp_transport_parse(str, transport)
...
proc attach*(self: RTSPWatch; context: glib.MainContext = nil): int =
  int(gst_rtsp_watch_attach(cast[ptr RTSPWatch00](self.impl), if context.isNil: nil else: cast[ptr glib.MainContext00](context.impl)))
...
proc rtspHeaderAsText*(field: RTSPHeaderField): string =
  let resul0 = gst_rtsp_header_as_text(field)
  if resul0.isNil:
    return
  result = $resul0
