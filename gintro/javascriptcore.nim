...
proc jsc_value_array_buffer_get_data(self: ptr Value00; size: ptr uint64): pointer {.
    importc, libprag.}

proc arrayBufferGetData*(self: Value; size: ptr uint64): pointer =
  jsc_value_array_buffer_get_data(cast[ptr Value00](self.impl), size)

proc jsc_value_array_buffer_get_size(self: ptr Value00): uint64 {.
    importc, libprag.}

proc arrayBufferGetSize*(self: Value): uint64 =
  jsc_value_array_buffer_get_size(cast[ptr Value00](self.impl))
...
proc jsc_value_is_array_buffer(self: ptr Value00): gboolean {.
    importc, libprag.}

proc isArrayBuffer*(self: Value): bool =
  toBool(jsc_value_is_array_buffer(cast[ptr Value00](self.impl)))
...
proc jsc_value_is_typed_array(self: ptr Value00): gboolean {.
    importc, libprag.}

proc isTypedArray*(self: Value): bool =
  toBool(jsc_value_is_typed_array(cast[ptr Value00](self.impl)))
...
proc jsc_value_typed_array_get_buffer(self: ptr Value00): ptr Value00 {.
    importc, libprag.}

proc typedArrayGetBuffer*(self: Value): Value =
  let gobj = jsc_value_typed_array_get_buffer(cast[ptr Value00](self.impl))
  let qdata = g_object_get_qdata(gobj, Quark)
  if qdata != nil:
    result = cast[type(result)](qdata)
    assert(result.impl == gobj)
...
proc jsc_value_typed_array_get_data(self: ptr Value00; length: var uint64): pointer {.
    importc, libprag.}

proc typedArrayGetData*(self: Value; length: var uint64): pointer =
  jsc_value_typed_array_get_data(cast[ptr Value00](self.impl), length)

proc jsc_value_typed_array_get_length(self: ptr Value00): uint64 {.
    importc, libprag.}

proc typedArrayGetLength*(self: Value): uint64 =
  jsc_value_typed_array_get_length(cast[ptr Value00](self.impl))

proc jsc_value_typed_array_get_offset(self: ptr Value00): uint64 {.
    importc, libprag.}

proc typedArrayGetOffset*(self: Value): uint64 =
  jsc_value_typed_array_get_offset(cast[ptr Value00](self.impl))

proc jsc_value_typed_array_get_size(self: ptr Value00): uint64 {.
    importc, libprag.}

proc typedArrayGetSize*(self: Value): uint64 =
  jsc_value_typed_array_get_size(cast[ptr Value00](self.impl))
...
proc jsc_context_evaluate_in_object(self: ptr Context00; code: cstring; length: int64;
    ...
    obj: var ptr Value00): ptr Value00 {.
    importc, libprag.}
...
proc jsc_value_new_array_buffer(context: ptr Context00; data: pointer; size: uint64;
    destroyNotify: DestroyNotify; userData: pointer): ptr Value00 {.
    importc, libprag.}

proc newValueArrayBuffer*(context: Context; data: pointer; size: uint64;
    destroyNotify: DestroyNotify; userData: pointer): Value =
  let gobj = jsc_value_new_array_buffer(cast[ptr Context00](context.impl), data, size, destroyNotify, userData)
  if gobj.isNil:
    return nil
  let qdata = g_object_get_qdata(gobj, Quark)
  if qdata != nil:
...
proc newValueArrayBuffer*(tdesc: typedesc; context: Context; data: pointer; size: uint64;
    destroyNotify: DestroyNotify; userData: pointer): tdesc =
...
proc initValueArrayBuffer*[T](result: var T; context: Context; data: pointer; size: uint64;
    destroyNotify: DestroyNotify; userData: pointer) {.deprecated.} =
  assert(result is Value)
  let gobj = jsc_value_new_array_buffer(cast[ptr Context00](context.impl), data, size, destroyNotify, userData)
  if gobj.isNil:
    return nil
...
type
  TypedArrayType* {.size: sizeof(cint), pure.} = enum
    none = 0
    int8 = 1
    int16 = 2
    int32 = 3
...
proc jsc_value_new_typed_array(context: ptr Context00; `type`: TypedArrayType;
    length: uint64): ptr Value00 {.
    importc, libprag.}

proc newValueTypedArray*(context: Context; `type`: TypedArrayType;
    length: uint64): Value =
  let gobj = jsc_value_new_typed_array(cast[ptr Context00](context.impl), `type`, length)
  let qdata = g_object_get_qdata(gobj, Quark)
  if qdata != nil:
    result = cast[type(result)](qdata)
    assert(result.impl == gobj)
...
proc newValueTypedArray*(tdesc: typedesc; context: Context; `type`: TypedArrayType;
    length: uint64): tdesc =
  assert(result is Value)
  let gobj = jsc_value_new_typed_array(cast[ptr Context00](context.impl), `type`, length)
  let qdata = g_object_get_qdata(gobj, Quark)
...
proc initValueTypedArray*[T](result: var T; context: Context; `type`: TypedArrayType;
    length: uint64) {.deprecated.} =
  assert(result is Value)
  let gobj = jsc_value_new_typed_array(cast[ptr Context00](context.impl), `type`, length)
  let qdata = g_object_get_qdata(gobj, Quark)
  if qdata != nil:
...
proc jsc_value_new_typed_array_with_buffer(self: ptr Value00; `type`: TypedArrayType;
    offset: uint64; length: int64): ptr Value00 {.
    importc, libprag.}

proc newValueTypedArrayWithBuffer*(self: Value; `type`: TypedArrayType;
    offset: uint64; length: int64): Value =
  let gobj = jsc_value_new_typed_array_with_buffer(cast[ptr Value00](self.impl), `type`, offset, length)
  let qdata = g_object_get_qdata(gobj, Quark)
  if qdata != nil:
    result = cast[type(result)](qdata)
    assert(result.impl == gobj)
...
proc jsc_value_typed_array_get_type(self: ptr Value00): TypedArrayType {.
    importc, libprag.}

proc typedArrayGetType*(self: Value): TypedArrayType =
  jsc_value_typed_array_get_type(cast[ptr Value00](self.impl))
...
type
  Executor* = proc (resolve: ptr Value00; reject: ptr Value00; userData: pointer) {.cdecl.}

proc jsc_value_new_promise(context: ptr Context00; executor: Executor; userData: pointer): ptr Value00 {.
    importc, libprag.}

proc newValuePromise*(context: Context; executor: Executor; userData: pointer): Value =
  let gobj = jsc_value_new_promise(cast[ptr Context00](context.impl), executor, userData)
  let qdata = g_object_get_qdata(gobj, Quark)
  if qdata != nil:
    result = cast[type(result)](qdata)
    assert(result.impl == gobj)
...
proc newValuePromise*(tdesc: typedesc; context: Context; executor: Executor; userData: pointer): tdesc =
  assert(result is Value)
  let gobj = jsc_value_new_promise(cast[ptr Context00](context.impl), executor, userData)
  let qdata = g_object_get_qdata(gobj, Quark)
  if qdata != nil:
...
proc initValuePromise*[T](result: var T; context: Context; executor: Executor; userData: pointer) {.deprecated.} =
  assert(result is Value)
  let gobj = jsc_value_new_promise(cast[ptr Context00](context.impl), executor, userData)
  let qdata = g_object_get_qdata(gobj, Quark)
  if qdata != nil:
