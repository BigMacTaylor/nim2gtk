...
proc setQdata*(self: gobject.Object; quark: int; p: pointer) =
    g_object_set_qdata(cast[ptr Object00](self.impl), uint32(quark), p)
...
proc g_binding_dup_source(self: ptr Binding00): ptr Object00 {.
    importc, libprag.}

proc dupSource*(self: Binding): Object =
  let gobj = g_binding_dup_source(cast[ptr Binding00](self.impl))
  if gobj.isNil:
    return nil
...
proc g_binding_dup_target(self: ptr Binding00): ptr Object00 {.
    importc, libprag.}

proc dupTarget*(self: Binding): Object =
  let gobj = g_binding_dup_target(cast[ptr Binding00](self.impl))
  if gobj.isNil:
    return nil
  let qdata = g_object_get_qdata(gobj, Quark)
  if qdata != nil:
...
proc getName*(self: ParamSpec): string =
  result = $g_param_spec_get_name(cast[ptr ParamSpec00](self.impl))

proc name*(self: ParamSpec): string =
  result = $g_param_spec_get_name(cast[ptr ParamSpec00](self.impl))

proc g_param_spec_get_name_quark(self: ptr ParamSpec00): uint32 {.
    importc, libprag.}

proc getNameQuark*(self: ParamSpec): int =
  int(g_param_spec_get_name_quark(cast[ptr ParamSpec00](self.impl)))

proc nameQuark*(self: ParamSpec): int =
  int(g_param_spec_get_name_quark(cast[ptr ParamSpec00](self.impl)))
...
proc g_closure_new_object(sizeofClosure: uint32; obj: ptr Object00): ptr Closure00 {.
    importc, libprag.}

proc newClosureObject*(sizeofClosure: int; obj: Object): Closure =
  fnew(result, gBoxedFreeGClosure)
  result.impl = g_closure_new_object(uint32(sizeofClosure), cast[ptr Object00](obj.impl))

proc newClosureObject*(tdesc: typedesc; sizeofClosure: int; obj: Object): tdesc =
  assert(result is Closure)
  fnew(result, gBoxedFreeGClosure)
  result.impl = g_closure_new_object(uint32(sizeofClosure), cast[ptr Object00](obj.impl))

proc initClosureObject*[T](result: var T; sizeofClosure: int; obj: Object) {.deprecated.} =
  assert(result is Closure)
  fnew(result, gBoxedFreeGClosure)
  result.impl = g_closure_new_object(uint32(sizeofClosure), cast[ptr Object00](obj.impl))
...
type
  BindingGroup* = ref object of Object
  BindingGroup00* = object of Object00

proc g_binding_group_get_type*(): GType {.importc, libprag.}

when defined(gcDestructors):
  proc `=destroy`*(self: var typeof(BindingGroup()[])) =
    when defined(gintroDebug):
      echo "destroy ", $typeof(self), ' ', cast[int](unsafeaddr self)
    g_object_set_qdata(self.impl, Quark, nil)
    if not self.ignoreFinalizer and self.impl != nil:
...
proc g_binding_group_new(): ptr BindingGroup00 {.
    importc, libprag.}

proc newBindingGroup*(): BindingGroup =
  let gobj = g_binding_group_new()
  let qdata = g_object_get_qdata(gobj, Quark)
  if qdata != nil:
    result = cast[type(result)](qdata)
    assert(result.impl == gobj)
...
proc newBindingGroup*(tdesc: typedesc): tdesc =
  assert(result is BindingGroup)
  let gobj = g_binding_group_new()
  let qdata = g_object_get_qdata(gobj, Quark)
  if qdata != nil:
    result = cast[type(result)](qdata)
...
proc initBindingGroup*[T](result: var T) {.deprecated.} =
  assert(result is BindingGroup)
  let gobj = g_binding_group_new()
  let qdata = g_object_get_qdata(gobj, Quark)
  if qdata != nil:
...
proc g_binding_group_bind(self: ptr BindingGroup00; sourceProperty: cstring;
    target: ptr Object00; targetProperty: cstring; flags: BindingFlags) {.
    importc, libprag.}

proc `bind`*(self: BindingGroup; sourceProperty: cstring; target: Object;
    targetProperty: cstring; flags: BindingFlags) =
  g_binding_group_bind(cast[ptr BindingGroup00](self.impl), sourceProperty, cast[ptr Object00](target.impl), targetProperty, flags)

proc g_binding_group_bind_with_closures(self: ptr BindingGroup00; sourceProperty: cstring;
    target: ptr Object00; targetProperty: cstring; flags: BindingFlags; transformTo: ptr Closure00;
    transformFrom: ptr Closure00) {.
    importc, libprag.}

proc bindFull*(self: BindingGroup; sourceProperty: cstring;
    target: Object; targetProperty: cstring; flags: BindingFlags; transformTo: Closure = nil;
    transformFrom: Closure = nil) =
  g_binding_group_bind_with_closures(cast[ptr BindingGroup00](self.impl), sourceProperty, cast[ptr Object00](target.impl), targetProperty, flags, if transformTo.isNil: nil else: cast[ptr Closure00](transformTo.impl), if transformFrom.isNil: nil else: cast[ptr Closure00](transformFrom.impl))

proc g_binding_group_dup_source(self: ptr BindingGroup00): ptr Object00 {.
    importc, libprag.}

proc dupSource*(self: BindingGroup): Object =
  let gobj = g_binding_group_dup_source(cast[ptr BindingGroup00](self.impl))
  if gobj.isNil:
    return nil
  let qdata = g_object_get_qdata(gobj, Quark)
  if qdata != nil:
...
proc g_binding_group_set_source(self: ptr BindingGroup00; source: ptr Object00) {.
    importc, libprag.}

proc setSource*(self: BindingGroup; source: Object = nil) =
  g_binding_group_set_source(cast[ptr BindingGroup00](self.impl), if source.isNil: nil else: cast[ptr Object00](source.impl))

proc `source=`*(self: BindingGroup; source: Object = nil) =
  g_binding_group_set_source(cast[ptr BindingGroup00](self.impl), if source.isNil: nil else: cast[ptr Object00](source.impl))
...
proc default*(t: typedesc[ConnectFlags]): ConnectFlags = ConnectFlags({})
...
proc none*(t: typedesc[TypeFlags]): TypeFlags = TypeFlags({})
...
type
  ObjectFinalizeFunc* = proc (obj: ptr Object00) {.cdecl.}
...
type
  ...
  SignalGroup00* = object of Object00

proc g_signal_group_get_type*(): GType {.importc, libprag.}

when defined(gcDestructors):
  proc `=destroy`*(self: var typeof(SignalGroup()[])) =
    when defined(gintroDebug):
      echo "destroy ", $typeof(self), ' ', cast[int](unsafeaddr self)
    g_object_set_qdata(self.impl, Quark, nil)
    if not self.ignoreFinalizer and self.impl != nil:
...
proc scBind*(self: SignalGroup;  p: proc (self: ptr SignalGroup00; instance: ptr Object00; xdata: pointer) {.cdecl.}, xdata: pointer, cf: gobject.ConnectFlags): culong =
  g_signal_connect_data(self.impl, "bind", cast[GCallback](p), xdata, nil, cf)

proc scUnbind*(self: SignalGroup;  p: proc (self: ptr gobject.Object00; xdata: pointer) {.cdecl.}, xdata: pointer, cf: gobject.ConnectFlags): culong =
  g_signal_connect_data(self.impl, "unbind", cast[GCallback](p), xdata, nil, cf)

proc g_signal_group_new(targetType: GType): ptr SignalGroup00 {.
    importc, libprag.}

proc newSignalGroup*(targetType: GType): SignalGroup =
  let gobj = g_signal_group_new(targetType)
  let qdata = g_object_get_qdata(gobj, Quark)
  if qdata != nil:
    result = cast[type(result)](qdata)
    assert(result.impl == gobj)
...
proc newSignalGroup*(tdesc: typedesc; targetType: GType): tdesc =
  assert(result is SignalGroup)
  let gobj = g_signal_group_new(targetType)
  let qdata = g_object_get_qdata(gobj, Quark)
  if qdata != nil:
    result = cast[type(result)](qdata)
...
proc initSignalGroup*[T](result: var T; targetType: GType) {.deprecated.} =
  assert(result is SignalGroup)
  let gobj = g_signal_group_new(targetType)
  let qdata = g_object_get_qdata(gobj, Quark)
  if qdata != nil:
...
proc g_signal_group_block(self: ptr SignalGroup00) {.
    importc, libprag.}

proc `block`*(self: SignalGroup) =
  g_signal_group_block(cast[ptr SignalGroup00](self.impl))

proc g_signal_group_connect_closure(self: ptr SignalGroup00; detailedSignal: cstring;
    closure: ptr Closure00; after: gboolean) {.
    importc, libprag.}

proc connectClosure*(self: SignalGroup; detailedSignal: cstring;
    closure: Closure; after: bool) =
  g_signal_group_connect_closure(cast[ptr SignalGroup00](self.impl), detailedSignal, cast[ptr Closure00](closure.impl), gboolean(after))

proc g_signal_group_connect_data(self: ptr SignalGroup00; detailedSignal: cstring;
    cHandler: Callback; data: pointer; notify: ClosureNotify; flags: ConnectFlags) {.
    importc, libprag.}

proc connectData*(self: SignalGroup; detailedSignal: cstring;
    cHandler: Callback; data: pointer; notify: ClosureNotify; flags: ConnectFlags) =
  g_signal_group_connect_data(cast[ptr SignalGroup00](self.impl), detailedSignal, cHandler, data, notify, flags)

proc g_signal_group_connect_swapped(self: ptr SignalGroup00; detailedSignal: cstring;
    cHandler: Callback; data: pointer) {.
    importc, libprag.}

proc connectSwapped*(self: SignalGroup; detailedSignal: cstring;
    cHandler: Callback; data: pointer) =
  g_signal_group_connect_swapped(cast[ptr SignalGroup00](self.impl), detailedSignal, cHandler, data)

proc g_signal_group_dup_target(self: ptr SignalGroup00): ptr Object00 {.
    importc, libprag.}

proc dupTarget*(self: SignalGroup): Object =
  let gobj = g_signal_group_dup_target(cast[ptr SignalGroup00](self.impl))
  if gobj.isNil:
    return nil
  let qdata = g_object_get_qdata(gobj, Quark)
  if qdata != nil:
...
proc g_signal_group_set_target(self: ptr SignalGroup00; target: ptr Object00) {.
    importc, libprag.}

proc setTarget*(self: SignalGroup; target: Object = nil) =
  g_signal_group_set_target(cast[ptr SignalGroup00](self.impl), if target.isNil: nil else: cast[ptr Object00](target.impl))

proc `target=`*(self: SignalGroup; target: Object = nil) =
  g_signal_group_set_target(cast[ptr SignalGroup00](self.impl), if target.isNil: nil else: cast[ptr Object00](target.impl))

proc g_signal_group_unblock(self: ptr SignalGroup00) {.
    importc, libprag.}

proc unblock*(self: SignalGroup) =
  g_signal_group_unblock(cast[ptr SignalGroup00](self.impl))

type
  SignalMatchTypeFlag* {.size: sizeof(cint), pure.} = enum
    id = 0
    detail = 1
    closure = 2
    `func` = 3
...
const TYPE_FUNDAMENTAL_MAX* = 1020'i32
...
type
  ToggleNotify* = proc (data: pointer; obj: ptr Object00; isLastRef: gboolean) {.cdecl.}
...
proc getString*(self: Value): string =
  let resul0 = g_value_get_string(self)
  if resul0.isNil:
    return
  result = $resul0
...
proc g_value_steal_string(self: Value): cstring {.
    importc, libprag.}

proc stealString*(self: Value): string =
  let resul0 = g_value_steal_string(self)
  if resul0.isNil:
    return
  result = $resul0
  cogfree(resul0)
...
type
  ...
  TypeValuePeekPointerFunc* = proc (value: Value): pointer {.cdecl.}
...
type
  TypeValueLCopyFunc* = proc (value: Value; nCollectValues: uint32; collectValues: ptr TypeCValue;
    collectFlags: uint32): cstring {.cdecl.}

type
  TypeValueInitFunc* = proc (value: Value) {.cdecl.}

type
  TypeValueFreeFunc* = proc (value: Value) {.cdecl.}

type
  TypeValueCopyFunc* = proc (srcValue: Value; destValue: var Value) {.cdecl.}

type
  TypeValueCollectFunc* = proc (value: Value; nCollectValues: uint32; collectValues: ptr TypeCValue;
    collectFlags: uint32): cstring {.cdecl.}
...
type
  ObjectSetPropertyFunc* = proc (obj: ptr Object00; propertyId: uint32; value: Value; pspec: ptr ParamSpec00) {.cdecl.}

type
  ObjectGetPropertyFunc* = proc (obj: ptr Object00; propertyId: uint32; value: Value; pspec: ptr ParamSpec00) {.cdecl.}
...
proc boxedTypeRegisterStatic*(name: cstring; boxedCopy: BoxedCopyFunc;
    boxedFree: BoxedFreeFunc): GType {.
    importc: "g_boxed_type_register_static", libprag.}
...
proc g_param_value_is_valid(pspec: ptr ParamSpec00; value: Value): gboolean {.
    importc, libprag.}

proc paramValueIsValid*(pspec: ParamSpec; value: Value): bool =
  toBool(g_param_value_is_valid(cast[ptr ParamSpec00](pspec.impl), value))
...
proc signalGetInvocationHint*(instance: Object): SignalInvocationHint =
  let impl0 = g_signal_get_invocation_hint(cast[ptr Object00](instance.impl))
  if impl0.isNil:
    return nil
  new(result)
  result.impl = impl0
...
proc g_signal_newv(signalName: cstring; itype: GType; signalFlags: SignalFlags;
    classClosure: ptr Closure00; accumulator: SignalAccumulator; accuData: pointer;
    cMarshaller: ClosureMarshal; returnType: GType; nParams: uint32; paramTypes: ptr GType): uint32 {.
    importc, libprag.}

proc signalNewv*(signalName: cstring; itype: GType; signalFlags: SignalFlags;
    classClosure: Closure = nil; accumulator: SignalAccumulator; accuData: pointer;
    cMarshaller: ClosureMarshal; returnType: GType; nParams: int; paramTypes: ptr GType): int =
  int(g_signal_newv(signalName, itype, signalFlags, if classClosure.isNil: nil else: cast[ptr Closure00](classClosure.impl), accumulator, accuData, cMarshaller, returnType, uint32(nParams), paramTypes))
...
proc signalOverrideClassHandler*(signalName: cstring; instanceType: GType;
    ...
    importc: "g_signal_override_class_handler", libprag.}
