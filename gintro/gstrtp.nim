...
const RTCP_REDUCED_SIZE_VALID_MASK* = 49400'i32
...
type
  RTPHeaderExtension* = ref object of gst.Element
  RTPHeaderExtension00* = object of gst.Element00

proc gst_rtp_header_extension_get_type*(): GType {.importc, libprag.}

when defined(gcDestructors):
  proc `=destroy`*(self: var typeof(RTPHeaderExtension()[])) =
...
proc gst_rtp_header_extension_create_from_uri(uri: cstring): ptr RTPHeaderExtension00 {.
    importc, libprag.}

proc createFromUri*(uri: cstring): RTPHeaderExtension =
  let gobj = gst_rtp_header_extension_create_from_uri(uri)
  if gobj.isNil:
    return nil
  ...
  if qdata != nil:
    ...
    fnew(result, gstrtp.finalizeGObject)
    result.impl = gobj
    GC_ref(result)
...
proc gst_rtp_header_extension_get_id(self: ptr RTPHeaderExtension00): uint32 {.
    importc, libprag.}

proc getId*(self: RTPHeaderExtension): int =
  int(gst_rtp_header_extension_get_id(cast[ptr RTPHeaderExtension00](self.impl)))

proc id*(self: RTPHeaderExtension): int =
  int(gst_rtp_header_extension_get_id(cast[ptr RTPHeaderExtension00](self.impl)))

proc gst_rtp_header_extension_get_max_size(self: ptr RTPHeaderExtension00;
    inputMeta: ptr gst.Buffer00): uint64 {.
    importc, libprag.}

proc getMaxSize*(self: RTPHeaderExtension; inputMeta: gst.Buffer): uint64 =
  gst_rtp_header_extension_get_max_size(cast[ptr RTPHeaderExtension00](self.impl), cast[ptr gst.Buffer00](inputMeta.impl))

proc gst_rtp_header_extension_get_sdp_caps_field_name(self: ptr RTPHeaderExtension00): cstring {.
    importc, libprag.}

proc getSdpCapsFieldName*(self: RTPHeaderExtension): string =
  let resul0 = gst_rtp_header_extension_get_sdp_caps_field_name(cast[ptr RTPHeaderExtension00](self.impl))
  result = $resul0
  cogfree(resul0)

proc sdpCapsFieldName*(self: RTPHeaderExtension): string =
  let resul0 = gst_rtp_header_extension_get_sdp_caps_field_name(cast[ptr RTPHeaderExtension00](self.impl))
  result = $resul0
  cogfree(resul0)

proc gst_rtp_header_extension_get_uri(self: ptr RTPHeaderExtension00): cstring {.
    importc, libprag.}

proc getUri*(self: RTPHeaderExtension): string =
  let resul0 = gst_rtp_header_extension_get_uri(cast[ptr RTPHeaderExtension00](self.impl))
  if resul0.isNil:
    return
  result = $resul0

proc uri*(self: RTPHeaderExtension): string =
  let resul0 = gst_rtp_header_extension_get_uri(cast[ptr RTPHeaderExtension00](self.impl))
  if resul0.isNil:
    return
  result = $resul0

proc gst_rtp_header_extension_set_attributes_from_caps(self: ptr RTPHeaderExtension00;
    caps: ptr gst.Caps00): gboolean {.
    importc, libprag.}

proc setAttributesFromCaps*(self: RTPHeaderExtension;
    caps: gst.Caps): bool =
  toBool(gst_rtp_header_extension_set_attributes_from_caps(cast[ptr RTPHeaderExtension00](self.impl), cast[ptr gst.Caps00](caps.impl)))

proc gst_rtp_header_extension_set_caps_from_attributes(self: ptr RTPHeaderExtension00;
    caps: ptr gst.Caps00): gboolean {.
    importc, libprag.}

proc setCapsFromAttributes*(self: RTPHeaderExtension;
    caps: gst.Caps): bool =
  toBool(gst_rtp_header_extension_set_caps_from_attributes(cast[ptr RTPHeaderExtension00](self.impl), cast[ptr gst.Caps00](caps.impl)))

proc gst_rtp_header_extension_set_caps_from_attributes_helper(self: ptr RTPHeaderExtension00;
    caps: ptr gst.Caps00; attributes: cstring): gboolean {.
    importc, libprag.}

proc setCapsFromAttributesHelper*(self: RTPHeaderExtension;
    caps: gst.Caps; attributes: cstring): bool =
  toBool(gst_rtp_header_extension_set_caps_from_attributes_helper(cast[ptr RTPHeaderExtension00](self.impl), cast[ptr gst.Caps00](caps.impl), attributes))

proc gst_rtp_header_extension_set_id(self: ptr RTPHeaderExtension00; extId: uint32) {.
    importc, libprag.}

proc setId*(self: RTPHeaderExtension; extId: int) =
  gst_rtp_header_extension_set_id(cast[ptr RTPHeaderExtension00](self.impl), uint32(extId))

proc `id=`*(self: RTPHeaderExtension; extId: int) =
  gst_rtp_header_extension_set_id(cast[ptr RTPHeaderExtension00](self.impl), uint32(extId))

proc gst_rtp_header_extension_set_non_rtp_sink_caps(self: ptr RTPHeaderExtension00;
    caps: ptr gst.Caps00): gboolean {.
    importc, libprag.}

proc setNonRtpSinkCaps*(self: RTPHeaderExtension;
    caps: gst.Caps): bool =
  toBool(gst_rtp_header_extension_set_non_rtp_sink_caps(cast[ptr RTPHeaderExtension00](self.impl), cast[ptr gst.Caps00](caps.impl)))

proc gst_rtp_header_extension_set_wants_update_non_rtp_src_caps(self: ptr RTPHeaderExtension00;
    state: gboolean) {.
    importc, libprag.}

proc setWantsUpdateNonRtpSrcCaps*(self: RTPHeaderExtension;
    state: bool = true) =
  gst_rtp_header_extension_set_wants_update_non_rtp_src_caps(cast[ptr RTPHeaderExtension00](self.impl), gboolean(state))

proc `wantsUpdateNonRtpSrcCaps=`*(self: RTPHeaderExtension;
    state: bool) =
  gst_rtp_header_extension_set_wants_update_non_rtp_src_caps(cast[ptr RTPHeaderExtension00](self.impl), gboolean(state))

proc gst_rtp_header_extension_update_non_rtp_src_caps(self: ptr RTPHeaderExtension00;
    caps: ptr gst.Caps00): gboolean {.
    importc, libprag.}

proc updateNonRtpSrcCaps*(self: RTPHeaderExtension;
    caps: gst.Caps): bool =
  toBool(gst_rtp_header_extension_update_non_rtp_src_caps(cast[ptr RTPHeaderExtension00](self.impl), cast[ptr gst.Caps00](caps.impl)))

proc gst_rtp_header_extension_wants_update_non_rtp_src_caps(self: ptr RTPHeaderExtension00): gboolean {.
    importc, libprag.}

proc wantsUpdateNonRtpSrcCaps*(self: RTPHeaderExtension): bool =
  toBool(gst_rtp_header_extension_wants_update_non_rtp_src_caps(cast[ptr RTPHeaderExtension00](self.impl)))
...
proc scAddExtension*(self: RTPBasePayload;  p: proc (self: ptr RTPBasePayload00; ext: ptr RTPHeaderExtension00; xdata: pointer) {.cdecl.}, xdata: pointer, cf: gobject.ConnectFlags): culong =
  g_signal_connect_data(self.impl, "add-extension", cast[GCallback](p), xdata, nil, cf)

proc scClearExtensions*(self: RTPBasePayload;  p: proc (self: ptr gobject.Object00; xdata: pointer) {.cdecl.}, xdata: pointer, cf: gobject.ConnectFlags): culong =
  g_signal_connect_data(self.impl, "clear-extensions", cast[GCallback](p), xdata, nil, cf)

proc scRequestExtension*(self: RTPBasePayload;  p: proc (self: ptr RTPBasePayload00; extId: uint32; extUri: cstring; xdata: pointer): ptr RTPHeaderExtension00 {.cdecl.}, xdata: pointer, cf: gobject.ConnectFlags): culong =
  g_signal_connect_data(self.impl, "request-extension", cast[GCallback](p), xdata, nil, cf)
...
proc push*(self: RTPBasePayload; buffer: gst.Buffer): gst.FlowReturn =
  gst_rtp_base_payload_push(cast[ptr RTPBasePayload00](self.impl), cast[ptr gst.Buffer00](g_boxed_copy(gst_buffer_get_type(), buffer.impl)))
...
proc pushList*(self: RTPBasePayload; list: gst.BufferList): gst.FlowReturn =
  gst_rtp_base_payload_push_list(cast[ptr RTPBasePayload00](self.impl), cast[ptr gst.BufferList00](g_boxed_copy(gst_buffer_list_get_type(), list.impl)))
...
proc gst_rtp_base_payload_set_outcaps_structure(self: ptr RTPBasePayload00;
    s: ptr gst.Structure00): gboolean {.
    importc, libprag.}

proc setOutcapsStructure*(self: RTPBasePayload; s: gst.Structure = nil): bool =
  toBool(gst_rtp_base_payload_set_outcaps_structure(cast[ptr RTPBasePayload00](self.impl), if s.isNil: nil else: cast[ptr gst.Structure00](s.impl)))
...
type
  RTPHeaderExtensionDirection* {.size: sizeof(cint), pure.} = enum
    inactive = 0
    sendonly = 1
    recvonly = 2
    sendrecv = 3
...
proc gst_rtp_header_extension_get_direction(self: ptr RTPHeaderExtension00): RTPHeaderExtensionDirection {.
    importc, libprag.}

proc getDirection*(self: RTPHeaderExtension): RTPHeaderExtensionDirection =
  gst_rtp_header_extension_get_direction(cast[ptr RTPHeaderExtension00](self.impl))

proc direction*(self: RTPHeaderExtension): RTPHeaderExtensionDirection =
  gst_rtp_header_extension_get_direction(cast[ptr RTPHeaderExtension00](self.impl))

proc gst_rtp_header_extension_set_direction(self: ptr RTPHeaderExtension00;
    direction: RTPHeaderExtensionDirection) {.
    importc, libprag.}

proc setDirection*(self: RTPHeaderExtension; direction: RTPHeaderExtensionDirection) =
  gst_rtp_header_extension_set_direction(cast[ptr RTPHeaderExtension00](self.impl), direction)

proc `direction=`*(self: RTPHeaderExtension; direction: RTPHeaderExtensionDirection) =
  gst_rtp_header_extension_set_direction(cast[ptr RTPHeaderExtension00](self.impl), direction)

type
  RTPHeaderExtensionFlag* {.size: sizeof(cint), pure.} = enum
    oneByte = 0
    twoByte = 1
...
proc getSupportedFlags*(self: RTPHeaderExtension): RTPHeaderExtensionFlags =
  gst_rtp_header_extension_get_supported_flags(cast[ptr RTPHeaderExtension00](self.impl))

proc supportedFlags*(self: RTPHeaderExtension): RTPHeaderExtensionFlags =
  gst_rtp_header_extension_get_supported_flags(cast[ptr RTPHeaderExtension00](self.impl))

proc gst_rtp_header_extension_read(self: ptr RTPHeaderExtension00; readFlags: RTPHeaderExtensionFlags;
    data: ptr uint8; size: uint64; buffer: ptr gst.Buffer00): gboolean {.
    importc, libprag.}

proc read*(self: RTPHeaderExtension; readFlags: RTPHeaderExtensionFlags;
    data: seq[uint8] | string; buffer: gst.Buffer): bool =
  let size = uint64(data.len)
  toBool(gst_rtp_header_extension_read(cast[ptr RTPHeaderExtension00](self.impl), readFlags, cast[ptr uint8](unsafeaddr(data[0])), size, cast[ptr gst.Buffer00](buffer.impl)))

proc gst_rtp_header_extension_write(self: ptr RTPHeaderExtension00; inputMeta: ptr gst.Buffer00;
    writeFlags: RTPHeaderExtensionFlags; output: ptr gst.Buffer00; data: ptr uint8;
    size: uint64): int64 {.
    importc, libprag.}

proc write*(self: RTPHeaderExtension; inputMeta: gst.Buffer;
    writeFlags: RTPHeaderExtensionFlags; output: gst.Buffer; data: seq[uint8] | string): int64 =
  let size = uint64(data.len)
  gst_rtp_header_extension_write(cast[ptr RTPHeaderExtension00](self.impl), cast[ptr gst.Buffer00](inputMeta.impl), writeFlags, cast[ptr gst.Buffer00](output.impl), cast[ptr uint8](unsafeaddr(data[0])), size)
...
proc scAddExtension*(self: RTPBaseDepayload;  p: proc (self: ptr RTPBaseDepayload00; ext: ptr RTPHeaderExtension00; xdata: pointer) {.cdecl.}, xdata: pointer, cf: gobject.ConnectFlags): culong =
  g_signal_connect_data(self.impl, "add-extension", cast[GCallback](p), xdata, nil, cf)

proc scClearExtensions*(self: RTPBaseDepayload;  p: proc (self: ptr gobject.Object00; xdata: pointer) {.cdecl.}, xdata: pointer, cf: gobject.ConnectFlags): culong =
  g_signal_connect_data(self.impl, "clear-extensions", cast[GCallback](p), xdata, nil, cf)

proc scRequestExtension*(self: RTPBaseDepayload;  p: proc (self: ptr RTPBaseDepayload00; extId: uint32; extUri: cstring; xdata: pointer): ptr RTPHeaderExtension00 {.cdecl.}, xdata: pointer, cf: gobject.ConnectFlags): culong =
  g_signal_connect_data(self.impl, "request-extension", cast[GCallback](p), xdata, nil, cf)

proc gst_rtp_base_depayload_delayed(self: ptr RTPBaseDepayload00) {.
    importc, libprag.}

proc delayed*(self: RTPBaseDepayload) =
  gst_rtp_base_depayload_delayed(cast[ptr RTPBaseDepayload00](self.impl))

proc gst_rtp_base_depayload_dropped(self: ptr RTPBaseDepayload00) {.
    importc, libprag.}

proc dropped*(self: RTPBaseDepayload) =
  gst_rtp_base_depayload_dropped(cast[ptr RTPBaseDepayload00](self.impl))

proc gst_rtp_base_depayload_flush(self: ptr RTPBaseDepayload00; keepCurrent: gboolean) {.
    importc, libprag.}

proc flush*(self: RTPBaseDepayload; keepCurrent: bool) =
  gst_rtp_base_depayload_flush(cast[ptr RTPBaseDepayload00](self.impl), gboolean(keepCurrent))

proc gst_rtp_base_depayload_is_aggregate_hdrext_enabled(self: ptr RTPBaseDepayload00): gboolean {.
    importc, libprag.}

proc isAggregateHdrextEnabled*(self: RTPBaseDepayload): bool =
  toBool(gst_rtp_base_depayload_is_aggregate_hdrext_enabled(cast[ptr RTPBaseDepayload00](self.impl)))
...
proc push*(self: RTPBaseDepayload; outBuf: gst.Buffer): gst.FlowReturn =
  gst_rtp_base_depayload_push(cast[ptr RTPBaseDepayload00](self.impl), cast[ptr gst.Buffer00](g_boxed_copy(gst_buffer_get_type(), outBuf.impl)))
...
proc pushList*(self: RTPBaseDepayload; outList: gst.BufferList): gst.FlowReturn =
  gst_rtp_base_depayload_push_list(cast[ptr RTPBaseDepayload00](self.impl), cast[ptr gst.BufferList00](g_boxed_copy(gst_buffer_list_get_type(), outList.impl)))

proc gst_rtp_base_depayload_set_aggregate_hdrext_enabled(self: ptr RTPBaseDepayload00;
    enable: gboolean) {.
    importc, libprag.}

proc setAggregateHdrextEnabled*(self: RTPBaseDepayload;
    enable: bool = true) =
  gst_rtp_base_depayload_set_aggregate_hdrext_enabled(cast[ptr RTPBaseDepayload00](self.impl), gboolean(enable))

proc `aggregateHdrextEnabled=`*(self: RTPBaseDepayload;
    enable: bool) =
  gst_rtp_base_depayload_set_aggregate_hdrext_enabled(cast[ptr RTPBaseDepayload00](self.impl), gboolean(enable))
...
proc getExtensionData*(self: RTPBuffer; bits: var uint16): glib.Bytes =
  let impl0 = gst_rtp_buffer_get_extension_bytes(self, bits)
  if impl0.isNil:
    return nil
...
proc getExtensionOnebyteHeader*(self: RTPBuffer; id: uint8;
    nth: int; data: var seq[uint8] = cast[var seq[uint8]](nil); size: var int = cast[var int](nil)): bool =
  var data_00: ptr uint8
  var size_00: uint32
...
proc getExtensionTwobytesHeader*(self: RTPBuffer; appbits: var uint8 = cast[var uint8](nil);
    id: uint8; nth: int; data: var seq[uint8] = cast[var seq[uint8]](nil);
    size: var int = cast[var int](nil)): bool =
  var data_00: ptr uint8
  var size_00: uint32
...
proc getPayload*(self: RTPBuffer): glib.Bytes =
  let impl0 = gst_rtp_buffer_get_payload_bytes(self)
  if impl0.isNil:
    return nil
  fnew(result, gBoxedFreeGBytes)
  result.impl = impl0
...
proc gst_rtp_buffer_remove_extension_data(self: RTPBuffer) {.
    importc, libprag.}

proc removeExtensionData*(self: RTPBuffer) =
  gst_rtp_buffer_remove_extension_data(self)
...
proc forName*(media: cstring; encodingName: cstring): RTPPayloadInfo =
  let impl0 = gst_rtp_payload_info_for_name(media, encodingName)
  if impl0.isNil:
    return nil
  new(result)
  result.impl = impl0
...
proc forPt*(payloadType: uint8): RTPPayloadInfo =
  let impl0 = gst_rtp_payload_info_for_pt(payloadType)
  if impl0.isNil:
    return nil
  new(result)
  result.impl = impl0
...
proc appendCsrc*(self: RTPSourceMeta; csrc: seq[uint32]): bool =
  let csrcCount = int(csrc.len)
  toBool(gst_rtp_source_meta_append_csrc(cast[ptr RTPSourceMeta00](self.impl), cast[ptr uint32](unsafeaddr(csrc[0])), uint32(csrcCount)))
...
proc bufferAddRtpSourceMeta*(buffer: gst.Buffer; ssrc: ptr uint32;
    csrc: seq[uint32]): RTPSourceMeta =
  let csrcCount = int(csrc.len)
  new(result)
  result.impl = gst_buffer_add_rtp_source_meta(cast[ptr gst.Buffer00](buffer.impl), ssrc, cast[ptr uint32](unsafeaddr(csrc[0])), uint32(csrcCount))
  result.ignoreFinalizer = true
...
proc bufferGetRtpSourceMeta*(buffer: gst.Buffer): RTPSourceMeta =
  let impl0 = gst_buffer_get_rtp_source_meta(cast[ptr gst.Buffer00](buffer.impl))
  if impl0.isNil:
    return nil
  new(result)
  result.impl = impl0
...
proc gst_rtp_get_header_extension_list(): ptr glib.List {.
    importc, libprag.}

proc rtpGetHeaderExtensionList*(): seq[gst.ElementFactory] =
  let resul0 = gst_rtp_get_header_extension_list()
  result = glistObjects2seq(gst.ElementFactory, resul0, true)
  g_list_free(resul0)
