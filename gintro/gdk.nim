...
import freetype2, glib, gdkpixbuf, harfbuzz, gmodule, cairo, gobject, pango, gio
...
proc scCreateSurface*(self: Window;  p: proc (self: ptr Window00; width: int32; height: int32; xdata: pointer): ptr cairo.Surface00 {.cdecl.}, xdata: pointer, cf: gobject.ConnectFlags): culong =
...
proc scPickEmbeddedChild*(self: Window;  p: proc (self: ptr Window00; x: cdouble; y: cdouble; xdata: pointer): ptr Window00 {.cdecl.}, xdata: pointer, cf: gobject.ConnectFlags): culong =
...
proc gdk_window_create_similar_image_surface(self: ptr Window00; format: cairo.Format;
    width: int32; height: int32; scale: int32): ptr cairo.Surface00 {.
    importc, libprag.}

proc createSimilarImageSurface*(self: Window; format: cairo.Format;
    width: int; height: int; scale: int): cairo.Surface =
  fnew(result, gBoxedFreeCairoSurface)
  result.impl = gdk_window_create_similar_image_surface(cast[ptr Window00](self.impl), format, int32(width), int32(height), int32(scale))
...
type
  WindowAttributesTypeFlag* {.size: sizeof(cint), pure.} = enum
    ignoreThisDummyValue = 0
    title = 1
    x = 2
    y = 3
...
type
  WindowStateFlag* {.size: sizeof(cint), pure.} = enum
    withdrawn = 0
    iconified = 1
    maximized = 2
    sticky = 3
...
type
  WMDecorationFlag* {.size: sizeof(cint), pure.} = enum
    all = 0
    border = 1
    resizeh = 2
    title = 3
...
type
  WMFunctionFlag* {.size: sizeof(cint), pure.} = enum
    all = 0
    resize = 1
    move = 2
    minimize = 3
