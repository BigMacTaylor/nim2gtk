...
type
  AncillaryMeta00* {.pure.} = object
  AncillaryMeta* = ref object
    impl*: ptr AncillaryMeta00
    ignoreFinalizer*: bool

proc gst_ancillary_meta_get_info(): ptr gst.MetaInfo00 {.
    importc, libprag.}

proc getInfoAncillaryMeta*(): gst.MetaInfo =
  new(result)
  result.impl = gst_ancillary_meta_get_info()
  result.ignoreFinalizer = true

type
  AncillaryMetaField* {.size: sizeof(cint), pure.} = enum
    progressive = 0
    interlacedFirst = 16
    interlacedSecond = 17
...
proc gst_navigation_event_get_coordinates(event: ptr gst.Event00; x: var cdouble;
    y: var cdouble): gboolean {.
    importc, libprag.}

proc eventGetCoordinates*(event: gst.Event; x: var cdouble = cast[var cdouble](nil);
    y: var cdouble = cast[var cdouble](nil)): bool =
  toBool(gst_navigation_event_get_coordinates(cast[ptr gst.Event00](event.impl), x, y))
...
proc gst_navigation_event_parse_touch_event(event: ptr gst.Event00; identifier: var uint32;
    x: var cdouble; y: var cdouble; pressure: var cdouble): gboolean {.
    importc, libprag.}

proc eventParseTouchEvent*(event: gst.Event; identifier: var int = cast[var int](nil);
    x: var cdouble = cast[var cdouble](nil); y: var cdouble = cast[var cdouble](nil);
    pressure: var cdouble = cast[var cdouble](nil)): bool =
  var identifier_00: uint32
  result = toBool(gst_navigation_event_parse_touch_event(cast[ptr gst.Event00](event.impl), identifier_00, x, y, pressure))
  if identifier.addr != nil:
    identifier = int(identifier_00)

proc gst_navigation_event_parse_touch_up_event(event: ptr gst.Event00; identifier: var uint32;
    x: var cdouble; y: var cdouble): gboolean {.
    importc, libprag.}

proc eventParseTouchUpEvent*(event: gst.Event; identifier: var int = cast[var int](nil);
    x: var cdouble = cast[var cdouble](nil); y: var cdouble = cast[var cdouble](nil)): bool =
  var identifier_00: uint32
  result = toBool(gst_navigation_event_parse_touch_up_event(cast[ptr gst.Event00](event.impl), identifier_00, x, y))
  if identifier.addr != nil:
    identifier = int(identifier_00)

proc gst_navigation_event_set_coordinates(event: ptr gst.Event00; x: cdouble;
    y: cdouble): gboolean {.
    importc, libprag.}

proc eventSetCoordinates*(event: gst.Event; x: cdouble; y: cdouble): bool =
  toBool(gst_navigation_event_set_coordinates(cast[ptr gst.Event00](event.impl), x, y))
...
proc gst_navigation_send_event_simple(self: ptr Navigation00; event: ptr gst.Event00) {.
    importc, libprag.}

proc sendEventSimple*(self: Navigation; event: gst.Event) =
  gst_navigation_send_event_simple(cast[ptr Navigation00](self.impl), cast[ptr gst.Event00](g_boxed_copy(gst_event_get_type(), event.impl)))
...
type
  NavigationModifierType* {.size: sizeof(cint), pure.} = enum
    none = 0
    shiftMask = 1
    lockMask = 2
    controlMask = 4
...
proc gst_navigation_event_new_key_press(key: cstring; state: NavigationModifierType): ptr gst.Event00 {.
    importc, libprag.}

proc eventNewKeyPress*(key: cstring; state: NavigationModifierType): gst.Event =
  fnew(result, gBoxedFreeGstEvent)
  result.impl = gst_navigation_event_new_key_press(key, state)

proc gst_navigation_event_new_key_release(key: cstring; state: NavigationModifierType): ptr gst.Event00 {.
    importc, libprag.}

proc eventNewKeyRelease*(key: cstring; state: NavigationModifierType): gst.Event =
  fnew(result, gBoxedFreeGstEvent)
  result.impl = gst_navigation_event_new_key_release(key, state)

proc gst_navigation_event_new_mouse_button_press(button: int32; x: cdouble;
    y: cdouble; state: NavigationModifierType): ptr gst.Event00 {.
    importc, libprag.}

proc eventNewMouseButtonPress*(button: int; x: cdouble;
    y: cdouble; state: NavigationModifierType): gst.Event =
  fnew(result, gBoxedFreeGstEvent)
  result.impl = gst_navigation_event_new_mouse_button_press(int32(button), x, y, state)

proc gst_navigation_event_new_mouse_button_release(button: int32; x: cdouble;
    y: cdouble; state: NavigationModifierType): ptr gst.Event00 {.
    importc, libprag.}

proc eventNewMouseButtonRelease*(button: int; x: cdouble;
    y: cdouble; state: NavigationModifierType): gst.Event =
  fnew(result, gBoxedFreeGstEvent)
  result.impl = gst_navigation_event_new_mouse_button_release(int32(button), x, y, state)

proc gst_navigation_event_new_mouse_move(x: cdouble; y: cdouble; state: NavigationModifierType): ptr gst.Event00 {.
    importc, libprag.}

proc eventNewMouseMove*(x: cdouble; y: cdouble; state: NavigationModifierType): gst.Event =
  fnew(result, gBoxedFreeGstEvent)
  result.impl = gst_navigation_event_new_mouse_move(x, y, state)

proc gst_navigation_event_new_mouse_scroll(x: cdouble; y: cdouble; deltaX: cdouble;
    deltaY: cdouble; state: NavigationModifierType): ptr gst.Event00 {.
    importc, libprag.}

proc eventNewMouseScroll*(x: cdouble; y: cdouble; deltaX: cdouble;
    deltaY: cdouble; state: NavigationModifierType): gst.Event =
  fnew(result, gBoxedFreeGstEvent)
  result.impl = gst_navigation_event_new_mouse_scroll(x, y, deltaX, deltaY, state)

proc gst_navigation_event_new_touch_cancel(state: NavigationModifierType): ptr gst.Event00 {.
    importc, libprag.}

proc eventNewTouchCancel*(state: NavigationModifierType): gst.Event =
  fnew(result, gBoxedFreeGstEvent)
  result.impl = gst_navigation_event_new_touch_cancel(state)

proc gst_navigation_event_new_touch_down(identifier: uint32; x: cdouble;
    y: cdouble; pressure: cdouble; state: NavigationModifierType): ptr gst.Event00 {.
    importc, libprag.}

proc eventNewTouchDown*(identifier: int; x: cdouble; y: cdouble;
    pressure: cdouble; state: NavigationModifierType): gst.Event =
  fnew(result, gBoxedFreeGstEvent)
  result.impl = gst_navigation_event_new_touch_down(uint32(identifier), x, y, pressure, state)

proc gst_navigation_event_new_touch_frame(state: NavigationModifierType): ptr gst.Event00 {.
    importc, libprag.}

proc eventNewTouchFrame*(state: NavigationModifierType): gst.Event =
  fnew(result, gBoxedFreeGstEvent)
  result.impl = gst_navigation_event_new_touch_frame(state)

proc gst_navigation_event_new_touch_motion(identifier: uint32; x: cdouble;
    y: cdouble; pressure: cdouble; state: NavigationModifierType): ptr gst.Event00 {.
    importc, libprag.}

proc eventNewTouchMotion*(identifier: int; x: cdouble; y: cdouble;
    pressure: cdouble; state: NavigationModifierType): gst.Event =
  fnew(result, gBoxedFreeGstEvent)
  result.impl = gst_navigation_event_new_touch_motion(uint32(identifier), x, y, pressure, state)

proc gst_navigation_event_new_touch_up(identifier: uint32; x: cdouble; y: cdouble;
    state: NavigationModifierType): ptr gst.Event00 {.
    importc, libprag.}

proc eventNewTouchUp*(identifier: int; x: cdouble; y: cdouble;
    state: NavigationModifierType): gst.Event =
  fnew(result, gBoxedFreeGstEvent)
  result.impl = gst_navigation_event_new_touch_up(uint32(identifier), x, y, state)

proc gst_navigation_event_parse_modifier_state(event: ptr gst.Event00; state: ptr NavigationModifierType): gboolean {.
    importc, libprag.}

proc eventParseModifierState*(event: gst.Event; state: ptr NavigationModifierType): bool =
  toBool(gst_navigation_event_parse_modifier_state(cast[ptr gst.Event00](event.impl), state))
...
proc gst_navigation_event_new_command(command: NavigationCommand): ptr gst.Event00 {.
    importc, libprag.}

proc eventNewCommand*(command: NavigationCommand): gst.Event =
  fnew(result, gBoxedFreeGstEvent)
  result.impl = gst_navigation_event_new_command(command)
...
const VIDEO_DECODER_MAX_ERRORS* = -1'i32
...
proc gst_video_aggregator_get_execution_task_pool(self: ptr VideoAggregator00): ptr gst.TaskPool00 {.
    importc, libprag.}

proc getExecutionTaskPool*(self: VideoAggregator): gst.TaskPool =
  let gobj = gst_video_aggregator_get_execution_task_pool(cast[ptr VideoAggregator00](self.impl))
...
proc executionTaskPool*(self: VideoAggregator): gst.TaskPool =
  let gobj = gst_video_aggregator_get_execution_task_pool(cast[ptr VideoAggregator00](self.impl))
  let qdata = g_object_get_qdata(gobj, Quark)
  if qdata != nil:
    result = cast[type(result)](qdata)
...
proc gst_video_info_new_from_caps(caps: ptr gst.Caps00): ptr VideoInfo00 {.
    importc, libprag.}

proc newVideoInfoFromCaps*(caps: gst.Caps): VideoInfo =
  let impl0 = gst_video_info_new_from_caps(cast[ptr gst.Caps00](caps.impl))
  if impl0.isNil:
    return nil
  fnew(result, gBoxedFreeGstVideoInfo)
  result.impl = impl0

proc newVideoInfoFromCaps*(tdesc: typedesc; caps: gst.Caps): tdesc =
...
proc initVideoInfoFromCaps*[T](result: var T; caps: gst.Caps) {.deprecated.} =
  assert(result is VideoInfo)
  let impl0 = gst_video_info_new_from_caps(cast[ptr gst.Caps00](caps.impl))
  if impl0.isNil:
    return nil
...
proc gst_video_info_from_caps(info: var VideoInfo00; caps: ptr gst.Caps00): gboolean {.
    importc, libprag.}

proc fromCaps*(info: var VideoInfo00; caps: gst.Caps): bool =
  toBool(gst_video_info_from_caps(info, cast[ptr gst.Caps00](caps.impl)))

proc gst_video_info_init(info: var VideoInfo00) {.
    importc, libprag.}

proc init*(info: var VideoInfo00) =
...
proc gst_video_frame_map(frame: var VideoFrame; info: ptr VideoInfo00; buffer: ptr gst.Buffer00;
    flags: gst.MapFlags): gboolean {.
    importc, libprag.}

proc map*(frame: var VideoFrame; info: VideoInfo; buffer: gst.Buffer;
    flags: gst.MapFlags): bool =
  toBool(gst_video_frame_map(frame, cast[ptr VideoInfo00](info.impl), cast[ptr gst.Buffer00](buffer.impl), flags))

proc gst_video_frame_map_id(frame: var VideoFrame; info: ptr VideoInfo00;
    buffer: ptr gst.Buffer00; id: int32; flags: gst.MapFlags): gboolean {.
    importc, libprag.}

proc mapId*(frame: var VideoFrame; info: VideoInfo; buffer: gst.Buffer;
    id: int; flags: gst.MapFlags): bool =
  toBool(gst_video_frame_map_id(frame, cast[ptr VideoInfo00](info.impl), cast[ptr gst.Buffer00](buffer.impl), int32(id), flags))
...
type
  ...
  VideoAggregatorParallelConvertPad00* = object of VideoAggregatorConvertPad00

proc gst_video_aggregator_parallel_convert_pad_get_type*(): GType {.importc, libprag.}

when defined(gcDestructors):
  proc `=destroy`*(self: var typeof(VideoAggregatorParallelConvertPad()[])) =
    when defined(gintroDebug):
      echo "destroy ", $typeof(self), ' ', cast[int](unsafeaddr self)
    g_object_set_qdata(self.impl, Quark, nil)
    if not self.ignoreFinalizer and self.impl != nil:
...
proc gst_video_chroma_resample_get_info(self: ptr VideoChromaResample00;
    nLines: var uint32; offset: var int32) {.
    importc, libprag.}
...
proc gst_video_chroma_site_to_string(site: VideoChromaSite): cstring {.
    importc, libprag.}

proc toString*(site: VideoChromaSite): string =
  let resul0 = gst_video_chroma_site_to_string(site)
  if resul0.isNil:
    return
  result = $resul0
  cogfree(resul0)
...
type
  VideoCodecAlphaMeta00* {.pure.} = object
  VideoCodecAlphaMeta* = ref object
    impl*: ptr VideoCodecAlphaMeta00
    ignoreFinalizer*: bool

proc gst_video_codec_alpha_meta_get_info(): ptr gst.MetaInfo00 {.
    importc, libprag.}

proc getInfoVideoCodecAlphaMeta*(): gst.MetaInfo =
  new(result)
  result.impl = gst_video_codec_alpha_meta_get_info()
  result.ignoreFinalizer = true
...
proc gst_video_color_primaries_is_equivalent(primaries: VideoColorPrimaries;
    other: VideoColorPrimaries): gboolean {.
    importc, libprag.}

proc isEquivalent*(primaries: VideoColorPrimaries;
    other: VideoColorPrimaries): bool =
  toBool(gst_video_color_primaries_is_equivalent(primaries, other))
...
proc decode*(`func`: VideoTransferFunction; val: cdouble): cdouble {.
    importc: "gst_video_transfer_function_decode", libprag.}

proc encode*(`func`: VideoTransferFunction; val: cdouble): cdouble {.
    importc: "gst_video_transfer_function_encode", libprag.}
...
proc gst_video_colorimetry_is_equivalent(self: VideoColorimetry; bitdepth: uint32;
    other: VideoColorimetry; otherBitdepth: uint32): gboolean {.
    importc, libprag.}

proc isEquivalent*(self: VideoColorimetry; bitdepth: int;
    other: VideoColorimetry; otherBitdepth: int): bool =
  toBool(gst_video_colorimetry_is_equivalent(self, uint32(bitdepth), other, uint32(otherBitdepth)))
...
proc gst_video_content_light_level_is_equal(self: VideoContentLightLevel;
    other: VideoContentLightLevel): gboolean {.
    importc, libprag.}

proc isEqual*(self: VideoContentLightLevel;
    other: VideoContentLightLevel): bool =
  toBool(gst_video_content_light_level_is_equal(self, other))
...
proc gst_video_converter_frame_finish(self: ptr VideoConverter00) {.
    importc, libprag.}

proc frameFinish*(self: VideoConverter) =
  gst_video_converter_frame_finish(cast[ptr VideoConverter00](self.impl))
...
proc gst_video_converter_get_in_info(self: ptr VideoConverter00): ptr VideoInfo00 {.
    importc, libprag.}

proc getInInfo*(self: VideoConverter): VideoInfo =
  fnew(result, gBoxedFreeGstVideoInfo)
  result.impl = gst_video_converter_get_in_info(cast[ptr VideoConverter00](self.impl))
  result.impl = cast[typeof(result.impl)](g_boxed_copy(gst_video_info_get_type(), result.impl))

proc inInfo*(self: VideoConverter): VideoInfo =
  fnew(result, gBoxedFreeGstVideoInfo)
  result.impl = gst_video_converter_get_in_info(cast[ptr VideoConverter00](self.impl))
  result.impl = cast[typeof(result.impl)](g_boxed_copy(gst_video_info_get_type(), result.impl))

proc gst_video_converter_get_out_info(self: ptr VideoConverter00): ptr VideoInfo00 {.
    importc, libprag.}

proc getOutInfo*(self: VideoConverter): VideoInfo =
  fnew(result, gBoxedFreeGstVideoInfo)
  result.impl = gst_video_converter_get_out_info(cast[ptr VideoConverter00](self.impl))
  result.impl = cast[typeof(result.impl)](g_boxed_copy(gst_video_info_get_type(), result.impl))

proc outInfo*(self: VideoConverter): VideoInfo =
  fnew(result, gBoxedFreeGstVideoInfo)
  result.impl = gst_video_converter_get_out_info(cast[ptr VideoConverter00](self.impl))
  result.impl = cast[typeof(result.impl)](g_boxed_copy(gst_video_info_get_type(), result.impl))
...
proc allocateOutputBuffer*(self: VideoDecoder): gst.Buffer =
  let impl0 = gst_video_decoder_allocate_output_buffer(cast[ptr VideoDecoder00](self.impl))
  if impl0.isNil:
    return nil
  fnew(result, gBoxedFreeGstBuffer)
  result.impl = impl0
...
proc gst_video_decoder_drop_subframe(self: ptr VideoDecoder00; frame: ptr VideoCodecFrame00): gst.FlowReturn {.
    importc, libprag.}

proc dropSubframe*(self: VideoDecoder; frame: VideoCodecFrame): gst.FlowReturn =
  gst_video_decoder_drop_subframe(cast[ptr VideoDecoder00](self.impl), cast[ptr VideoCodecFrame00](g_boxed_copy(gst_video_codec_frame_get_type(), frame.impl)))
...
proc gst_video_decoder_finish_subframe(self: ptr VideoDecoder00; frame: ptr VideoCodecFrame00): gst.FlowReturn {.
    importc, libprag.}

proc finishSubframe*(self: VideoDecoder; frame: VideoCodecFrame): gst.FlowReturn =
  gst_video_decoder_finish_subframe(cast[ptr VideoDecoder00](self.impl), cast[ptr VideoCodecFrame00](g_boxed_copy(gst_video_codec_frame_get_type(), frame.impl)))
...
proc getFrame*(self: VideoDecoder; frameNumber: int): VideoCodecFrame =
  let impl0 = gst_video_decoder_get_frame(cast[ptr VideoDecoder00](self.impl), int32(frameNumber))
  if impl0.isNil:
    return nil
  fnew(result, gBoxedFreeGstVideoCodecFrame)
  result.impl = impl0
...
proc gst_video_decoder_get_input_subframe_index(self: ptr VideoDecoder00;
    frame: ptr VideoCodecFrame00): uint32 {.
    importc, libprag.}

proc getInputSubframeIndex*(self: VideoDecoder; frame: VideoCodecFrame): int =
  int(gst_video_decoder_get_input_subframe_index(cast[ptr VideoDecoder00](self.impl), cast[ptr VideoCodecFrame00](frame.impl)))
...
proc gst_video_decoder_get_needs_sync_point(self: ptr VideoDecoder00): gboolean {.
    importc, libprag.}

proc getNeedsSyncPoint*(self: VideoDecoder): bool =
  toBool(gst_video_decoder_get_needs_sync_point(cast[ptr VideoDecoder00](self.impl)))

proc needsSyncPoint*(self: VideoDecoder): bool =
  toBool(gst_video_decoder_get_needs_sync_point(cast[ptr VideoDecoder00](self.impl)))
...
proc getOldestFrame*(self: VideoDecoder): VideoCodecFrame =
  let impl0 = gst_video_decoder_get_oldest_frame(cast[ptr VideoDecoder00](self.impl))
  if impl0.isNil:
    return nil
  fnew(result, gBoxedFreeGstVideoCodecFrame)
  result.impl = impl0
...
proc getOutputState*(self: VideoDecoder): VideoCodecState =
  let impl0 = gst_video_decoder_get_output_state(cast[ptr VideoDecoder00](self.impl))
  if impl0.isNil:
    return nil
  fnew(result, gBoxedFreeGstVideoCodecState)
  result.impl = impl0
...
proc gst_video_decoder_get_processed_subframe_index(self: ptr VideoDecoder00;
    frame: ptr VideoCodecFrame00): uint32 {.
    importc, libprag.}

proc getProcessedSubframeIndex*(self: VideoDecoder; frame: VideoCodecFrame): int =
  int(gst_video_decoder_get_processed_subframe_index(cast[ptr VideoDecoder00](self.impl), cast[ptr VideoCodecFrame00](frame.impl)))
...
proc gst_video_decoder_get_subframe_mode(self: ptr VideoDecoder00): gboolean {.
    importc, libprag.}

proc getSubframeMode*(self: VideoDecoder): bool =
  toBool(gst_video_decoder_get_subframe_mode(cast[ptr VideoDecoder00](self.impl)))

proc subframeMode*(self: VideoDecoder): bool =
  toBool(gst_video_decoder_get_subframe_mode(cast[ptr VideoDecoder00](self.impl)))
...
proc gst_video_decoder_have_last_subframe(self: ptr VideoDecoder00; frame: ptr VideoCodecFrame00): gst.FlowReturn {.
    importc, libprag.}

proc haveLastSubframe*(self: VideoDecoder; frame: VideoCodecFrame): gst.FlowReturn =
  gst_video_decoder_have_last_subframe(cast[ptr VideoDecoder00](self.impl), cast[ptr VideoCodecFrame00](frame.impl))
...
proc setInterlacedOutputState*(self: VideoDecoder; fmt: VideoFormat;
  ...
  let impl0 = gst_video_decoder_set_interlaced_output_state(cast[ptr VideoDecoder00](self.impl), fmt, interlaceMode, uint32(width), uint32(height), if reference.isNil: nil else: cast[ptr VideoCodecState00](reference.impl))
  if impl0.isNil:
    return nil
  fnew(result, gBoxedFreeGstVideoCodecState)
  result.impl = impl0
...
proc gst_video_decoder_set_needs_sync_point(self: ptr VideoDecoder00; enabled: gboolean) {.
    importc, libprag.}

proc setNeedsSyncPoint*(self: VideoDecoder; enabled: bool = true) =
  gst_video_decoder_set_needs_sync_point(cast[ptr VideoDecoder00](self.impl), gboolean(enabled))

proc `needsSyncPoint=`*(self: VideoDecoder; enabled: bool) =
  gst_video_decoder_set_needs_sync_point(cast[ptr VideoDecoder00](self.impl), gboolean(enabled))
...
proc setOutputState*(self: VideoDecoder; fmt: VideoFormat;
  ...
  let impl0 = gst_video_decoder_set_output_state(cast[ptr VideoDecoder00](self.impl), fmt, uint32(width), uint32(height), if reference.isNil: nil else: cast[ptr VideoCodecState00](reference.impl))
  if impl0.isNil:
    return nil
  fnew(result, gBoxedFreeGstVideoCodecState)
  result.impl = impl0
...
proc gst_video_decoder_set_subframe_mode(self: ptr VideoDecoder00; subframeMode: gboolean) {.
    importc, libprag.}

proc setSubframeMode*(self: VideoDecoder; subframeMode: bool = true) =
  gst_video_decoder_set_subframe_mode(cast[ptr VideoDecoder00](self.impl), gboolean(subframeMode))

proc `subframeMode=`*(self: VideoDecoder; subframeMode: bool) =
  gst_video_decoder_set_subframe_mode(cast[ptr VideoDecoder00](self.impl), gboolean(subframeMode))
...
type
  VideoDecoderRequestSyncPointFlag* {.size: sizeof(cint), pure.} = enum
    discardInput = 0
    corruptOutput = 1
...
proc requestSyncPoint*(self: VideoDecoder; frame: VideoCodecFrame;
    flags: VideoDecoderRequestSyncPointFlags) =
  gst_video_decoder_request_sync_point(cast[ptr VideoDecoder00](self.impl), cast[ptr VideoCodecFrame00](frame.impl), flags)
...
proc getFrame*(self: VideoEncoder; frameNumber: int): VideoCodecFrame =
  let impl0 = gst_video_encoder_get_frame(cast[ptr VideoEncoder00](self.impl), int32(frameNumber))
  if impl0.isNil:
    return nil
  fnew(result, gBoxedFreeGstVideoCodecFrame)
  result.impl = impl0
...
proc getOldestFrame*(self: VideoEncoder): VideoCodecFrame =
  let impl0 = gst_video_encoder_get_oldest_frame(cast[ptr VideoEncoder00](self.impl))
  if impl0.isNil:
    return nil
  fnew(result, gBoxedFreeGstVideoCodecFrame)
  result.impl = impl0
...
proc getOutputState*(self: VideoEncoder): VideoCodecState =
  let impl0 = gst_video_encoder_get_output_state(cast[ptr VideoEncoder00](self.impl))
  if impl0.isNil:
    return nil
  fnew(result, gBoxedFreeGstVideoCodecState)
  result.impl = impl0
...
proc setOutputState*(self: VideoEncoder; caps: gst.Caps;
  ...
  let impl0 = gst_video_encoder_set_output_state(cast[ptr VideoEncoder00](self.impl), cast[ptr gst.Caps00](g_boxed_copy(gst_caps_get_type(), caps.impl)), if reference.isNil: nil else: cast[ptr VideoCodecState00](reference.impl))
  if impl0.isNil:
    return nil
  fnew(result, gBoxedFreeGstVideoCodecState)
  result.impl = impl0
...
type
  VideoTileModeFlag* {.size: sizeof(cint), pure.} = enum
    zflipz_2x2 = 16
    linear = 17
...
proc unknown*(t: typedesc[VideoTileMode]): VideoTileMode = VideoTileMode({})

type
  VideoTileInfo00* {.pure.} = object
  VideoTileInfo* = ref object
    impl*: ptr VideoTileInfo00
    ignoreFinalizer*: bool
type
  ...
  VideoFormatInfo* {.pure, byRef.} = object
    ...
    tileWs*: uint32
    tileHs*: uint32
    tileInfo*: array[4, VideoTileInfo00]
...
proc gst_video_format_info_extrapolate_stride(self: VideoFormatInfo; plane: int32;
    stride: int32): int32 {.
    importc, libprag.}

proc extrapolateStride*(self: VideoFormatInfo; plane: int;
    stride: int): int =
  int(gst_video_format_info_extrapolate_stride(self, int32(plane), int32(stride)))
...
type
  VideoInfoDmaDrm00* {.pure.} = object
  VideoInfoDmaDrm* = ref object
    impl*: ptr VideoInfoDmaDrm00
    ignoreFinalizer*: bool

proc gst_video_info_dma_drm_get_type*(): GType {.importc, libprag.}

proc gBoxedFreeGstVideoInfoDmaDrm*(self: VideoInfoDmaDrm) =
  if not self.ignoreFinalizer:
    boxedFree(gst_video_info_dma_drm_get_type(), cast[ptr VideoInfoDmaDrm00](self.impl))

when defined(gcDestructors):
  proc `=destroy`*(self: var typeof(VideoInfoDmaDrm()[])) =
    when defined(gintroDebug):
      echo "destroy ", $typeof(self), ' ', cast[int](unsafeaddr self)
    if not self.ignoreFinalizer and self.impl != nil:
      boxedFree(gst_video_info_dma_drm_get_type(), cast[ptr VideoInfoDmaDrm00](self.impl))
...
proc newWithFinalizer*(x: var VideoInfoDmaDrm) =
  when defined(gcDestructors):
    new(x)
  else:
    new(x, gBoxedFreeGstVideoInfoDmaDrm)

proc gst_video_info_dma_drm_free(self: ptr VideoInfoDmaDrm00) {.
    importc, libprag.}

proc free*(self: VideoInfoDmaDrm) =
  gst_video_info_dma_drm_free(cast[ptr VideoInfoDmaDrm00](self.impl))

proc finalizerfree*(self: VideoInfoDmaDrm) =
  if not self.ignoreFinalizer:
    gst_video_info_dma_drm_free(cast[ptr VideoInfoDmaDrm00](self.impl))

proc gst_video_info_dma_drm_new_from_caps(caps: ptr gst.Caps00): ptr VideoInfoDmaDrm00 {.
    importc, libprag.}

proc newVideoInfoDmaDrmFromCaps*(caps: gst.Caps): VideoInfoDmaDrm =
  let impl0 = gst_video_info_dma_drm_new_from_caps(cast[ptr gst.Caps00](caps.impl))
  if impl0.isNil:
    return nil
  fnew(result, gBoxedFreeGstVideoInfoDmaDrm)
  result.impl = impl0

proc newVideoInfoDmaDrmFromCaps*(tdesc: typedesc; caps: gst.Caps): tdesc =
  assert(result is VideoInfoDmaDrm)
  let impl0 = gst_video_info_dma_drm_new_from_caps(cast[ptr gst.Caps00](caps.impl))
  if impl0.isNil:
    return nil
  fnew(result, gBoxedFreeGstVideoInfoDmaDrm)
...
proc initVideoInfoDmaDrmFromCaps*[T](result: var T; caps: gst.Caps) {.deprecated.} =
  assert(result is VideoInfoDmaDrm)
  let impl0 = gst_video_info_dma_drm_new_from_caps(cast[ptr gst.Caps00](caps.impl))
  if impl0.isNil:
    return nil
...
proc gst_video_info_dma_drm_new(): ptr VideoInfoDmaDrm00 {.
    importc, libprag.}

proc newVideoInfoDmaDrm*(): VideoInfoDmaDrm =
  fnew(result, gBoxedFreeGstVideoInfoDmaDrm)
  result.impl = gst_video_info_dma_drm_new()

proc newVideoInfoDmaDrm*(tdesc: typedesc): tdesc =
  assert(result is VideoInfoDmaDrm)
  fnew(result, gBoxedFreeGstVideoInfoDmaDrm)
  result.impl = gst_video_info_dma_drm_new()

proc initVideoInfoDmaDrm*[T](result: var T) {.deprecated.} =
  assert(result is VideoInfoDmaDrm)
  fnew(result, gBoxedFreeGstVideoInfoDmaDrm)
  result.impl = gst_video_info_dma_drm_new()

proc gst_video_info_dma_drm_to_caps(self: ptr VideoInfoDmaDrm00): ptr gst.Caps00 {.
    importc, libprag.}

proc toCaps*(self: VideoInfoDmaDrm): gst.Caps =
  let impl0 = gst_video_info_dma_drm_to_caps(cast[ptr VideoInfoDmaDrm00](self.impl))
  if impl0.isNil:
    return nil
  fnew(result, gBoxedFreeGstCaps)
  result.impl = impl0

proc gst_video_info_dma_drm_to_video_info(self: ptr VideoInfoDmaDrm00; info: var VideoInfo00): gboolean {.
    importc, libprag.}

proc toVideoInfo*(self: VideoInfoDmaDrm; info: var VideoInfo00): bool =
  toBool(gst_video_info_dma_drm_to_video_info(cast[ptr VideoInfoDmaDrm00](self.impl), info))

proc gst_video_info_dma_drm_from_caps(drmInfo: var VideoInfoDmaDrm00; caps: ptr gst.Caps00): gboolean {.
    importc, libprag.}

proc fromCaps*(drmInfo: var VideoInfoDmaDrm00; caps: gst.Caps): bool =
  toBool(gst_video_info_dma_drm_from_caps(drmInfo, cast[ptr gst.Caps00](caps.impl)))

proc gst_video_info_dma_drm_from_video_info(drmInfo: var VideoInfoDmaDrm00;
    info: ptr VideoInfo00; modifier: uint64): gboolean {.
    importc, libprag.}

proc fromVideoInfo*(drmInfo: var VideoInfoDmaDrm00;
    info: VideoInfo; modifier: uint64): bool =
  toBool(gst_video_info_dma_drm_from_video_info(drmInfo, cast[ptr VideoInfo00](info.impl), modifier))

proc gst_video_info_dma_drm_init(drmInfo: var VideoInfoDmaDrm00) {.
    importc, libprag.}

proc init*(drmInfo: var VideoInfoDmaDrm00) =
...
proc toCapsString*(mviewMode: VideoMultiviewMode): string =
  let resul0 = gst_video_multiview_mode_to_caps_string(mviewMode)
  if resul0.isNil:
    return
  result = $resul0
...
proc gst_video_orientation_from_tag(taglist: ptr gst.TagList00; `method`: var VideoOrientationMethod): gboolean {.
    importc, libprag.}

proc fromTag*(taglist: gst.TagList; `method`: var VideoOrientationMethod): bool =
  toBool(gst_video_orientation_from_tag(cast[ptr gst.TagList00](taglist.impl), `method`))
...
proc gst_video_overlay_set_property(obj: ptr gobject.Object00; lastPropId: int32;
    propertyId: uint32; value: gobject.Value): gboolean {.
    importc, libprag.}

proc setProperty*(obj: gobject.Object; lastPropId: int;
    propertyId: int; value: gobject.Value): bool =
  toBool(gst_video_overlay_set_property(cast[ptr gobject.Object00](obj.impl), int32(lastPropId), uint32(propertyId), value))
...
proc newVideoOverlayComposition*(rectangle: VideoOverlayRectangle = nil): VideoOverlayComposition =
  fnew(result, gBoxedFreeGstVideoOverlayComposition)
  result.impl = gst_video_overlay_composition_new(if rectangle.isNil: nil else: cast[ptr VideoOverlayRectangle00](rectangle.impl))

proc newVideoOverlayComposition*(tdesc: typedesc; rectangle: VideoOverlayRectangle = nil): tdesc =
  assert(result is VideoOverlayComposition)
  fnew(result, gBoxedFreeGstVideoOverlayComposition)
  result.impl = gst_video_overlay_composition_new(if rectangle.isNil: nil else: cast[ptr VideoOverlayRectangle00](rectangle.impl))

proc initVideoOverlayComposition*[T](result: var T; rectangle: VideoOverlayRectangle = nil) {.deprecated.} =
  assert(result is VideoOverlayComposition)
  fnew(result, gBoxedFreeGstVideoOverlayComposition)
  result.impl = gst_video_overlay_composition_new(if rectangle.isNil: nil else: cast[ptr VideoOverlayRectangle00](rectangle.impl))
...
proc getRectangle*(self: VideoOverlayComposition;
  ...
  let impl0 = gst_video_overlay_composition_get_rectangle(cast[ptr VideoOverlayComposition00](self.impl), uint32(n))
  if impl0.isNil:
    return nil
  fnew(result, gBoxedFreeGstVideoOverlayRectangle)
  result.impl = cast[typeof(result.impl)](g_boxed_copy(gst_video_overlay_rectangle_get_type(), impl0))
...
type
  VideoSEIUserDataUnregisteredMeta00* {.pure.} = object
  VideoSEIUserDataUnregisteredMeta* = ref object
    impl*: ptr VideoSEIUserDataUnregisteredMeta00
    ignoreFinalizer*: bool

proc gst_video_sei_user_data_unregistered_meta_get_info(): ptr gst.MetaInfo00 {.
    importc, libprag.}

proc getInfoVideoSEIUserDataUnregisteredMeta*(): gst.MetaInfo =
  new(result)
  result.impl = gst_video_sei_user_data_unregistered_meta_get_info()
  result.ignoreFinalizer = true
...
proc gst_video_scaler_get_coeff(self: ptr VideoScaler00; outOffset: uint32;
    inOffset: var uint32; nTaps: var uint32): ptr cdouble {.
    importc, libprag.}

proc getCoeff*(self: VideoScaler; outOffset: int; inOffset: var int = cast[var int](nil);
    nTaps: var int = cast[var int](nil)): ptr cdouble =
  var inOffset_00: uint32
  var nTaps_00: uint32
  result = gst_video_scaler_get_coeff(cast[ptr VideoScaler00](self.impl), uint32(outOffset), inOffset_00, nTaps_00)
  if inOffset.addr != nil:
    inOffset = int(inOffset_00)
  if nTaps.addr != nil:
    nTaps = int(nTaps_00)
...
proc gst_video_sink_center_rect(src: VideoRectangle; dst: VideoRectangle;
    resu: var VideoRectangle; scaling: gboolean) {.
    importc, libprag.}

proc centerRect*(src: VideoRectangle; dst: VideoRectangle;
    resu: var VideoRectangle; scaling: bool) =
...
proc newVideoTimeCodeFromDateTimeFull*(fpsN: int; fpsD: int; dt: glib.DateTime;
    flags: VideoTimeCodeFlags; fieldCount: int): VideoTimeCode =
  let impl0 = gst_video_time_code_new_from_date_time_full(uint32(fpsN), uint32(fpsD), cast[ptr glib.DateTime00](dt.impl), flags, uint32(fieldCount))
  if impl0.isNil:
    return nil
  fnew(result, gBoxedFreeGstVideoTimeCode)
  result.impl = impl0
...
proc newVideoVBIEncoder*(format: VideoFormat; pixelWidth: int): VideoVBIEncoder =
  let impl0 = gst_video_vbi_encoder_new(format, uint32(pixelWidth))
  if impl0.isNil:
    return nil
  fnew(result, gBoxedFreeGstVideoVBIEncoder)
  result.impl = impl0
...
proc newVideoVBIParser*(format: VideoFormat; pixelWidth: int): VideoVBIParser =
  let impl0 = gst_video_vbi_parser_new(format, uint32(pixelWidth))
  if impl0.isNil:
    return nil
  fnew(result, gBoxedFreeGstVideoVBIParser)
  result.impl = impl0
...
proc ancillaryMetaApiGetType*(): GType {.
    importc: "gst_ancillary_meta_api_get_type", libprag.}

proc gst_buffer_add_ancillary_meta(buffer: ptr gst.Buffer00): ptr AncillaryMeta00 {.
    importc, libprag.}

proc bufferAddAncillaryMeta*(buffer: gst.Buffer): AncillaryMeta =
  new(result)
  result.impl = gst_buffer_add_ancillary_meta(cast[ptr gst.Buffer00](buffer.impl))
  result.ignoreFinalizer = true
...
proc gst_buffer_add_video_codec_alpha_meta(buffer: ptr gst.Buffer00; alphaBuffer: ptr gst.Buffer00): ptr VideoCodecAlphaMeta00 {.
    importc, libprag.}

proc bufferAddVideoCodecAlphaMeta*(buffer: gst.Buffer; alphaBuffer: gst.Buffer): VideoCodecAlphaMeta =
  new(result)
  result.impl = gst_buffer_add_video_codec_alpha_meta(cast[ptr gst.Buffer00](buffer.impl), cast[ptr gst.Buffer00](g_boxed_copy(gst_buffer_get_type(), alphaBuffer.impl)))
  result.ignoreFinalizer = true
...
proc gst_buffer_add_video_sei_user_data_unregistered_meta(buffer: ptr gst.Buffer00;
    uuid: ptr uint8; data: ptr uint8; size: uint64): ptr VideoSEIUserDataUnregisteredMeta00 {.
    importc, libprag.}

proc bufferAddVideoSeiUserDataUnregisteredMeta*(buffer: gst.Buffer;
    uuid: ptr uint8; data: ptr uint8; size: uint64): VideoSEIUserDataUnregisteredMeta =
  new(result)
  result.impl = gst_buffer_add_video_sei_user_data_unregistered_meta(cast[ptr gst.Buffer00](buffer.impl), uuid, data, size)
  result.ignoreFinalizer = true
...
proc bufferAddVideoTimeCodeMetaFull*(buffer: gst.Buffer; fpsN: int;
  ...
  let impl0 = gst_buffer_add_video_time_code_meta_full(cast[ptr gst.Buffer00](buffer.impl), uint32(fpsN), uint32(fpsD), cast[ptr glib.DateTime00](latestDailyJam.impl), flags, uint32(hours), uint32(minutes), uint32(seconds), uint32(frames), uint32(fieldCount))
  if impl0.isNil:
    return nil
  new(result)
  result.impl = impl0
...
proc bufferGetVideoRegionOfInterestMetaId*(buffer: gst.Buffer;
  ...
  let impl0 = gst_buffer_get_video_region_of_interest_meta_id(cast[ptr gst.Buffer00](buffer.impl), int32(id))
  if impl0.isNil:
    return nil
  new(result)
  result.impl = impl0
...
proc gst_video_center_rect(src: VideoRectangle; dst: VideoRectangle; resu: var VideoRectangle;
    scaling: gboolean) {.
    importc, libprag.}

proc videoCenterRect*(src: VideoRectangle; dst: VideoRectangle; resu: var VideoRectangle;
    scaling: bool) =
  gst_video_center_rect(src, dst, resu, gboolean(scaling))
...
proc videoCodecAlphaMetaApiGetType*(): GType {.
    importc: "gst_video_codec_alpha_meta_api_get_type", libprag.}
...
proc gst_video_dma_drm_fourcc_from_format(format: VideoFormat): uint32 {.
    importc, libprag.}

proc videoDmaDrmFourccFromFormat*(format: VideoFormat): int =
  int(gst_video_dma_drm_fourcc_from_format(format))

proc gst_video_dma_drm_fourcc_from_string(formatStr: cstring; modifier: var uint64): uint32 {.
    importc, libprag.}

proc videoDmaDrmFourccFromString*(formatStr: cstring; modifier: var uint64 = cast[var uint64](nil)): int =
  int(gst_video_dma_drm_fourcc_from_string(formatStr, modifier))

proc gst_video_dma_drm_fourcc_to_format(fourcc: uint32): VideoFormat {.
    importc, libprag.}

proc videoDmaDrmFourccToFormat*(fourcc: int): VideoFormat =
  gst_video_dma_drm_fourcc_to_format(uint32(fourcc))

proc gst_video_dma_drm_fourcc_to_string(fourcc: uint32; modifier: uint64): cstring {.
    importc, libprag.}

proc videoDmaDrmFourccToString*(fourcc: int; modifier: uint64): string =
  let resul0 = gst_video_dma_drm_fourcc_to_string(uint32(fourcc), modifier)
  if resul0.isNil:
    return
  result = $resul0
  cogfree(resul0)
...
proc gst_video_formats_any(len: var uint32): ptr VideoFormat {.
    importc, libprag.}

proc videoFormatsAny*(len: var int): ptr VideoFormat =
  var len_00: uint32
  result = gst_video_formats_any(len_00)
  if len.addr != nil:
    len = int(len_00)
...
proc videoIsCommonAspectRatio*(width: int; height: int; parN: int;
    parD: int): bool =
  toBool(gst_video_is_common_aspect_ratio(int32(width), int32(height), int32(parN), int32(parD)))

proc gst_video_is_dma_drm_caps(caps: ptr gst.Caps00): gboolean {.
    importc, libprag.}

proc videoIsDmaDrmCaps*(caps: gst.Caps): bool =
  toBool(gst_video_is_dma_drm_caps(cast[ptr gst.Caps00](caps.impl)))
...
proc videoSeiUserDataUnregisteredMetaApiGetType*(): GType {.
    importc: "gst_video_sei_user_data_unregistered_meta_api_get_type", libprag.}

proc gst_video_sei_user_data_unregistered_parse_precision_time_stamp(userData: ptr VideoSEIUserDataUnregisteredMeta00;
    status: var uint8; precisionTimeStamp: var uint64): gboolean {.
    importc, libprag.}

proc videoSeiUserDataUnregisteredParsePrecisionTimeStamp*(userData: VideoSEIUserDataUnregisteredMeta;
    status: var uint8; precisionTimeStamp: var uint64): bool =
  toBool(gst_video_sei_user_data_unregistered_parse_precision_time_stamp(cast[ptr VideoSEIUserDataUnregisteredMeta00](userData.impl), status, precisionTimeStamp))
