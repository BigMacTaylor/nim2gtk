...
const AUDIO_DECODER_MAX_ERRORS* = -1'i32
...
proc gst_audio_ring_buffer_commit(self: ptr AudioRingBuffer00; sample: var uint64;
    data: ptr uint8; inSamples: int32; outSamples: int32; accum: var int32): uint32 {.
    importc, libprag.}

proc commit*(self: AudioRingBuffer; sample: var uint64;
...
proc gst_audio_ring_buffer_set_errored(self: ptr AudioRingBuffer00) {.
    importc, libprag.}

proc setErrored*(self: AudioRingBuffer) =
  gst_audio_ring_buffer_set_errored(cast[ptr AudioRingBuffer00](self.impl))
...
proc clip*(buffer: gst.Buffer; segment: gst.Segment; rate: int;
    bpf: int): gst.Buffer =
  let impl0 = gst_audio_buffer_clip(cast[ptr gst.Buffer00](g_boxed_copy(gst_buffer_get_type(), buffer.impl)), cast[ptr gst.Segment00](segment.impl), int32(rate), int32(bpf))
  if impl0.isNil:
    return nil
...
proc gst_audio_info_new_from_caps(caps: ptr gst.Caps00): ptr AudioInfo00 {.
    importc, libprag.}

proc newAudioInfoFromCaps*(caps: gst.Caps): AudioInfo =
  let impl0 = gst_audio_info_new_from_caps(cast[ptr gst.Caps00](caps.impl))
  if impl0.isNil:
    return nil
  fnew(result, gBoxedFreeGstAudioInfo)
  result.impl = impl0

proc newAudioInfoFromCaps*(tdesc: typedesc; caps: gst.Caps): tdesc =
...
proc initAudioInfoFromCaps*[T](result: var T; caps: gst.Caps) {.deprecated.} =
  assert(result is AudioInfo)
  let impl0 = gst_audio_info_new_from_caps(cast[ptr gst.Caps00](caps.impl))
  if impl0.isNil:
    return nil
...
proc gst_audio_info_from_caps(info: var AudioInfo00; caps: ptr gst.Caps00): gboolean {.
    importc, libprag.}

proc fromCaps*(info: var AudioInfo00; caps: gst.Caps): bool =
  toBool(gst_audio_info_from_caps(info, cast[ptr gst.Caps00](caps.impl)))

proc gst_audio_info_init(info: var AudioInfo00) {.
    importc, libprag.}

proc init*(info: var AudioInfo00) =
  gst_audio_info_init(info)

proc gst_audio_buffer_map(buffer: var AudioBuffer00; info: ptr AudioInfo00;
    gstbuffer: ptr gst.Buffer00; flags: gst.MapFlags): gboolean {.
    importc, libprag.}

proc map*(buffer: var AudioBuffer00; info: AudioInfo; gstbuffer: gst.Buffer;
    flags: gst.MapFlags): bool =
  toBool(gst_audio_buffer_map(buffer, cast[ptr AudioInfo00](info.impl), cast[ptr gst.Buffer00](gstbuffer.impl), flags))
...
proc gst_audio_format_info_fill_silence(self: ptr AudioFormatInfo00; dest: ptr uint8;
    length: uint64) {.
    importc, libprag.}

proc fillSilence*(self: AudioFormatInfo; dest: seq[uint8] | string) =
  let length = uint64(dest.len)
  gst_audio_format_info_fill_silence(cast[ptr AudioFormatInfo00](self.impl), cast[ptr uint8](unsafeaddr(dest[0])), length)
...
proc newAudioConverter*(flags: AudioConverterFlags; inInfo: AudioInfo;
  ...
  let impl0 = gst_audio_converter_new(flags, cast[ptr AudioInfo00](inInfo.impl), cast[ptr AudioInfo00](outInfo.impl), if config.isNil: nil else: cast[ptr gst.Structure00](g_boxed_copy(gst_structure_get_type(), config.impl)))
  if impl0.isNil:
    return nil
...
proc getAudioInfo*(self: AudioDecoder): AudioInfo =
  ...
  result.impl = cast[typeof(result.impl)](g_boxed_copy(gst_audio_info_get_type(), result.impl))
...
type
  AudioLevelMeta00* {.pure.} = object
  AudioLevelMeta* = ref object
    impl*: ptr AudioLevelMeta00
    ignoreFinalizer*: bool

proc gst_audio_level_meta_get_info(): ptr gst.MetaInfo00 {.
    importc, libprag.}

proc getInfoAudioLevelMeta*(): gst.MetaInfo =
  new(result)
  result.impl = gst_audio_level_meta_get_info()
  result.ignoreFinalizer = true
...
const DSD_SILENCE_PATTERN_BYTE* = 105'i32

type
  DsdFormat* {.size: sizeof(cint), pure.} = enum
    dsdFormatUnknown = 0
    dsdFormatU8 = 1
    dsdFormatU16 = 2
    dsdFormatU16be = 3
...
proc gst_dsd_format_get_width(format: DsdFormat): uint32 {.
    importc, libprag.}

proc getWidth*(format: DsdFormat): int =
  int(gst_dsd_format_get_width(format))

proc width*(format: DsdFormat): int =
  int(gst_dsd_format_get_width(format))

proc gst_dsd_format_to_string(format: DsdFormat): cstring {.
    importc, libprag.}

proc toString*(format: DsdFormat): string =
  result = $gst_dsd_format_to_string(format)

type
  DsdInfo00* {.pure.} = object
  DsdInfo* = ref object
    impl*: ptr DsdInfo00
    ignoreFinalizer*: bool

proc gst_dsd_info_get_type*(): GType {.importc, libprag.}

proc gBoxedFreeGstDsdInfo*(self: DsdInfo) =
  if not self.ignoreFinalizer:
    boxedFree(gst_dsd_info_get_type(), cast[ptr DsdInfo00](self.impl))

when defined(gcDestructors):
  proc `=destroy`*(self: var typeof(DsdInfo()[])) =
    when defined(gintroDebug):
      echo "destroy ", $typeof(self), ' ', cast[int](unsafeaddr self)
    if not self.ignoreFinalizer and self.impl != nil:
      boxedFree(gst_dsd_info_get_type(), cast[ptr DsdInfo00](self.impl))
...
proc newWithFinalizer*(x: var DsdInfo) =
  when defined(gcDestructors):
    new(x)
  else:
    new(x, gBoxedFreeGstDsdInfo)

proc gst_dsd_info_free(self: ptr DsdInfo00) {.
    importc, libprag.}

proc free*(self: DsdInfo) =
  gst_dsd_info_free(cast[ptr DsdInfo00](self.impl))

proc finalizerfree*(self: DsdInfo) =
  if not self.ignoreFinalizer:
    gst_dsd_info_free(cast[ptr DsdInfo00](self.impl))

proc gst_dsd_info_new_from_caps(caps: ptr gst.Caps00): ptr DsdInfo00 {.
    importc, libprag.}

proc newDsdInfoFromCaps*(caps: gst.Caps): DsdInfo =
  fnew(result, gBoxedFreeGstDsdInfo)
  result.impl = gst_dsd_info_new_from_caps(cast[ptr gst.Caps00](caps.impl))

proc newDsdInfoFromCaps*(tdesc: typedesc; caps: gst.Caps): tdesc =
  assert(result is DsdInfo)
  fnew(result, gBoxedFreeGstDsdInfo)
  result.impl = gst_dsd_info_new_from_caps(cast[ptr gst.Caps00](caps.impl))

proc initDsdInfoFromCaps*[T](result: var T; caps: gst.Caps) {.deprecated.} =
  assert(result is DsdInfo)
  fnew(result, gBoxedFreeGstDsdInfo)
  result.impl = gst_dsd_info_new_from_caps(cast[ptr gst.Caps00](caps.impl))

proc gst_dsd_info_copy(self: ptr DsdInfo00): ptr DsdInfo00 {.
    importc, libprag.}

proc copy*(self: DsdInfo): DsdInfo =
  fnew(result, gBoxedFreeGstDsdInfo)
  result.impl = gst_dsd_info_copy(cast[ptr DsdInfo00](self.impl))

proc gst_dsd_info_new(): ptr DsdInfo00 {.
    importc, libprag.}

proc newDsdInfo*(): DsdInfo =
  fnew(result, gBoxedFreeGstDsdInfo)
  result.impl = gst_dsd_info_new()

proc newDsdInfo*(tdesc: typedesc): tdesc =
  assert(result is DsdInfo)
  fnew(result, gBoxedFreeGstDsdInfo)
  result.impl = gst_dsd_info_new()

proc initDsdInfo*[T](result: var T) {.deprecated.} =
  assert(result is DsdInfo)
  fnew(result, gBoxedFreeGstDsdInfo)
  result.impl = gst_dsd_info_new()

proc gst_dsd_info_is_equal(self: ptr DsdInfo00; other: ptr DsdInfo00): gboolean {.
    importc, libprag.}

proc isEqual*(self: DsdInfo; other: DsdInfo): bool =
  toBool(gst_dsd_info_is_equal(cast[ptr DsdInfo00](self.impl), cast[ptr DsdInfo00](other.impl)))

proc gst_dsd_info_set_format(self: ptr DsdInfo00; format: DsdFormat; rate: int32;
    channels: int32; positions: ptr array[64, AudioChannelPosition]) {.
    importc, libprag.}

proc setFormat*(self: DsdInfo; format: DsdFormat; rate: int;
    channels: int; positions: ptr array[64, AudioChannelPosition]) =
  gst_dsd_info_set_format(cast[ptr DsdInfo00](self.impl), format, int32(rate), int32(channels), positions)

proc gst_dsd_info_to_caps(self: ptr DsdInfo00): ptr gst.Caps00 {.
    importc, libprag.}

proc toCaps*(self: DsdInfo): gst.Caps =
  fnew(result, gBoxedFreeGstCaps)
  result.impl = gst_dsd_info_to_caps(cast[ptr DsdInfo00](self.impl))

proc gst_dsd_info_from_caps(info: var DsdInfo00; caps: ptr gst.Caps00): gboolean {.
    importc, libprag.}

proc fromCaps*(info: var DsdInfo00; caps: gst.Caps): bool =
  toBool(gst_dsd_info_from_caps(info, cast[ptr gst.Caps00](caps.impl)))

proc gst_dsd_info_init(info: var DsdInfo00) {.
    importc, libprag.}

proc init*(info: var DsdInfo00) =
  gst_dsd_info_init(info)

type
  DsdPlaneOffsetMeta00* {.pure.} = object
  DsdPlaneOffsetMeta* = ref object
    impl*: ptr DsdPlaneOffsetMeta00
    ignoreFinalizer*: bool

proc gst_dsd_plane_offset_meta_get_info(): ptr gst.MetaInfo00 {.
    importc, libprag.}

proc getInfoDsdPlaneOffsetMeta*(): gst.MetaInfo =
  new(result)
  result.impl = gst_dsd_plane_offset_meta_get_info()
  result.ignoreFinalizer = true
...
proc audioLevelMetaApiGetType*(): GType {.
    importc: "gst_audio_level_meta_api_get_type", libprag.}
...
proc gst_buffer_add_audio_level_meta(buffer: ptr gst.Buffer00; level: uint8;
    voiceActivity: gboolean): ptr AudioLevelMeta00 {.
    importc, libprag.}

proc bufferAddAudioLevelMeta*(buffer: gst.Buffer; level: uint8; voiceActivity: bool): AudioLevelMeta =
  let impl0 = gst_buffer_add_audio_level_meta(cast[ptr gst.Buffer00](buffer.impl), level, gboolean(voiceActivity))
  if impl0.isNil:
    return nil
  new(result)
  result.impl = impl0
...
proc gst_buffer_add_dsd_plane_offset_meta(buffer: ptr gst.Buffer00; numChannels: int32;
    numBytesPerChannel: uint64; offsets: ptr uint64): ptr DsdPlaneOffsetMeta00 {.
    importc, libprag.}

proc bufferAddDsdPlaneOffsetMeta*(buffer: gst.Buffer; numChannels: int;
    numBytesPerChannel: uint64; offsets: ptr uint64): DsdPlaneOffsetMeta =
  new(result)
  result.impl = gst_buffer_add_dsd_plane_offset_meta(cast[ptr gst.Buffer00](buffer.impl), int32(numChannels), numBytesPerChannel, offsets)
  result.ignoreFinalizer = true
...
proc gst_buffer_get_audio_level_meta(buffer: ptr gst.Buffer00): ptr AudioLevelMeta00 {.
    importc, libprag.}

proc bufferGetAudioLevelMeta*(buffer: gst.Buffer): AudioLevelMeta =
  let impl0 = gst_buffer_get_audio_level_meta(cast[ptr gst.Buffer00](buffer.impl))
  if impl0.isNil:
    return nil
  new(result)
  result.impl = impl0
...
proc gst_dsd_convert(inputData: ptr uint8; outputData: ptr uint8; inputFormat: DsdFormat;
    outputFormat: DsdFormat; inputLayout: AudioLayout; outputLayout: AudioLayout;
    inputPlaneOffsets: ptr uint64; outputPlaneOffsets: ptr uint64; numDsdBytes: uint64;
    numChannels: int32; reverseByteBits: gboolean) {.
    importc, libprag.}

proc dsdConvert*(inputData: ptr uint8; outputData: ptr uint8; inputFormat: DsdFormat;
    outputFormat: DsdFormat; inputLayout: AudioLayout; outputLayout: AudioLayout;
    inputPlaneOffsets: ptr uint64; outputPlaneOffsets: ptr uint64; numDsdBytes: uint64;
    numChannels: int; reverseByteBits: bool) =
  gst_dsd_convert(inputData, outputData, inputFormat, outputFormat, inputLayout, outputLayout, inputPlaneOffsets, outputPlaneOffsets, numDsdBytes, int32(numChannels), gboolean(reverseByteBits))

proc dsdPlaneOffsetMetaApiGetType*(): GType {.
    importc: "gst_dsd_plane_offset_meta_api_get_type", libprag.}
# === remaining symbols:
