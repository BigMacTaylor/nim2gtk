...
proc getOption*(self: Pixbuf; key: cstring): string =
  let resul0 = gdk_pixbuf_get_option(cast[ptr Pixbuf00](self.impl), key)
  if resul0.isNil:
    return
  result = $resul0
...
type
  PixbufFormatFlag* {.size: sizeof(cint), pure.} = enum
    writable = 0
    scalable = 1
    threadsafe = 2
...
type
  PixbufModule00* {.pure.} = object
  PixbufModule* = ref object
    impl*: ptr PixbufModule00
    ignoreFinalizer*: bool

type
  PixbufModuleFillInfoFunc* = proc (info: ptr PixbufFormat00) {.cdecl.}

type
  PixbufModuleFillVtableFunc* = proc (module: ptr PixbufModule00) {.cdecl.}

type
  PixbufModuleIncrementLoadFunc* = proc (context: pointer; buf: ptr uint8; size: uint32; error: ptr ptr glib.Error = nil): gboolean {.cdecl.}

type
  PixbufModuleLoadAnimationFunc* = proc (f: pointer; error: ptr ptr glib.Error = nil): ptr PixbufAnimation00 {.cdecl.}

type
  PixbufModuleLoadFunc* = proc (f: pointer; error: ptr ptr glib.Error = nil): ptr Pixbuf00 {.cdecl.}

type
  PixbufModuleLoadXpmDataFunc* = proc (data: ptr cstring): ptr Pixbuf00 {.cdecl.}

type
  PixbufModulePattern00* {.pure.} = object
  PixbufModulePattern* = ref object
    impl*: ptr PixbufModulePattern00
    ignoreFinalizer*: bool

type
  PixbufModulePreparedFunc* = proc (pixbuf: ptr Pixbuf00; anim: ptr PixbufAnimation00; userData: pointer) {.cdecl.}

type
  PixbufModuleSaveFunc* = proc (f: pointer; pixbuf: ptr Pixbuf00; paramKeys: ptr cstring; paramValues: ptr cstring;
    error: ptr ptr glib.Error = nil): gboolean {.cdecl.}

type
  PixbufModuleSaveOptionSupportedFunc* = proc (optionKey: cstring): gboolean {.cdecl.}

type
  PixbufModuleSizeFunc* = proc (width: ptr int32; height: ptr int32; userData: pointer) {.cdecl.}

type
  PixbufModuleStopLoadFunc* = proc (context: pointer; error: ptr ptr glib.Error = nil): gboolean {.cdecl.}

type
  PixbufModuleUpdatedFunc* = proc (pixbuf: ptr Pixbuf00; x: int32; y: int32; width: int32; height: int32;
    userData: pointer) {.cdecl.}

type
  PixbufNonAnim* = ref object of PixbufAnimation
  PixbufNonAnim00* = object of PixbufAnimation00

proc gdk_pixbuf_non_anim_get_type*(): GType {.importc, libprag.}

when defined(gcDestructors):
  proc `=destroy`*(self: var typeof(PixbufNonAnim()[])) =
    when defined(gintroDebug):
      echo "destroy ", $typeof(self), ' ', cast[int](unsafeaddr self)
    g_object_set_qdata(self.impl, Quark, nil)
    if not self.ignoreFinalizer and self.impl != nil:
...
proc gdk_pixbuf_non_anim_new(pixbuf: ptr Pixbuf00): ptr PixbufNonAnim00 {.
    importc, libprag.}

proc newPixbufNonAnim*(pixbuf: Pixbuf): PixbufNonAnim =
  let gobj = gdk_pixbuf_non_anim_new(cast[ptr Pixbuf00](pixbuf.impl))
  let qdata = g_object_get_qdata(gobj, Quark)
  if qdata != nil:
    result = cast[type(result)](qdata)
    assert(result.impl == gobj)
...
proc newPixbufNonAnim*(tdesc: typedesc; pixbuf: Pixbuf): tdesc =
  assert(result is PixbufNonAnim)
  let gobj = gdk_pixbuf_non_anim_new(cast[ptr Pixbuf00](pixbuf.impl))
  let qdata = g_object_get_qdata(gobj, Quark)
  if qdata != nil:
    result = cast[type(result)](qdata)
...
proc initPixbufNonAnim*[T](result: var T; pixbuf: Pixbuf) {.deprecated.} =
  assert(result is PixbufNonAnim)
  let gobj = gdk_pixbuf_non_anim_new(cast[ptr Pixbuf00](pixbuf.impl))
  let qdata = g_object_get_qdata(gobj, Quark)
  if qdata != nil:
