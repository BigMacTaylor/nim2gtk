...
const ALLOCATOR_LIST* = 1'i32

const ALLOCATOR_NODE* = 3'i32

const ALLOCATOR_SLIST* = 2'i32

const ALLOC_AND_FREE* = 2'i32

const ALLOC_ONLY* = 1'i32

const ANALYZER_ANALYZING* = 1'i32

const ASCII_DTOSTR_BUF_SIZE* = 39'i32

const ATOMIC_REF_COUNT_INIT* = 1'i32

type
  Allocator00* {.pure.} = object
  Allocator* = ref object
    impl*: ptr Allocator00
    ignoreFinalizer*: bool

proc g_allocator_free(self: ptr Allocator00) {.
    importc, libprag.}

proc free*(self: Allocator) =
  g_allocator_free(cast[ptr Allocator00](self.impl))

proc finalizerfree*(self: Allocator) =
...
when defined(gcDestructors):
  proc `=destroy`*(self: var typeof(Allocator()[])) =
    ...
    if not self.ignoreFinalizer and self.impl != nil:
      g_allocator_free(self.impl)
      self.impl = nil

proc newWithFinalizer*(x: var Allocator) =
  when defined(gcDestructors):
    new(x)
  else:
    new(x, finalizerfree)
...
type
  AsciiTypeFlag* {.size: sizeof(cint), pure.} = enum
    alnum = 0
    alpha = 1
    cntrl = 2
    digit = 3
...
proc g_bookmark_file_get_type*(): GType {.importc, gobjectlibprag.}

proc gBoxedFreeGBookmarkFile*(self: BookmarkFile) =
  if not self.ignoreFinalizer:
    boxedFree(g_bookmark_file_get_type(), cast[ptr BookmarkFile00](self.impl))
...
proc g_bookmark_file_copy(self: ptr BookmarkFile00): ptr BookmarkFile00 {.
    importc, libprag.}

proc copy*(self: BookmarkFile): BookmarkFile =
  fnew(result, gBoxedFreeGBookmarkFile)
  result.impl = g_bookmark_file_copy(cast[ptr BookmarkFile00](self.impl))

proc g_bookmark_file_new(): ptr BookmarkFile00 {.
    importc, libprag.}

proc newBookmarkFile*(): BookmarkFile =
  fnew(result, gBoxedFreeGBookmarkFile)
  result.impl = g_bookmark_file_new()

proc newBookmarkFile*(tdesc: typedesc): tdesc =
  assert(result is BookmarkFile)
  fnew(result, gBoxedFreeGBookmarkFile)
  result.impl = g_bookmark_file_new()

proc initBookmarkFile*[T](result: var T) {.deprecated.} =
  assert(result is BookmarkFile)
  fnew(result, gBoxedFreeGBookmarkFile)
  result.impl = g_bookmark_file_new()
...
proc g_date_time_new_from_unix_local_usec(usecs: int64): ptr DateTime00 {.
    importc, libprag.}

proc newDateTimeFromUnixLocalUsec*(usecs: int64): DateTime =
  let impl0 = g_date_time_new_from_unix_local_usec(usecs)
  if impl0.isNil:
    return nil
  fnew(result, gBoxedFreeGDateTime)
  result.impl = impl0

proc newDateTimeFromUnixLocalUsec*(tdesc: typedesc; usecs: int64): tdesc =
  assert(result is DateTime)
  let impl0 = g_date_time_new_from_unix_local_usec(usecs)
  if impl0.isNil:
    return nil
...
proc initDateTimeFromUnixLocalUsec*[T](result: var T; usecs: int64) {.deprecated.} =
  assert(result is DateTime)
  let impl0 = g_date_time_new_from_unix_local_usec(usecs)
  if impl0.isNil:
    return nil
...
proc g_date_time_new_from_unix_utc_usec(usecs: int64): ptr DateTime00 {.
    importc, libprag.}

proc newDateTimeFromUnixUtcUsec*(usecs: int64): DateTime =
  let impl0 = g_date_time_new_from_unix_utc_usec(usecs)
  if impl0.isNil:
    return nil
  fnew(result, gBoxedFreeGDateTime)
  result.impl = impl0

proc newDateTimeFromUnixUtcUsec*(tdesc: typedesc; usecs: int64): tdesc =
  assert(result is DateTime)
  let impl0 = g_date_time_new_from_unix_utc_usec(usecs)
  if impl0.isNil:
    return nil
...
proc initDateTimeFromUnixUtcUsec*[T](result: var T; usecs: int64) {.deprecated.} =
  assert(result is DateTime)
  let impl0 = g_date_time_new_from_unix_utc_usec(usecs)
  if impl0.isNil:
    return nil
...
proc g_date_time_compare(self: ptr DateTime00; dt2: ptr DateTime00): int32 {.
    importc, libprag.}

proc compare*(self: DateTime; dt2: DateTime): int =
  int(g_date_time_compare(cast[ptr DateTime00](self.impl), cast[ptr DateTime00](dt2.impl)))
...
proc g_date_time_equal(self: ptr DateTime00; dt2: ptr DateTime00): gboolean {.
    importc, libprag.}

proc equal*(self: DateTime; dt2: DateTime): bool =
  toBool(g_date_time_equal(cast[ptr DateTime00](self.impl), cast[ptr DateTime00](dt2.impl)))
...
proc g_date_time_hash(self: ptr DateTime00): uint32 {.
    importc, libprag.}

proc hash*(self: DateTime): int =
  int(g_date_time_hash(cast[ptr DateTime00](self.impl)))
...
proc g_date_time_to_unix_usec(self: ptr DateTime00): int64 {.
    importc, libprag.}

proc toUnixUsec*(self: DateTime): int64 =
  g_date_time_to_unix_usec(cast[ptr DateTime00](self.impl))
...
proc g_time_zone_new_identifier(identifier: cstring): ptr TimeZone00 {.
    importc, libprag.}

proc newTimeZoneIdentifier*(identifier: cstring = nil): TimeZone =
  let impl0 = g_time_zone_new_identifier(identifier)
  if impl0.isNil:
    return nil
  fnew(result, gBoxedFreeGTimeZone)
  result.impl = impl0

proc newTimeZoneIdentifier*(tdesc: typedesc; identifier: cstring = nil): tdesc =
...
proc initTimeZoneIdentifier*[T](result: var T; identifier: cstring = nil) {.deprecated.} =
  assert(result is TimeZone)
  let impl0 = g_time_zone_new_identifier(identifier)
  if impl0.isNil:
    return nil
...
proc newTimeZone*(identifier: cstring = nil): TimeZone {.deprecated.}  =
  fnew(result, gBoxedFreeGTimeZone)
  result.impl = g_time_zone_new(identifier)

proc newTimeZone*(tdesc: typedesc; identifier: cstring = nil): tdesc {.deprecated.}  =
  assert(result is TimeZone)
  fnew(result, gBoxedFreeGTimeZone)
  result.impl = g_time_zone_new(identifier)
...
proc g_time_zone_adjust_time(self: ptr TimeZone00; `type`: TimeType; time: var int64): int32 {.
    importc, libprag.}

proc adjustTime*(self: TimeZone; `type`: TimeType; time: var int64): int =
...
proc g_byte_array_append(array: ptr ByteArray00; data: ptr uint8; len: uint32): ptr ByteArray00 {.
    importc, libprag.}

proc append*(array: ptr ByteArray00; data: ptr uint8; len: int): ptr ByteArray00 =
  g_byte_array_append(array, data, uint32(len))
...
proc g_byte_array_prepend(array: ptr ByteArray00; data: ptr uint8; len: uint32): ptr ByteArray00 {.
    importc, libprag.}

proc prepend*(array: ptr ByteArray00; data: ptr uint8; len: int): ptr ByteArray00 =
  g_byte_array_prepend(array, data, uint32(len))

proc `ref`*(array: ptr ByteArray00): ptr ByteArray00 {.
    importc: "g_byte_array_ref", libprag.}

proc g_byte_array_remove_index(array: ptr ByteArray00; index: uint32): ptr ByteArray00 {.
    importc, libprag.}

proc removeIndex*(array: ptr ByteArray00; index: int): ptr ByteArray00 =
  g_byte_array_remove_index(array, uint32(index))

proc g_byte_array_remove_index_fast(array: ptr ByteArray00; index: uint32): ptr ByteArray00 {.
    importc, libprag.}

proc removeIndexFast*(array: ptr ByteArray00; index: int): ptr ByteArray00 =
  g_byte_array_remove_index_fast(array, uint32(index))

proc g_byte_array_remove_range(array: ptr ByteArray00; index: uint32; length: uint32): ptr ByteArray00 {.
    importc, libprag.}

proc removeRange*(array: ptr ByteArray00; index: int; length: int): ptr ByteArray00 =
  g_byte_array_remove_range(array, uint32(index), uint32(length))

proc g_byte_array_set_size(array: ptr ByteArray00; length: uint32): ptr ByteArray00 {.
    importc, libprag.}

proc setSize*(array: ptr ByteArray00; length: int): ptr ByteArray00 =
  g_byte_array_set_size(array, uint32(length))

proc g_byte_array_sized_new(reservedSize: uint32): ptr ByteArray00 {.
    importc, libprag.}

proc sizedNew*(reservedSize: int): ptr ByteArray00 =
  g_byte_array_sized_new(uint32(reservedSize))
...
proc g_bytes_get_region(self: ptr Bytes00; elementSize: uint64; offset: uint64;
    nElements: uint64): pointer {.
    importc, libprag.}

proc getRegion*(self: Bytes; elementSize: uint64; offset: uint64;
    nElements: uint64): pointer =
  g_bytes_get_region(cast[ptr Bytes00](self.impl), elementSize, offset, nElements)
...
const C_STD_VERSION* = 199000'i32
...
proc g_cache_destroy(self: ptr Cache00) {.
    importc, libprag.}

proc destroy*(self: Cache) =
  g_cache_destroy(cast[ptr Cache00](self.impl))

proc finalizerdestroy*(self: Cache) =
  if not self.ignoreFinalizer:
    g_cache_destroy(cast[ptr Cache00](self.impl))
...
when defined(gcDestructors):
  proc `=destroy`*(self: var typeof(Cache()[])) =
    when defined(gintroDebug):
      echo "destroy ", $typeof(self), ' ', cast[int](unsafeaddr self)
    if not self.ignoreFinalizer and self.impl != nil:
      g_cache_destroy(self.impl)
      self.impl = nil

proc newWithFinalizer*(x: var Cache) =
  when defined(gcDestructors):
    new(x)
  else:
    new(x, finalizerdestroy)

proc g_cache_insert(self: ptr Cache00; key: pointer): pointer {.
    importc, libprag.}

proc insert*(self: Cache; key: pointer): pointer =
  g_cache_insert(cast[ptr Cache00](self.impl), key)

proc g_cache_remove(self: ptr Cache00; value: pointer) {.
    importc, libprag.}

proc remove*(self: Cache; value: pointer) =
  g_cache_remove(cast[ptr Cache00](self.impl), value)

type
  CacheDestroyFunc* = proc (value: pointer) {.cdecl.}

type
  CacheDupFunc* = proc (value: pointer): pointer {.cdecl.}

type
  CacheNewFunc* = proc (key: pointer): pointer {.cdecl.}
...
proc newChecksum*(checksumType: ChecksumType): Checksum =
  let impl0 = g_checksum_new(checksumType)
  if impl0.isNil:
    return nil
  fnew(result, gBoxedFreeGChecksum)
  result.impl = impl0
...
type
  ChildWatchFunc* = proc (pid: int32; waitStatus: int32; userData: pointer) {.cdecl.}
...
proc g_async_queue_push_sorted(self: ptr AsyncQueue00; data: pointer; `func`: CompareDataFunc;
    userData: pointer) {.
    importc, libprag.}

proc pushSorted*(self: AsyncQueue; data: pointer; `func`: CompareDataFunc;
    userData: pointer) =
  g_async_queue_push_sorted(cast[ptr AsyncQueue00](self.impl), data, `func`, userData)

proc g_async_queue_push_sorted_unlocked(self: ptr AsyncQueue00; data: pointer;
    `func`: CompareDataFunc; userData: pointer) {.
    importc, libprag.}

proc pushSortedUnlocked*(self: AsyncQueue; data: pointer;
    `func`: CompareDataFunc; userData: pointer) =
  g_async_queue_push_sorted_unlocked(cast[ptr AsyncQueue00](self.impl), data, `func`, userData)

proc g_async_queue_sort(self: ptr AsyncQueue00; `func`: CompareDataFunc;
    userData: pointer) {.
    importc, libprag.}

proc sort*(self: AsyncQueue; `func`: CompareDataFunc; userData: pointer) =
  g_async_queue_sort(cast[ptr AsyncQueue00](self.impl), `func`, userData)

proc g_async_queue_sort_unlocked(self: ptr AsyncQueue00; `func`: CompareDataFunc;
    userData: pointer) {.
    importc, libprag.}

proc sortUnlocked*(self: AsyncQueue; `func`: CompareDataFunc;
    userData: pointer) =
  g_async_queue_sort_unlocked(cast[ptr AsyncQueue00](self.impl), `func`, userData)

proc sortWithData*(array: ptr ByteArray00; compareFunc: CompareDataFunc;
    userData: pointer) {.
    importc: "g_byte_array_sort_with_data", libprag.}

type
  CompareFunc* = proc (a: pointer; b: pointer): int32 {.cdecl.}

proc sort*(array: ptr ByteArray00; compareFunc: CompareFunc) {.
    importc: "g_byte_array_sort", libprag.}

type
  Completion00* {.pure.} = object
...
proc free*(self: Completion) =
  g_completion_free(cast[ptr Completion00](self.impl))

proc finalizerfree*(self: Completion) =
  if not self.ignoreFinalizer:
    g_completion_free(cast[ptr Completion00](self.impl))
...
when defined(gcDestructors):
  proc `=destroy`*(self: var typeof(Completion()[])) =
    when defined(gintroDebug):
      echo "destroy ", $typeof(self), ' ', cast[int](unsafeaddr self)
    if not self.ignoreFinalizer and self.impl != nil:
      g_completion_free(self.impl)
      self.impl = nil

proc newWithFinalizer*(x: var Completion) =
  when defined(gcDestructors):
    new(x)
  else:
    new(x, finalizerfree)

proc g_completion_complete_utf8(self: ptr Completion00; prefix: cstring;
    newPrefix: cstring): ptr glib.List {.
    importc, libprag.}

proc completeUtf8*(self: Completion; prefix: cstring; newPrefix: cstring): seq[cstring] =
  discard

proc g_completion_clear_items(self: ptr Completion00) {.
    importc, libprag.}

proc clearItems*(self: Completion) =
  g_completion_clear_items(cast[ptr Completion00](self.impl))

type
  CompletionFunc* = proc (item: pointer): cstring {.cdecl.}

type
  CompletionStrncmpFunc* = proc (s1: cstring; s2: cstring; n: uint64): int32 {.cdecl.}
...
proc g_dir_get_type*(): GType {.importc, gobjectlibprag.}

proc g_dir_unref(self: Dir) {.
    importc, libprag.}
...
proc g_dir_ref(self: Dir): ptr Dir {.
    importc, libprag.}

proc `ref`*(self: Dir): ptr Dir =
  g_dir_ref(self)
...
proc g_dir_open(path: cstring; flags: uint32; error: ptr ptr glib.Error = nil): ptr Dir {.
    importc, libprag.}

proc open*(path: cstring; flags: int): ptr Dir =
  var gerror: ptr glib.Error
  let resul0 = g_dir_open(path, uint32(flags), addr gerror)
  if gerror != nil:
    let msg = $gerror.message
    g_error_free(gerror[])
    raise newException(GException, msg)
...
type
  EqualFuncFull* = proc (a: pointer; b: pointer; userData: pointer): gboolean {.cdecl.}

type
  ErrorClearFunc* = proc (error: ptr glib.Error) {.cdecl.}

type
  ErrorCopyFunc* = proc (srcError: ptr glib.Error; destError: ptr glib.Error) {.cdecl.}

type
  ErrorInitFunc* = proc (error: ptr glib.Error) {.cdecl.}

proc g_error_domain_register(errorTypeName: cstring; errorTypePrivateSize: uint64;
    errorTypeInit: ErrorInitFunc; errorTypeCopy: ErrorCopyFunc; errorTypeClear: ErrorClearFunc): uint32 {.
    importc, libprag.}

proc domainRegister*(errorTypeName: cstring; errorTypePrivateSize: uint64;
    errorTypeInit: ErrorInitFunc; errorTypeCopy: ErrorCopyFunc; errorTypeClear: ErrorClearFunc): int =
  int(g_error_domain_register(errorTypeName, errorTypePrivateSize, errorTypeInit, errorTypeCopy, errorTypeClear))

proc g_error_domain_register_static(errorTypeName: cstring; errorTypePrivateSize: uint64;
    errorTypeInit: ErrorInitFunc; errorTypeCopy: ErrorCopyFunc; errorTypeClear: ErrorClearFunc): uint32 {.
    importc, libprag.}

proc domainRegisterStatic*(errorTypeName: cstring; errorTypePrivateSize: uint64;
    errorTypeInit: ErrorInitFunc; errorTypeCopy: ErrorCopyFunc; errorTypeClear: ErrorClearFunc): int =
  int(g_error_domain_register_static(errorTypeName, errorTypePrivateSize, errorTypeInit, errorTypeCopy, errorTypeClear))
...
type
  FileTestFlag* {.size: sizeof(cint), pure.} = enum
    isRegular = 0
    isSymlink = 1
    isDir = 2
    isExecutable = 3
...
proc g_cache_key_foreach(self: ptr Cache00; `func`: HFunc; userData: pointer) {.
    importc, libprag.}

proc keyForeach*(self: Cache; `func`: HFunc; userData: pointer) =
  g_cache_key_foreach(cast[ptr Cache00](self.impl), `func`, userData)

proc g_cache_value_foreach(self: ptr Cache00; `func`: HFunc; userData: pointer) {.
    importc, libprag.}

proc valueForeach*(self: Cache; `func`: HFunc; userData: pointer) =
  g_cache_value_foreach(cast[ptr Cache00](self.impl), `func`, userData)
...
proc findHashTable*(hashTable: ptr HashTable00; predicate: HRFunc; userData: pointer): pointer {.
    importc: "g_hash_table_find", libprag.}

proc foreach*(hashTable: ptr HashTable00; `func`: HFunc; userData: pointer) {.
    importc: "g_hash_table_foreach", libprag.}

proc g_hash_table_foreach_remove(hashTable: ptr HashTable00; `func`: HRFunc;
    userData: pointer): uint32 {.
    importc, libprag.}

proc foreachRemove*(hashTable: ptr HashTable00; `func`: HRFunc;
    userData: pointer): int =
  int(g_hash_table_foreach_remove(hashTable, `func`, userData))

proc g_hash_table_foreach_steal(hashTable: ptr HashTable00; `func`: HRFunc;
    userData: pointer): uint32 {.
    importc, libprag.}

proc foreachSteal*(hashTable: ptr HashTable00; `func`: HRFunc;
    userData: pointer): int =
  int(g_hash_table_foreach_steal(hashTable, `func`, userData))
...
proc newSimilar*(otherHashTable: ptr HashTable00): ptr HashTable00 {.
    importc: "g_hash_table_new_similar", libprag.}

proc `ref`*(hashTable: ptr HashTable00): ptr HashTable00 {.
    importc: "g_hash_table_ref", libprag.}
...
proc g_hash_table_iter_get_hash_table(self: HashTableIter): ptr HashTable00 {.
    importc, libprag.}

proc getHashTable*(self: HashTableIter): ptr HashTable00 =
  g_hash_table_iter_get_hash_table(self)

proc hashTable*(self: HashTableIter): ptr HashTable00 =
  g_hash_table_iter_get_hash_table(self)
...
proc g_hmac_get_type*(): GType {.importc, gobjectlibprag.}

proc gBoxedFreeGHmac*(self: Hmac) =
  if not self.ignoreFinalizer:
    boxedFree(g_hmac_get_type(), cast[ptr Hmac00](self.impl))
...
proc newWithFinalizer*(x: var Hmac) =
  when defined(gcDestructors):
    ...
    new(x, gBoxedFreeGHmac)

proc g_hmac_unref(self: ptr Hmac00) {.
    importc, libprag.}

proc unref*(self: Hmac) =
  g_hmac_unref(cast[ptr Hmac00](g_boxed_copy(g_hmac_get_type(), self.impl)))

proc finalizerunref*(self: Hmac) =
  if not self.ignoreFinalizer:
    g_hmac_unref(cast[ptr Hmac00](self.impl))

proc g_hmac_copy(self: ptr Hmac00): ptr Hmac00 {.
    importc, libprag.}

proc copy*(self: Hmac): Hmac =
  fnew(result, gBoxedFreeGHmac)
  result.impl = g_hmac_copy(cast[ptr Hmac00](self.impl))
...
proc g_hmac_ref(self: ptr Hmac00): ptr Hmac00 {.
    importc, libprag.}

proc `ref`*(self: Hmac): Hmac =
  fnew(result, gBoxedFreeGHmac)
  result.impl = g_hmac_ref(cast[ptr Hmac00](self.impl))

proc g_hmac_new(digestType: ChecksumType; key: ptr uint8; keyLen: uint64): ptr Hmac00 {.
    importc, libprag.}

proc newHmac*(digestType: ChecksumType; key: seq[uint8] | string): Hmac =
  let keyLen = uint64(key.len)
  let impl0 = g_hmac_new(digestType, cast[ptr uint8](unsafeaddr(key[0])), keyLen)
  if impl0.isNil:
    return nil
  fnew(result, gBoxedFreeGHmac)
  result.impl = impl0

proc newHmac*(tdesc: typedesc; digestType: ChecksumType; key: seq[uint8] | string): tdesc =
  let keyLen = uint64(key.len)
  assert(result is Hmac)
  let impl0 = g_hmac_new(digestType, cast[ptr uint8](unsafeaddr(key[0])), keyLen)
  if impl0.isNil:
    return nil
  fnew(result, gBoxedFreeGHmac)
...
proc initHmac*[T](result: var T; digestType: ChecksumType; key: seq[uint8] | string) {.deprecated.} =
  let keyLen = uint64(key.len)
  assert(result is Hmac)
  let impl0 = g_hmac_new(digestType, cast[ptr uint8](unsafeaddr(key[0])), keyLen)
  if impl0.isNil:
...
proc g_hook_list_marshal_check(self: ptr HookList00; mayRecurse: gboolean;
    marshaller: HookCheckMarshaller; marshalData: pointer) {.
    importc, libprag.}

proc marshalCheck*(self: HookList; mayRecurse: bool; marshaller: HookCheckMarshaller;
    marshalData: pointer) =
  g_hook_list_marshal_check(cast[ptr HookList00](self.impl), gboolean(mayRecurse), marshaller, marshalData)

type
  HookCompareFunc* = proc (newHook: ptr Hook00; sibling: ptr Hook00): int32 {.cdecl.}

proc g_hook_insert_sorted(hookList: ptr HookList00; hook: ptr Hook00; `func`: HookCompareFunc) {.
    importc, libprag.}

proc insertSorted*(hookList: HookList; hook: Hook; `func`: HookCompareFunc) =
  g_hook_insert_sorted(cast[ptr HookList00](hookList.impl), cast[ptr Hook00](hook.impl), `func`)
...
proc g_hook_list_marshal(self: ptr HookList00; mayRecurse: gboolean; marshaller: HookMarshaller;
    marshalData: pointer) {.
    importc, libprag.}

proc marshal*(self: HookList; mayRecurse: bool; marshaller: HookMarshaller;
    marshalData: pointer) =
  g_hook_list_marshal(cast[ptr HookList00](self.impl), gboolean(mayRecurse), marshaller, marshalData)
...
proc g_io_channel_get_line_term(self: ptr IOChannel00; length: var int32): cstring {.
    importc, libprag.}

proc getLineTerm*(self: IOChannel; length: var int = cast[var int](nil)): string =
  var length_00: int32
  result = $g_io_channel_get_line_term(cast[ptr IOChannel00](self.impl), length_00)
  if length.addr != nil:
...
proc newLen*(init: cstring; len: int64): ptr String {.
    importc: "g_string_new_len", libprag.}

proc newTake*(init: cstring): ptr String {.
    importc: "g_string_new_take", libprag.}

proc sizedNew*(dflSize: uint64): ptr String {.
...
proc new*(init: cstring): ptr String {.
    importc: "g_string_new", libprag.}

proc g_string_free_and_steal(self: String): cstring {.
    importc, libprag.}

proc freeAndSteal*(self: String): string =
  let resul0 = g_string_free_and_steal(self)
  result = $resul0
  cogfree(resul0)
...
proc g_string_replace(self: String; find: cstring; replace: cstring; limit: uint32): uint32 {.
    importc, libprag.}

proc replace*(self: String; find: cstring; replace: cstring; limit: int): int =
  int(g_string_replace(self, find, replace, uint32(limit)))
...
proc getComment*(self: KeyFile; groupName: cstring = nil; key: cstring = nil): string =
  var gerror: ptr glib.Error
...
proc getThreadDefault*(): MainContext =
  let impl0 = g_main_context_get_thread_default()
  if impl0.isNil:
    return nil
  fnew(result, gBoxedFreeGMainContext)
...
proc none*(t: typedesc[MainContextFlags]): MainContextFlags = MainContextFlags({})

proc g_main_context_new_with_flags(flags: MainContextFlags): ptr MainContext00 {.
    importc, libprag.}

proc newMainContextWithFlags*(flags: MainContextFlags): MainContext =
  fnew(result, gBoxedFreeGMainContext)
  result.impl = g_main_context_new_with_flags(flags)

proc newMainContextWithFlags*(tdesc: typedesc; flags: MainContextFlags): tdesc =
  assert(result is MainContext)
  fnew(result, gBoxedFreeGMainContext)
  result.impl = g_main_context_new_with_flags(flags)

proc initMainContextWithFlags*[T](result: var T; flags: MainContextFlags) {.deprecated.} =
  assert(result is MainContext)
  fnew(result, gBoxedFreeGMainContext)
  result.impl = g_main_context_new_with_flags(flags)
...
proc g_source_set_static_name(self: ptr Source00; name: cstring) {.
    importc, libprag.}

proc setStaticName*(self: Source; name: cstring) =
  g_source_set_static_name(cast[ptr Source00](self.impl), name)

proc `staticName=`*(self: Source; name: cstring) =
  g_source_set_static_name(cast[ptr Source00](self.impl), name)
...
proc getElementStack*(self: MarkupParseContext): seq[cstring] =
  discard

proc elementStack*(self: MarkupParseContext): seq[cstring] =
  discard
...
proc defaultFlags*(t: typedesc[MarkupParseFlags]): MarkupParseFlags = MarkupParseFlags({})
...
proc g_regex_escape_string(string: cstring; length: int32): cstring {.
    importc, libprag.}

proc escapeString*(string: cstring; length: int): string =
...
type
  MemChunk00* {.pure.} = object
  MemChunk* = ref object
    impl*: ptr MemChunk00
    ignoreFinalizer*: bool

proc g_mem_chunk_free(self: ptr MemChunk00; mem: pointer) {.
    importc, libprag.}

proc free*(self: MemChunk; mem: pointer) =
  g_mem_chunk_free(cast[ptr MemChunk00](self.impl), mem)

proc g_mem_chunk_alloc0(self: ptr MemChunk00): pointer {.
    importc, libprag.}

proc alloc0*(self: MemChunk): pointer =
  g_mem_chunk_alloc0(cast[ptr MemChunk00](self.impl))

proc g_mem_chunk_clean(self: ptr MemChunk00) {.
    importc, libprag.}

proc clean*(self: MemChunk) =
  g_mem_chunk_clean(cast[ptr MemChunk00](self.impl))

proc g_mem_chunk_destroy(self: ptr MemChunk00) {.
    importc, libprag.}

proc destroy*(self: MemChunk) =
  g_mem_chunk_destroy(cast[ptr MemChunk00](self.impl))

proc finalizerdestroy*(self: MemChunk) =
  if not self.ignoreFinalizer:
    g_mem_chunk_destroy(cast[ptr MemChunk00](self.impl))

proc g_mem_chunk_alloc(self: ptr MemChunk00): pointer {.
    importc, libprag.}

proc alloc*(self: MemChunk): pointer =
  g_mem_chunk_alloc(cast[ptr MemChunk00](self.impl))

proc g_mem_chunk_print(self: ptr MemChunk00) {.
    importc, libprag.}

proc print*(self: MemChunk) =
  g_mem_chunk_print(cast[ptr MemChunk00](self.impl))

proc g_mem_chunk_reset(self: ptr MemChunk00) {.
    importc, libprag.}

proc reset*(self: MemChunk) =
  g_mem_chunk_reset(cast[ptr MemChunk00](self.impl))

proc info*() {.
    importc: "g_mem_chunk_info", libprag.}
...
proc g_node_children_foreach(self: ptr Node00; flags: TraverseFlags; `func`: NodeForeachFunc;
    data: pointer) {.
    importc, libprag.}

proc childrenForeach*(self: Node; flags: TraverseFlags; `func`: NodeForeachFunc;
    data: pointer) =
  g_node_children_foreach(cast[ptr Node00](self.impl), flags, `func`, data)

type
  NodeTraverseFunc* = proc (node: ptr Node00; data: pointer): gboolean {.cdecl.}

proc g_node_traverse(self: ptr Node00; order: TraverseType; flags: TraverseFlags;
    maxDepth: int32; `func`: NodeTraverseFunc; data: pointer) {.
    importc, libprag.}

proc traverse*(self: Node; order: TraverseType; flags: TraverseFlags;
    maxDepth: int; `func`: NodeTraverseFunc; data: pointer) =
  g_node_traverse(cast[ptr Node00](self.impl), order, flags, int32(maxDepth), `func`, data)
...
proc g_once_init_enter(location: var pointer): gboolean {.
    importc, libprag.}

proc initEnter*(location: var pointer): bool =
  toBool(g_once_init_enter(location))

proc g_once_init_enter_impl(location: ptr uint64): gboolean {.
    importc, libprag.}

proc initEnterImpl*(location: ptr uint64): bool =
  toBool(g_once_init_enter_impl(location))

proc g_once_init_enter_pointer(location: pointer): gboolean {.
    importc, libprag.}

proc initEnterPointer*(location: pointer): bool =
  toBool(g_once_init_enter_pointer(location))

proc initLeave*(location: var pointer; resu: uint64) {.
    importc: "g_once_init_leave", libprag.}

proc initLeavePointer*(location: pointer; resu: pointer) {.
    importc: "g_once_init_leave_pointer", libprag.}
...
type
  PathBuf00* {.pure.} = object
  PathBuf* = ref object
    impl*: ptr PathBuf00
    ignoreFinalizer*: bool

proc g_path_buf_free(self: ptr PathBuf00) {.
    importc, libprag.}

proc free*(self: PathBuf) =
  g_path_buf_free(cast[ptr PathBuf00](self.impl))

proc finalizerfree*(self: PathBuf) =
  if not self.ignoreFinalizer:
    g_path_buf_free(cast[ptr PathBuf00](self.impl))

when defined(gcDestructors):
  proc `=destroy`*(self: var typeof(PathBuf()[])) =
    when defined(gintroDebug):
      echo "destroy ", $typeof(self), ' ', cast[int](unsafeaddr self)
    if not self.ignoreFinalizer and self.impl != nil:
      g_path_buf_free(self.impl)
      self.impl = nil

proc newWithFinalizer*(x: var PathBuf) =
  when defined(gcDestructors):
    new(x)
  else:
    new(x, finalizerfree)

proc g_path_buf_clear_to_path(self: ptr PathBuf00): cstring {.
    importc, libprag.}

proc clearToPath*(self: PathBuf): string =
  let resul0 = g_path_buf_clear_to_path(cast[ptr PathBuf00](self.impl))
  if resul0.isNil:
    return
  result = $resul0
  cogfree(resul0)

proc g_path_buf_clear(self: ptr PathBuf00) {.
    importc, libprag.}

proc clear*(self: PathBuf) =
  g_path_buf_clear(cast[ptr PathBuf00](self.impl))

proc g_path_buf_free_to_path(self: ptr PathBuf00): cstring {.
    importc, libprag.}

proc freeToPath*(self: PathBuf): string =
  let resul0 = g_path_buf_free_to_path(cast[ptr PathBuf00](self.impl))
  if resul0.isNil:
    return
  result = $resul0
  cogfree(resul0)

proc g_path_buf_init(self: ptr PathBuf00): ptr PathBuf00 {.
    importc, libprag.}

proc init*(self: PathBuf): PathBuf =
  fnew(result, finalizerfree)
  result.impl = g_path_buf_init(cast[ptr PathBuf00](self.impl))
  result.ignoreFinalizer = true

proc g_path_buf_init_from_path(self: ptr PathBuf00; path: cstring): ptr PathBuf00 {.
    importc, libprag.}

proc initFromPath*(self: PathBuf; path: cstring = nil): PathBuf =
  fnew(result, finalizerfree)
  result.impl = g_path_buf_init_from_path(cast[ptr PathBuf00](self.impl), path)
  result.ignoreFinalizer = true

proc g_path_buf_pop(self: ptr PathBuf00): gboolean {.
    importc, libprag.}

proc pop*(self: PathBuf): bool =
  toBool(g_path_buf_pop(cast[ptr PathBuf00](self.impl)))

proc g_path_buf_push(self: ptr PathBuf00; path: cstring): ptr PathBuf00 {.
    importc, libprag.}

proc push*(self: PathBuf; path: cstring): PathBuf =
  fnew(result, finalizerfree)
  result.impl = g_path_buf_push(cast[ptr PathBuf00](self.impl), path)
  result.ignoreFinalizer = true

proc g_path_buf_set_extension(self: ptr PathBuf00; extension: cstring): gboolean {.
    importc, libprag.}

proc setExtension*(self: PathBuf; extension: cstring = nil): bool =
  toBool(g_path_buf_set_extension(cast[ptr PathBuf00](self.impl), extension))

proc g_path_buf_set_filename(self: ptr PathBuf00; fileName: cstring): gboolean {.
    importc, libprag.}

proc setFilename*(self: PathBuf; fileName: cstring): bool =
  toBool(g_path_buf_set_filename(cast[ptr PathBuf00](self.impl), fileName))

proc g_path_buf_to_path(self: ptr PathBuf00): cstring {.
    importc, libprag.}

proc toPath*(self: PathBuf): string =
  let resul0 = g_path_buf_to_path(cast[ptr PathBuf00](self.impl))
  if resul0.isNil:
    return
  result = $resul0
  cogfree(resul0)

proc g_path_buf_equal(v1: pointer; v2: pointer): gboolean {.
    importc, libprag.}

proc equal*(v1: pointer; v2: pointer): bool =
  toBool(g_path_buf_equal(v1, v2))
...
proc g_pattern_spec_get_type*(): GType {.importc, gobjectlibprag.}

proc gBoxedFreeGPatternSpec*(self: PatternSpec) =
  if not self.ignoreFinalizer:
    boxedFree(g_pattern_spec_get_type(), cast[ptr PatternSpec00](self.impl))
...
proc g_pattern_spec_copy(self: ptr PatternSpec00): ptr PatternSpec00 {.
    importc, libprag.}

proc copy*(self: PatternSpec): PatternSpec =
  fnew(result, gBoxedFreeGPatternSpec)
  result.impl = g_pattern_spec_copy(cast[ptr PatternSpec00](self.impl))
...
proc g_pattern_spec_new(pattern: cstring): ptr PatternSpec00 {.
    importc, libprag.}

proc newPatternSpec*(pattern: cstring): PatternSpec =
  fnew(result, gBoxedFreeGPatternSpec)
  result.impl = g_pattern_spec_new(pattern)

proc newPatternSpec*(tdesc: typedesc; pattern: cstring): tdesc =
  assert(result is PatternSpec)
  fnew(result, gBoxedFreeGPatternSpec)
  result.impl = g_pattern_spec_new(pattern)

proc initPatternSpec*[T](result: var T; pattern: cstring) {.deprecated.} =
  assert(result is PatternSpec)
  fnew(result, gBoxedFreeGPatternSpec)
  result.impl = g_pattern_spec_new(pattern)

proc g_pattern_spec_match(self: ptr PatternSpec00; stringLength: uint64;
    string: cstring; stringReversed: cstring): gboolean {.
    importc, libprag.}

proc match*(self: PatternSpec; stringLength: uint64; string: cstring;
    stringReversed: cstring = nil): bool =
  toBool(g_pattern_spec_match(cast[ptr PatternSpec00](self.impl), stringLength, string, stringReversed))

proc g_pattern_spec_match_string(self: ptr PatternSpec00; string: cstring): gboolean {.
    importc, libprag.}

proc matchString*(self: PatternSpec; string: cstring): bool =
  toBool(g_pattern_spec_match_string(cast[ptr PatternSpec00](self.impl), string))
...
proc g_queue_foreach(self: ptr Queue00; `func`: Func; userData: pointer) {.
    importc, libprag.}

proc foreach*(self: Queue; `func`: Func; userData: pointer) =
  g_queue_foreach(cast[ptr Queue00](self.impl), `func`, userData)
...
proc g_queue_insert_sorted(self: ptr Queue00; data: pointer; `func`: CompareDataFunc;
    userData: pointer) {.
    importc, libprag.}

proc insertSorted*(self: Queue; data: pointer; `func`: CompareDataFunc;
    userData: pointer) =
  g_queue_insert_sorted(cast[ptr Queue00](self.impl), data, `func`, userData)
...
proc g_queue_sort(self: ptr Queue00; compareFunc: CompareDataFunc; userData: pointer) {.
    importc, libprag.}

proc sort*(self: Queue; compareFunc: CompareDataFunc; userData: pointer) =
  g_queue_sort(cast[ptr Queue00](self.impl), compareFunc, userData)

const REF_COUNT_INIT* = -1'i32
...
proc g_rand_get_type*(): GType {.importc, gobjectlibprag.}

proc gBoxedFreeGRand*(self: Rand) =
  if not self.ignoreFinalizer:
    boxedFree(g_rand_get_type(), cast[ptr Rand00](self.impl))
...
proc g_rand_new_with_seed(seed: uint32): ptr Rand00 {.
    importc, libprag.}

proc newRandWithSeed*(seed: int): Rand =
  fnew(result, gBoxedFreeGRand)
  result.impl = g_rand_new_with_seed(uint32(seed))

proc newRandWithSeed*(tdesc: typedesc; seed: int): tdesc =
  assert(result is Rand)
  fnew(result, gBoxedFreeGRand)
  result.impl = g_rand_new_with_seed(uint32(seed))

proc initRandWithSeed*[T](result: var T; seed: int) {.deprecated.} =
  assert(result is Rand)
  fnew(result, gBoxedFreeGRand)
  result.impl = g_rand_new_with_seed(uint32(seed))

proc g_rand_new_with_seed_array(seed: ptr uint32; seedLength: uint32): ptr Rand00 {.
    importc, libprag.}

proc newRandWithSeedArray*(seed: ptr uint32; seedLength: int): Rand =
  fnew(result, gBoxedFreeGRand)
  result.impl = g_rand_new_with_seed_array(seed, uint32(seedLength))

proc newRandWithSeedArray*(tdesc: typedesc; seed: ptr uint32; seedLength: int): tdesc =
  assert(result is Rand)
  fnew(result, gBoxedFreeGRand)
  result.impl = g_rand_new_with_seed_array(seed, uint32(seedLength))

proc initRandWithSeedArray*[T](result: var T; seed: ptr uint32; seedLength: int) {.deprecated.} =
  assert(result is Rand)
  fnew(result, gBoxedFreeGRand)
  result.impl = g_rand_new_with_seed_array(seed, uint32(seedLength))

proc g_rand_copy(self: ptr Rand00): ptr Rand00 {.
    importc, libprag.}

proc copy*(self: Rand): Rand =
  fnew(result, gBoxedFreeGRand)
  result.impl = g_rand_copy(cast[ptr Rand00](self.impl))
...
proc g_rand_new(): ptr Rand00 {.
    importc, libprag.}

proc newRand*(): Rand =
  fnew(result, gBoxedFreeGRand)
  result.impl = g_rand_new()

proc newRand*(tdesc: typedesc): tdesc =
  assert(result is Rand)
  fnew(result, gBoxedFreeGRand)
  result.impl = g_rand_new()

proc initRand*[T](result: var T) {.deprecated.} =
  assert(result is Rand)
  fnew(result, gBoxedFreeGRand)
  result.impl = g_rand_new()
...
proc g_regex_replace_eval(self: ptr Regex00; string: ptr cstring; stringLen: int64;
    startPosition: int32; matchOptions: RegexMatchFlags; eval: RegexEvalCallback;
    userData: pointer; error: ptr ptr glib.Error = nil): cstring {.
    importc, libprag.}

proc replaceEval*(self: Regex; string: openArray[string]; stringLen: int64;
    startPosition: int; matchOptions: RegexMatchFlags; eval: RegexEvalCallback;
    userData: pointer): string =
  var fs469n23x: array[256, pointer]
  var fs469n23: cstringArray = cast[cstringArray](addr fs469n23x)
  var gerror: ptr glib.Error
  let resul0 = g_regex_replace_eval(cast[ptr Regex00](self.impl), seq2CstringArray(string, fs469n23), stringLen, int32(startPosition), matchOptions, eval, userData, addr gerror)
  if gerror != nil:
    let msg = $gerror.message
    g_error_free(gerror[])
    raise newException(GException, msg)
...
type
  Relation00* {.pure.} = object
  Relation* = ref object
    impl*: ptr Relation00
    ignoreFinalizer*: bool

proc g_relation_destroy(self: ptr Relation00) {.
    importc, libprag.}

proc destroy*(self: Relation) =
  g_relation_destroy(cast[ptr Relation00](self.impl))

proc finalizerdestroy*(self: Relation) =
  if not self.ignoreFinalizer:
    g_relation_destroy(cast[ptr Relation00](self.impl))
...
when defined(gcDestructors):
  proc `=destroy`*(self: var typeof(Relation()[])) =
    when defined(gintroDebug):
      echo "destroy ", $typeof(self), ' ', cast[int](unsafeaddr self)
    if not self.ignoreFinalizer and self.impl != nil:
      g_relation_destroy(self.impl)
      self.impl = nil

proc newWithFinalizer*(x: var Relation) =
  when defined(gcDestructors):
    new(x)
  else:
    new(x, finalizerdestroy)

proc g_relation_delete(self: ptr Relation00; key: pointer; field: int32): int32 {.
    importc, libprag.}

proc delete*(self: Relation; key: pointer; field: int): int =
  int(g_relation_delete(cast[ptr Relation00](self.impl), key, int32(field)))

proc g_relation_count(self: ptr Relation00; key: pointer; field: int32): int32 {.
    importc, libprag.}

proc count*(self: Relation; key: pointer; field: int): int =
  int(g_relation_count(cast[ptr Relation00](self.impl), key, int32(field)))

proc g_relation_print(self: ptr Relation00) {.
    importc, libprag.}

proc print*(self: Relation) =
  g_relation_print(cast[ptr Relation00](self.impl))
...
proc g_scanner_scope_foreach_symbol(self: ptr Scanner00; scopeId: uint32;
    `func`: HFunc; userData: pointer) {.
    importc, libprag.}

proc scopeForeachSymbol*(self: Scanner; scopeId: int; `func`: HFunc;
    userData: pointer) =
  g_scanner_scope_foreach_symbol(cast[ptr Scanner00](self.impl), uint32(scopeId), `func`, userData)
...
proc g_sequence_foreach(self: ptr Sequence00; `func`: Func; userData: pointer) {.
    importc, libprag.}

proc foreach*(self: Sequence; `func`: Func; userData: pointer) =
  g_sequence_foreach(cast[ptr Sequence00](self.impl), `func`, userData)
...
proc g_sequence_sort(self: ptr Sequence00; cmpFunc: CompareDataFunc; cmpData: pointer) {.
    importc, libprag.}

proc sort*(self: Sequence; cmpFunc: CompareDataFunc; cmpData: pointer) =
  g_sequence_sort(cast[ptr Sequence00](self.impl), cmpFunc, cmpData)
...
proc g_sequence_insert_sorted(self: ptr Sequence00; data: pointer; cmpFunc: CompareDataFunc;
    cmpData: pointer): ptr SequenceIter {.
    importc, libprag.}

proc insertSorted*(self: Sequence; data: pointer; cmpFunc: CompareDataFunc;
    cmpData: pointer): ptr SequenceIter =
  g_sequence_insert_sorted(cast[ptr Sequence00](self.impl), data, cmpFunc, cmpData)

proc g_sequence_lookup(self: ptr Sequence00; data: pointer; cmpFunc: CompareDataFunc;
    cmpData: pointer): ptr SequenceIter {.
    importc, libprag.}

proc lookup*(self: Sequence; data: pointer; cmpFunc: CompareDataFunc;
    cmpData: pointer): ptr SequenceIter =
  g_sequence_lookup(cast[ptr Sequence00](self.impl), data, cmpFunc, cmpData)
...
proc g_sequence_search(self: ptr Sequence00; data: pointer; cmpFunc: CompareDataFunc;
    cmpData: pointer): ptr SequenceIter {.
    importc, libprag.}

proc search*(self: Sequence; data: pointer; cmpFunc: CompareDataFunc;
    cmpData: pointer): ptr SequenceIter =
  g_sequence_search(cast[ptr Sequence00](self.impl), data, cmpFunc, cmpData)

proc foreachRange*(begin: SequenceIter; `end`: SequenceIter; `func`: Func;
    userData: pointer) {.
    importc: "g_sequence_foreach_range", libprag.}
...
proc sortChanged*(iter: SequenceIter; cmpFunc: CompareDataFunc;
    cmpData: pointer) {.
    importc: "g_sequence_sort_changed", libprag.}
...
proc g_sequence_insert_sorted_iter(self: ptr Sequence00; data: pointer; iterCmp: SequenceIterCompareFunc;
    cmpData: pointer): ptr SequenceIter {.
    importc, libprag.}

proc insertSortedIter*(self: Sequence; data: pointer; iterCmp: SequenceIterCompareFunc;
    cmpData: pointer): ptr SequenceIter =
  g_sequence_insert_sorted_iter(cast[ptr Sequence00](self.impl), data, iterCmp, cmpData)

proc g_sequence_lookup_iter(self: ptr Sequence00; data: pointer; iterCmp: SequenceIterCompareFunc;
    cmpData: pointer): ptr SequenceIter {.
    importc, libprag.}

proc lookupIter*(self: Sequence; data: pointer; iterCmp: SequenceIterCompareFunc;
    cmpData: pointer): ptr SequenceIter =
  g_sequence_lookup_iter(cast[ptr Sequence00](self.impl), data, iterCmp, cmpData)

proc g_sequence_search_iter(self: ptr Sequence00; data: pointer; iterCmp: SequenceIterCompareFunc;
    cmpData: pointer): ptr SequenceIter {.
    importc, libprag.}

proc searchIter*(self: Sequence; data: pointer; iterCmp: SequenceIterCompareFunc;
    cmpData: pointer): ptr SequenceIter =
  g_sequence_search_iter(cast[ptr Sequence00](self.impl), data, iterCmp, cmpData)

proc g_sequence_sort_iter(self: ptr Sequence00; cmpFunc: SequenceIterCompareFunc;
    cmpData: pointer) {.
    importc, libprag.}

proc sortIter*(self: Sequence; cmpFunc: SequenceIterCompareFunc;
    cmpData: pointer) =
  g_sequence_sort_iter(cast[ptr Sequence00](self.impl), cmpFunc, cmpData)

proc sortChangedIter*(iter: SequenceIter; iterCmp: SequenceIterCompareFunc;
    cmpData: pointer) {.
    importc: "g_sequence_sort_changed_iter", libprag.}
...
type
  SourceOnceFunc* = proc (userData: pointer) {.cdecl.}

type
  SpawnChildSetupFunc* = proc (data: pointer) {.cdecl.}
...
type
  StrvBuilder00* {.pure.} = object
  StrvBuilder* = ref object
    impl*: ptr StrvBuilder00
    ignoreFinalizer*: bool

proc g_strv_builder_get_type*(): GType {.importc, gobjectlibprag.}

proc gBoxedFreeGStrvBuilder*(self: StrvBuilder) =
  if not self.ignoreFinalizer:
    boxedFree(g_strv_builder_get_type(), cast[ptr StrvBuilder00](self.impl))

when defined(gcDestructors):
  proc `=destroy`*(self: var typeof(StrvBuilder()[])) =
    when defined(gintroDebug):
      echo "destroy ", $typeof(self), ' ', cast[int](unsafeaddr self)
    if not self.ignoreFinalizer and self.impl != nil:
      boxedFree(g_strv_builder_get_type(), cast[ptr StrvBuilder00](self.impl))
...
proc newWithFinalizer*(x: var StrvBuilder) =
  when defined(gcDestructors):
    new(x)
  else:
    new(x, gBoxedFreeGStrvBuilder)

proc g_strv_builder_unref(self: ptr StrvBuilder00) {.
    importc, libprag.}

proc unref*(self: StrvBuilder) =
  g_strv_builder_unref(cast[ptr StrvBuilder00](g_boxed_copy(g_strv_builder_get_type(), self.impl)))

proc finalizerunref*(self: StrvBuilder) =
  if not self.ignoreFinalizer:
    g_strv_builder_unref(cast[ptr StrvBuilder00](self.impl))

proc g_strv_builder_add(self: ptr StrvBuilder00; value: cstring) {.
    importc, libprag.}

proc add*(self: StrvBuilder; value: cstring) =
  g_strv_builder_add(cast[ptr StrvBuilder00](self.impl), value)

proc g_strv_builder_addv(self: ptr StrvBuilder00; value: ptr cstring) {.
    importc, libprag.}

proc addv*(self: StrvBuilder; value: varargs[string, `$`]) =
  var fs469n23x: array[256, pointer]
  var fs469n23: cstringArray = cast[cstringArray](addr fs469n23x)
  g_strv_builder_addv(cast[ptr StrvBuilder00](self.impl), seq2CstringArray(value, fs469n23))

proc g_strv_builder_end(self: ptr StrvBuilder00): ptr cstring {.
    importc, libprag.}

proc `end`*(self: StrvBuilder): seq[string] =
  let resul0 = g_strv_builder_end(cast[ptr StrvBuilder00](self.impl))
  if resul0.isNil:
    return
  result = cstringArrayToSeq(resul0)
  g_strfreev(resul0)

proc g_strv_builder_ref(self: ptr StrvBuilder00): ptr StrvBuilder00 {.
    importc, libprag.}

proc `ref`*(self: StrvBuilder): StrvBuilder =
  fnew(result, gBoxedFreeGStrvBuilder)
  result.impl = g_strv_builder_ref(cast[ptr StrvBuilder00](self.impl))

proc g_strv_builder_take(self: ptr StrvBuilder00; value: cstring) {.
    importc, libprag.}

proc take*(self: StrvBuilder; value: cstring) =
  g_strv_builder_take(cast[ptr StrvBuilder00](self.impl), value)

proc g_strv_builder_new(): ptr StrvBuilder00 {.
    importc, libprag.}

proc newStrvBuilder*(): StrvBuilder =
  fnew(result, gBoxedFreeGStrvBuilder)
  result.impl = g_strv_builder_new()

proc newStrvBuilder*(tdesc: typedesc): tdesc =
  assert(result is StrvBuilder)
  fnew(result, gBoxedFreeGStrvBuilder)
  result.impl = g_strv_builder_new()

proc initStrvBuilder*[T](result: var T) {.deprecated.} =
  assert(result is StrvBuilder)
  fnew(result, gBoxedFreeGStrvBuilder)
  result.impl = g_strv_builder_new()
...
proc g_test_case_free(self: ptr TestCase00) {.
    importc, libprag.}

proc free*(self: TestCase) =
  g_test_case_free(cast[ptr TestCase00](self.impl))

proc finalizerfree*(self: TestCase) =
  if not self.ignoreFinalizer:
    g_test_case_free(cast[ptr TestCase00](self.impl))

when defined(gcDestructors):
  proc `=destroy`*(self: var typeof(TestCase()[])) =
    when defined(gintroDebug):
      echo "destroy ", $typeof(self), ' ', cast[int](unsafeaddr self)
    if not self.ignoreFinalizer and self.impl != nil:
      g_test_case_free(self.impl)
      self.impl = nil

proc newWithFinalizer*(x: var TestCase) =
  when defined(gcDestructors):
    new(x)
  else:
    new(x, finalizerfree)
...
proc default*(t: typedesc[TestSubprocessFlags]): TestSubprocessFlags = TestSubprocessFlags({})
...
proc g_test_suite_free(self: ptr TestSuite00) {.
    importc, libprag.}

proc free*(self: TestSuite) =
  g_test_suite_free(cast[ptr TestSuite00](self.impl))

proc finalizerfree*(self: TestSuite) =
  if not self.ignoreFinalizer:
    g_test_suite_free(cast[ptr TestSuite00](self.impl))

when defined(gcDestructors):
  proc `=destroy`*(self: var typeof(TestSuite()[])) =
    when defined(gintroDebug):
      echo "destroy ", $typeof(self), ' ', cast[int](unsafeaddr self)
    if not self.ignoreFinalizer and self.impl != nil:
      g_test_suite_free(self.impl)
      self.impl = nil

proc newWithFinalizer*(x: var TestSuite) =
  when defined(gcDestructors):
    new(x)
  else:
    new(x, finalizerfree)
...
proc default*(t: typedesc[TestTrapFlags]): TestTrapFlags = TestTrapFlags({})
...
type
  TreeNode00* {.pure.} = object
  TreeNode* = ref object
    impl*: ptr TreeNode00
    ignoreFinalizer*: bool

proc g_tree_node_key(self: ptr TreeNode00): pointer {.
    importc, libprag.}

proc key*(self: TreeNode): pointer =
  g_tree_node_key(cast[ptr TreeNode00](self.impl))

proc g_tree_node_next(self: ptr TreeNode00): ptr TreeNode00 {.
    importc, libprag.}

proc next*(self: TreeNode): TreeNode =
  let impl0 = g_tree_node_next(cast[ptr TreeNode00](self.impl))
  if impl0.isNil:
    return nil
  new(result)
  result.impl = impl0
...
proc g_tree_node_previous(self: ptr TreeNode00): ptr TreeNode00 {.
    importc, libprag.}

proc previous*(self: TreeNode): TreeNode =
  let impl0 = g_tree_node_previous(cast[ptr TreeNode00](self.impl))
  if impl0.isNil:
    return nil
  new(result)
  result.impl = impl0
...
proc g_tree_node_value(self: ptr TreeNode00): pointer {.
    importc, libprag.}

proc value*(self: TreeNode): pointer =
  g_tree_node_value(cast[ptr TreeNode00](self.impl))

type
  TraverseNodeFunc* = proc (node: ptr TreeNode00; data: pointer): gboolean {.cdecl.}
...
proc g_tree_get_type*(): GType {.importc, gobjectlibprag.}

proc gBoxedFreeGTree*(self: Tree) =
  if not self.ignoreFinalizer:
    boxedFree(g_tree_get_type(), cast[ptr Tree00](self.impl))
...
proc g_tree_foreach(self: ptr Tree00; `func`: TraverseFunc; userData: pointer) {.
    importc, libprag.}

proc foreach*(self: Tree; `func`: TraverseFunc; userData: pointer) =
  g_tree_foreach(cast[ptr Tree00](self.impl), `func`, userData)

proc g_tree_foreach_node(self: ptr Tree00; `func`: TraverseNodeFunc; userData: pointer) {.
    importc, libprag.}

proc foreachNode*(self: Tree; `func`: TraverseNodeFunc; userData: pointer) =
  g_tree_foreach_node(cast[ptr Tree00](self.impl), `func`, userData)
...
proc g_tree_insert_node(self: ptr Tree00; key: pointer; value: pointer): ptr TreeNode00 {.
    importc, libprag.}

proc insertNode*(self: Tree; key: pointer; value: pointer): TreeNode =
  let impl0 = g_tree_insert_node(cast[ptr Tree00](self.impl), key, value)
  if impl0.isNil:
    return nil
  new(result)
  result.impl = impl0
...
proc g_tree_lookup_node(self: ptr Tree00; key: pointer): ptr TreeNode00 {.
    importc, libprag.}

proc lookupNode*(self: Tree; key: pointer): TreeNode =
  let impl0 = g_tree_lookup_node(cast[ptr Tree00](self.impl), key)
  if impl0.isNil:
    return nil
  new(result)
  result.impl = impl0
...
proc g_tree_lower_bound(self: ptr Tree00; key: pointer): ptr TreeNode00 {.
    importc, libprag.}

proc lowerBound*(self: Tree; key: pointer): TreeNode =
  let impl0 = g_tree_lower_bound(cast[ptr Tree00](self.impl), key)
  if impl0.isNil:
    return nil
  new(result)
  result.impl = impl0
...
proc g_tree_node_first(self: ptr Tree00): ptr TreeNode00 {.
    importc, libprag.}

proc nodeFirst*(self: Tree): TreeNode =
  let impl0 = g_tree_node_first(cast[ptr Tree00](self.impl))
  if impl0.isNil:
    return nil
  new(result)
  result.impl = impl0
...
proc g_tree_node_last(self: ptr Tree00): ptr TreeNode00 {.
    importc, libprag.}

proc nodeLast*(self: Tree): TreeNode =
  let impl0 = g_tree_node_last(cast[ptr Tree00](self.impl))
  if impl0.isNil:
    return nil
  new(result)
  result.impl = impl0
...
proc g_tree_ref(self: ptr Tree00): ptr Tree00 {.
    importc, libprag.}

proc `ref`*(self: Tree): Tree =
  fnew(result, gBoxedFreeGTree)
  result.impl = g_tree_ref(cast[ptr Tree00](self.impl))
...
proc g_tree_remove_all(self: ptr Tree00) {.
    importc, libprag.}

proc removeAll*(self: Tree) =
  g_tree_remove_all(cast[ptr Tree00](self.impl))
...
proc g_tree_replace_node(self: ptr Tree00; key: pointer; value: pointer): ptr TreeNode00 {.
    importc, libprag.}

proc replaceNode*(self: Tree; key: pointer; value: pointer): TreeNode =
  let impl0 = g_tree_replace_node(cast[ptr Tree00](self.impl), key, value)
  if impl0.isNil:
    return nil
  new(result)
  result.impl = impl0
...
proc g_tree_search(self: ptr Tree00; searchFunc: CompareFunc; userData: pointer): pointer {.
    importc, libprag.}

proc search*(self: Tree; searchFunc: CompareFunc; userData: pointer): pointer =
  g_tree_search(cast[ptr Tree00](self.impl), searchFunc, userData)

proc g_tree_search_node(self: ptr Tree00; searchFunc: CompareFunc; userData: pointer): ptr TreeNode00 {.
    importc, libprag.}

proc searchNode*(self: Tree; searchFunc: CompareFunc; userData: pointer): TreeNode =
  let impl0 = g_tree_search_node(cast[ptr Tree00](self.impl), searchFunc, userData)
  if impl0.isNil:
    return nil
  new(result)
  result.impl = impl0
...
proc g_tree_traverse(self: ptr Tree00; traverseFunc: TraverseFunc; traverseType: TraverseType;
    userData: pointer) {.
    importc, libprag.}

proc traverse*(self: Tree; traverseFunc: TraverseFunc; traverseType: TraverseType;
    userData: pointer) =
  g_tree_traverse(cast[ptr Tree00](self.impl), traverseFunc, traverseType, userData)

proc g_tree_new_full(keyCompareFunc: CompareDataFunc; keyCompareData: pointer;
    keyDestroyFunc: DestroyNotify; valueDestroyFunc: DestroyNotify): ptr Tree00 {.
    importc, libprag.}

proc newTreeFull*(keyCompareFunc: CompareDataFunc; keyCompareData: pointer;
    keyDestroyFunc: DestroyNotify; valueDestroyFunc: DestroyNotify): Tree =
  fnew(result, gBoxedFreeGTree)
  result.impl = g_tree_new_full(keyCompareFunc, keyCompareData, keyDestroyFunc, valueDestroyFunc)

proc newTreeFull*(tdesc: typedesc; keyCompareFunc: CompareDataFunc; keyCompareData: pointer;
    keyDestroyFunc: DestroyNotify; valueDestroyFunc: DestroyNotify): tdesc =
  assert(result is Tree)
  fnew(result, gBoxedFreeGTree)
  result.impl = g_tree_new_full(keyCompareFunc, keyCompareData, keyDestroyFunc, valueDestroyFunc)

proc initTreeFull*[T](result: var T; keyCompareFunc: CompareDataFunc; keyCompareData: pointer;
    keyDestroyFunc: DestroyNotify; valueDestroyFunc: DestroyNotify) {.deprecated.} =
  assert(result is Tree)
  fnew(result, gBoxedFreeGTree)
  result.impl = g_tree_new_full(keyCompareFunc, keyCompareData, keyDestroyFunc, valueDestroyFunc)

proc g_tree_upper_bound(self: ptr Tree00; key: pointer): ptr TreeNode00 {.
    importc, libprag.}

proc upperBound*(self: Tree; key: pointer): TreeNode =
  let impl0 = g_tree_upper_bound(cast[ptr Tree00](self.impl), key)
  if impl0.isNil:
    return nil
  new(result)
  result.impl = impl0
...
type
  Tuples00* {.pure.} = object
  Tuples* = ref object
    impl*: ptr Tuples00
    ignoreFinalizer*: bool

proc g_tuples_destroy(self: ptr Tuples00) {.
    importc, libprag.}

proc destroy*(self: Tuples) =
  g_tuples_destroy(cast[ptr Tuples00](self.impl))

proc finalizerdestroy*(self: Tuples) =
  if not self.ignoreFinalizer:
    g_tuples_destroy(cast[ptr Tuples00](self.impl))

when defined(gcDestructors):
  proc `=destroy`*(self: var typeof(Tuples()[])) =
    when defined(gintroDebug):
      echo "destroy ", $typeof(self), ' ', cast[int](unsafeaddr self)
    if not self.ignoreFinalizer and self.impl != nil:
      g_tuples_destroy(self.impl)
      self.impl = nil

proc newWithFinalizer*(x: var Tuples) =
  when defined(gcDestructors):
    new(x)
  else:
    new(x, finalizerdestroy)

proc g_tuples_index(self: ptr Tuples00; index: int32; field: int32): pointer {.
    importc, libprag.}

proc index*(self: Tuples; index: int; field: int): pointer =
  g_tuples_index(cast[ptr Tuples00](self.impl), int32(index), int32(field))
...
const
  UnicodeBreakTypeCloseParenthesis* = UnicodeBreakType.closeParanthesis
...
type
  UnixPipe00* {.pure.} = object
  UnixPipe* = ref object
    impl*: ptr UnixPipe00
    ignoreFinalizer*: bool

type
  UnixPipeEnd* {.size: sizeof(cint), pure.} = enum
    read = 0
    write = 1
...
proc getHost*(self: Uri): string =
  let resul0 = g_uri_get_host(cast[ptr Uri00](self.impl))
  if resul0.isNil:
    return
  result = $resul0
...
proc lookupValue*(self: VariantDict; key: cstring; expectedType: VariantType = nil): Variant =
  let impl0 = g_variant_dict_lookup_value(cast[ptr VariantDict00](self.impl), key, if expectedType.isNil: nil else: cast[ptr VariantType00](expectedType.impl))
  if impl0.isNil:
    return nil
  fnew(result, finalizerunref)
  result.impl = impl0
...
proc alignedAlloc*(nBlocks: uint64; nBlockBytes: uint64; alignment: uint64): pointer {.
    importc: "g_aligned_alloc", libprag.}

proc alignedAlloc0*(nBlocks: uint64; nBlockBytes: uint64; alignment: uint64): pointer {.
    importc: "g_aligned_alloc0", libprag.}

proc alignedFree*(mem: pointer) {.
    importc: "g_aligned_free", libprag.}

proc alignedFreeSized*(mem: pointer; alignment: uint64; size: uint64) {.
    importc: "g_aligned_free_sized", libprag.}
...
proc g_assertion_message_cmpint(domain: cstring; file: cstring; line: int32;
    `func`: cstring; expr: cstring; arg1: uint64; cmp: cstring; arg2: uint64;
    numtype: int8) {.
    importc, libprag.}

proc assertionMessageCmpint*(domain: cstring; file: cstring; line: int;
    `func`: cstring; expr: cstring; arg1: uint64; cmp: cstring; arg2: uint64;
    numtype: int8) =
  g_assertion_message_cmpint(domain, file, int32(line), `func`, expr, arg1, cmp, arg2, numtype)
...
proc g_assertion_message_cmpstrv(domain: cstring; file: cstring; line: int32;
    `func`: cstring; expr: cstring; arg1: cstring; arg2: cstring; firstWrongIdx: uint64) {.
    importc, libprag.}

proc assertionMessageCmpstrv*(domain: cstring; file: cstring; line: int;
    `func`: cstring; expr: cstring; arg1: cstring; arg2: cstring; firstWrongIdx: uint64) =
  g_assertion_message_cmpstrv(domain, file, int32(line), `func`, expr, arg1, arg2, firstWrongIdx)
...
proc g_atomic_int_compare_and_exchange_full(atomic: ptr int32; oldval: int32;
    newval: int32; preval: var int32): gboolean {.
    importc, libprag.}

proc atomicIntCompareAndExchangeFull*(atomic: ptr int32; oldval: int;
    newval: int; preval: var int): bool =
  var preval_00: int32
  result = toBool(g_atomic_int_compare_and_exchange_full(atomic, int32(oldval), int32(newval), preval_00))
  if preval.addr != nil:
    preval = int(preval_00)
...
proc g_atomic_int_exchange(atomic: ptr int32; newval: int32): int32 {.
    importc, libprag.}

proc atomicIntExchange*(atomic: ptr int32; newval: int): int =
  int(g_atomic_int_exchange(atomic, int32(newval)))
...
proc g_atomic_pointer_compare_and_exchange_full(atomic: pointer; oldval: pointer;
    newval: pointer; preval: var pointer): gboolean {.
    importc, libprag.}

proc atomicPointerCompareAndExchangeFull*(atomic: pointer; oldval: pointer;
    newval: pointer; preval: var pointer): bool =
  toBool(g_atomic_pointer_compare_and_exchange_full(atomic, oldval, newval, preval))

proc atomicPointerExchange*(atomic: pointer; newval: pointer): pointer {.
    importc: "g_atomic_pointer_exchange", libprag.}
...
proc g_chmod(filename: cstring; mode: int32): int32 {.
    importc, libprag.}

proc chmod*(filename: cstring; mode: int): int =
  int(g_chmod(filename, int32(mode)))
...
proc g_closefrom(lowfd: int32): int32 {.
    importc, libprag.}

proc closefrom*(lowfd: int): int =
  int(g_closefrom(int32(lowfd)))
...
proc g_creat(filename: cstring; mode: int32): int32 {.
    importc, libprag.}

proc creat*(filename: cstring; mode: int): int =
  int(g_creat(filename, int32(mode)))
...
proc g_datalist_id_remove_multiple(datalist: Data; keys: ptr uint32; nKeys: uint64) {.
    importc, libprag.}

proc datalistIdRemoveMultiple*(datalist: Data; keys: seq[uint32]) =
  let nKeys = uint64(keys.len)
  g_datalist_id_remove_multiple(datalist, cast[ptr uint32](unsafeaddr(keys[0])), nKeys)
...
proc environGetenv*(envp: openArray[string]; variable: cstring): string =
  var fs469n23x: array[256, pointer]
  var fs469n23: cstringArray = cast[cstringArray](addr fs469n23x)
  let resul0 = g_environ_getenv(seq2CstringArray(envp, fs469n23), variable)
  if resul0.isNil:
    return
  result = $resul0
...
proc g_fdwalk_set_cloexec(lowfd: int32): int32 {.
    importc, libprag.}

proc fdwalkSetCloexec*(lowfd: int): int =
  int(g_fdwalk_set_cloexec(int32(lowfd)))
...
proc fopen*(filename: cstring; mode: cstring): pointer {.
    importc: "g_fopen", libprag.}
...
proc freeSized*(mem: pointer; size: uint64) {.
    importc: "g_free_sized", libprag.}

proc freopen*(filename: cstring; mode: cstring; stream: pointer): pointer {.
    importc: "g_freopen", libprag.}

proc g_fsync(fd: int32): int32 {.
    importc, libprag.}

proc fsync*(fd: int): int =
  int(g_fsync(int32(fd)))
...
proc getUserSpecialDir*(directory: UserDirectory): string =
  let resul0 = g_get_user_special_dir(directory)
  if resul0.isNil:
    return
  result = $resul0
...
proc g_get_user_state_dir(): cstring {.
    importc, libprag.}

proc getUserStateDir*(): string =
  result = $g_get_user_state_dir()

proc g_getenv(variable: cstring): cstring {.
    importc, libprag.}

proc getenv*(variable: cstring): string =
  let resul0 = g_getenv(variable)
  if resul0.isNil:
    return
  result = $resul0
...
proc g_log_get_debug_enabled(): gboolean {.
    importc, libprag.}

proc logGetDebugEnabled*(): bool =
  toBool(g_log_get_debug_enabled())
...
proc g_log_set_debug_enabled(enabled: gboolean) {.
    importc, libprag.}

proc logSetDebugEnabled*(enabled: bool = true) =
...
proc logWriterDefaultSetDebugDomains*(domains: cstring) {.
    importc: "g_log_writer_default_set_debug_domains", libprag.}

proc g_log_writer_default_set_use_stderr(useStderr: gboolean) {.
    importc, libprag.}

proc logWriterDefaultSetUseStderr*(useStderr: bool = true) =
  g_log_writer_default_set_use_stderr(gboolean(useStderr))

proc g_log_writer_default_would_drop(logLevel: LogLevelFlags; logDomain: cstring): gboolean {.
    importc, libprag.}

proc logWriterDefaultWouldDrop*(logLevel: LogLevelFlags; logDomain: cstring = nil): bool =
  toBool(g_log_writer_default_would_drop(logLevel, logDomain))
...
proc logWriterSyslog*(logLevel: LogLevelFlags; fields: ptr LogField; nFields: uint64;
    userData: pointer): LogWriterOutput {.
    importc: "g_log_writer_syslog", libprag.}

proc g_lstat(filename: cstring; buf: StatBuf): int32 {.
    importc, libprag.}

proc lstat*(filename: cstring; buf: StatBuf): int =
  int(g_lstat(filename, buf))

const macroHasAttributeNoreturn* = 0'i32

proc g_main_current_source(): ptr Source00 {.
    importc, libprag.}

proc mainCurrentSource*(): Source =
  let impl0 = g_main_current_source()
  if impl0.isNil:
    return nil
  fnew(result, gBoxedFreeGSource)
  result.impl = cast[typeof(result.impl)](g_boxed_copy(g_source_get_type(), impl0))
...
proc memdup2*(mem: pointer; byteSize: uint64): pointer {.
    importc: "g_memdup2", libprag.}

proc g_mkdir(filename: cstring; mode: int32): int32 {.
    importc, libprag.}

proc mkdir*(filename: cstring; mode: int): int =
  int(g_mkdir(filename, int32(mode)))
...
proc g_open(filename: cstring; flags: int32; mode: int32): int32 {.
    importc, libprag.}

proc open*(filename: cstring; flags: int; mode: int): int =
  int(g_open(filename, int32(flags), int32(mode)))
...
proc g_pointer_bit_lock_and_get(address: pointer; lockBit: uint32; outPtr: var uint64) {.
    importc, libprag.}

proc pointerBitLockAndGet*(address: pointer; lockBit: int; outPtr: var uint64 = cast[var uint64](nil)) =
  g_pointer_bit_lock_and_get(address, uint32(lockBit), outPtr)

proc g_pointer_bit_lock_mask_ptr(`ptr`: pointer; lockBit: uint32; set: gboolean;
    preserveMask: uint64; preservePtr: pointer): pointer {.
    importc, libprag.}

proc pointerBitLockMaskPtr*(`ptr`: pointer; lockBit: int; set: bool;
    preserveMask: uint64; preservePtr: pointer): pointer =
  g_pointer_bit_lock_mask_ptr(`ptr`, uint32(lockBit), gboolean(set), preserveMask, preservePtr)
...
proc g_pointer_bit_unlock_and_set(address: pointer; lockBit: uint32; `ptr`: pointer;
    preserveMask: uint64) {.
    importc, libprag.}

proc pointerBitUnlockAndSet*(address: pointer; lockBit: int; `ptr`: pointer;
    preserveMask: uint64) =
  g_pointer_bit_unlock_and_set(address, uint32(lockBit), `ptr`, preserveMask)
...
proc prefixErrorLiteral*(err: var ptr glib.Error; prefix: cstring) {.
    importc: "g_prefix_error_literal", libprag.}

proc propagateError*(dest: var ptr glib.Error; src: ptr glib.Error) {.
    importc: "g_propagate_error", libprag.}

proc g_qsort_with_data(pbase: pointer; totalElems: int32; size: uint64; compareFunc: CompareDataFunc;
    userData: pointer) {.
    importc, libprag.}

proc qsortWithData*(pbase: pointer; totalElems: int; size: uint64; compareFunc: CompareDataFunc;
    userData: pointer) =
  g_qsort_with_data(pbase, int32(totalElems), size, compareFunc, userData)
...
proc g_remove(filename: cstring): int32 {.
    importc, libprag.}

proc remove*(filename: cstring): int =
  int(g_remove(filename))

proc g_rename(oldfilename: cstring; newfilename: cstring): int32 {.
    importc, libprag.}

proc rename*(oldfilename: cstring; newfilename: cstring): int =
  int(g_rename(oldfilename, newfilename))
...
proc g_spawn_async_with_pipes_and_fds(workingDirectory: cstring; argv: ptr cstring;
    envp: ptr cstring; flags: SpawnFlags; childSetup: SpawnChildSetupFunc;
    userData: pointer; stdinFd: int32; stdoutFd: int32; stderrFd: int32; sourceFds: ptr int32;
    targetFds: ptr int32; nFds: uint64; childPidOut: var int32; stdinPipeOut: var int32;
    stdoutPipeOut: var int32; stderrPipeOut: var int32; error: ptr ptr glib.Error = nil): gboolean {.
    importc, libprag.}

proc spawnAsyncWithPipesAndFds*(workingDirectory: cstring = nil; argv: openArray[string];
    envp: openArray[string]; flags: SpawnFlags; childSetup: SpawnChildSetupFunc;
    userData: pointer; stdinFd: int; stdoutFd: int; stderrFd: int; sourceFds: seq[int32];
    targetFds: seq[int32]; childPidOut: var int = cast[var int](nil); stdinPipeOut: var int = cast[var int](nil);
    stdoutPipeOut: var int = cast[var int](nil); stderrPipeOut: var int = cast[var int](nil)): bool =
  let nFds = uint64(targetFds.len)
  var fs469n23x: array[256, pointer]
  var fs469n23: cstringArray = cast[cstringArray](addr fs469n23x)
  var fs469n232x: array[256, pointer]
  var fs469n232: cstringArray = cast[cstringArray](addr fs469n232x)
  var gerror: ptr glib.Error
  var stdinPipeOut_00: int32
  var childPidOut_00: int32
  var stderrPipeOut_00: int32
  var stdoutPipeOut_00: int32
  let resul0 = g_spawn_async_with_pipes_and_fds(workingDirectory, seq2CstringArray(argv, fs469n23), seq2CstringArray(envp, fs469n232), flags, childSetup, userData, int32(stdinFd), int32(stdoutFd), int32(stderrFd), cast[ptr int32](unsafeaddr(sourceFds[0])), cast[ptr int32](unsafeaddr(targetFds[0])), nFds, childPidOut_00, stdinPipeOut_00, stdoutPipeOut_00, stderrPipeOut_00, addr gerror)
  if gerror != nil:
    let msg = $gerror.message
    g_error_free(gerror[])
    raise newException(GException, msg)
  result = toBool(resul0)
  if stdinPipeOut.addr != nil:
    stdinPipeOut = int(stdinPipeOut_00)
  if childPidOut.addr != nil:
    childPidOut = int(childPidOut_00)
  if stderrPipeOut.addr != nil:
    stderrPipeOut = int(stderrPipeOut_00)
  if stdoutPipeOut.addr != nil:
    stdoutPipeOut = int(stdoutPipeOut_00)

proc g_spawn_check_exit_status(waitStatus: int32; error: ptr ptr glib.Error = nil): gboolean {.
    importc, libprag.}

proc spawnCheckExitStatus*(waitStatus: int): bool =
  var gerror: ptr glib.Error
  let resul0 = g_spawn_check_exit_status(int32(waitStatus), addr gerror)
  if gerror != nil:
    let msg = $gerror.message
    g_error_free(gerror[])
    raise newException(GException, msg)
...
proc g_spawn_check_wait_status(waitStatus: int32; error: ptr ptr glib.Error = nil): gboolean {.
    importc, libprag.}

proc spawnCheckWaitStatus*(waitStatus: int): bool =
  var gerror: ptr glib.Error
  let resul0 = g_spawn_check_wait_status(int32(waitStatus), addr gerror)
  if gerror != nil:
    let msg = $gerror.message
    g_error_free(gerror[])
    raise newException(GException, msg)
...
proc g_stat(filename: cstring; buf: StatBuf): int32 {.
    importc, libprag.}

proc stat*(filename: cstring; buf: StatBuf): int =
  int(g_stat(filename, buf))
...
proc strTokenizeAndFold*(string: cstring; translitLocale: cstring = nil;
    asciiAlternates: var seq[string] = cast[var seq[string]](nil)): seq[string] =
  var fs469n23x: array[256, pointer]
  var fs469n23: cstringArray = cast[cstringArray](addr fs469n23x)
...
proc g_strdupv(strArray: ptr cstring): ptr cstring {.
    importc, libprag.}

proc strdupv*(strArray: varargs[string, `$`]): seq[string] =
  var fs469n23x: array[256, pointer]
  var fs469n23: cstringArray = cast[cstringArray](addr fs469n23x)
  let resul0 = g_strdupv(seq2CstringArray(strArray, fs469n23))
  if resul0.isNil:
    return
  result = cstringArrayToSeq(resul0)
  g_strfreev(resul0)
...
proc g_strjoinv(separator: cstring; strArray: ptr cstring): cstring {.
    importc, libprag.}

proc strjoinv*(separator: cstring = nil; strArray: varargs[string, `$`]): string =
  var fs469n23x: array[256, pointer]
  var fs469n23: cstringArray = cast[cstringArray](addr fs469n23x)
  let resul0 = g_strjoinv(separator, seq2CstringArray(strArray, fs469n23))
  result = $resul0
  cogfree(resul0)
...
proc strndup*(str: cstring = nil; n: uint64): string =
...
proc g_strsplit(string: cstring; delimiter: cstring; maxTokens: int32): ptr cstring {.
    importc, libprag.}

proc strsplit*(string: cstring; delimiter: cstring; maxTokens: int): seq[string] =
  let resul0 = g_strsplit(string, delimiter, int32(maxTokens))
  if resul0.isNil:
    return
  result = cstringArrayToSeq(resul0)
  g_strfreev(resul0)

proc g_strsplit_set(string: cstring; delimiters: cstring; maxTokens: int32): ptr cstring {.
    importc, libprag.}

proc strsplitSet*(string: cstring; delimiters: cstring; maxTokens: int): seq[string] =
  let resul0 = g_strsplit_set(string, delimiters, int32(maxTokens))
  if resul0.isNil:
    return
  result = cstringArrayToSeq(resul0)
  g_strfreev(resul0)
...
proc g_strv_contains(strv: ptr cstring; str: cstring): gboolean {.
    importc, libprag.}

proc strvContains*(strv: openArray[string]; str: cstring): bool =
  var fs469n23x: array[256, pointer]
  var fs469n23: cstringArray = cast[cstringArray](addr fs469n23x)
  toBool(g_strv_contains(seq2CstringArray(strv, fs469n23), str))

proc g_strv_equal(strv1: ptr cstring; strv2: ptr cstring): gboolean {.
    importc, libprag.}

proc strvEqual*(strv1: openArray[string]; strv2: varargs[string, `$`]): bool =
  var fs469n23x: array[256, pointer]
  var fs469n23: cstringArray = cast[cstringArray](addr fs469n23x)
  var fs469n232x: array[256, pointer]
  var fs469n232: cstringArray = cast[cstringArray](addr fs469n232x)
  toBool(g_strv_equal(seq2CstringArray(strv1, fs469n23), seq2CstringArray(strv2, fs469n232)))

proc strvGetType*(): GType {.
    importc: "g_strv_get_type", libprag.}

proc g_strv_length(strArray: ptr cstring): uint32 {.
    importc, libprag.}
...
proc testDisableCrashReporting*() {.
    importc: "g_test_disable_crash_reporting", libprag.}
...
proc g_test_get_path(): cstring {.
    importc, libprag.}

proc testGetPath*(): string =
  result = $g_test_get_path()
...
proc g_test_trap_subprocess_with_envp(testPath: cstring; envp: ptr cstring;
    usecTimeout: uint64; testFlags: TestSubprocessFlags) {.
    importc, libprag.}

proc testTrapSubprocessWithEnvp*(testPath: cstring = nil; envp: openArray[string];
    usecTimeout: uint64; testFlags: TestSubprocessFlags) =
  var fs469n23x: array[256, pointer]
  var fs469n23: cstringArray = cast[cstringArray](addr fs469n23x)
  g_test_trap_subprocess_with_envp(testPath, seq2CstringArray(envp, fs469n23), usecTimeout, testFlags)
...
proc g_unichar_get_mirror_char(ch: gunichar; mirroredCh: var gunichar): gboolean {.
    importc, libprag.}

proc unicharGetMirrorChar*(ch: gunichar; mirroredCh: var gunichar): bool =
...
proc g_unix_open_pipe(fds: ptr array[2, int32]; flags: int32; error: ptr ptr glib.Error = nil): gboolean {.
    importc, libprag.}

proc unixOpenPipe*(fds: ptr array[2, int32]; flags: int): bool =
  var gerror: ptr glib.Error
...
proc g_utf8_truncate_middle(string: cstring; truncateLength: uint64): cstring {.
    importc, libprag.}

proc utf8TruncateMiddle*(string: cstring; truncateLength: uint64): string =
  let resul0 = g_utf8_truncate_middle(string, truncateLength)
  result = $resul0
  cogfree(resul0)
...
proc g_utime(filename: cstring; utb: pointer): int32 {.
    importc, libprag.}

proc utime*(filename: cstring; utb: pointer): int =
  int(g_utime(filename, utb))
